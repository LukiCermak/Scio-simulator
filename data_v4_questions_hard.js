window.SCIO_V4_RAW_BATTERIES_HARD = [
  {
    "id": 1,
    "label": "Baterie 1",
    "title": "Funkční posouzení obtíže a primární bariéra",
    "subtitle": "Krátké diferenční položky na přístup k informaci, formu výkonu, podmínky prostředí a funkční dopad.",
    "purpose": "Ověřit, zda uchazeč odliší přístup k informaci od porozumění, formu výkonu od kompetence a hlavní bariéru od sekundárního projevu bez zkratky podle diagnózy nebo povrchového dojmu.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "přístup k informaci vs porozumění",
      "forma výkonu vs kompetence",
      "prostředí vs schopnost",
      "hlavní bariéra vs sekundární projev",
      "diagnóza vs funkční dopad",
      "kanál vyjádření vs obsahové chápání",
      "tempo, únava a regulační zátěž"
    ],
    "breakdown": [
      "16× bariéra × forma výkonu",
      "12× prostředí × schopnost",
      "10× přístup k informaci × porozumění",
      "6× stabilita obtíže napříč situacemi",
      "6× účinnost a neúčinnost podpory"
    ],
    "tags": [
      "funkční posouzení obtíže",
      "primární bariéra",
      "přístup k informaci",
      "forma výkonu",
      "UPOL",
      "poradenství"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Žák selhává stejně v ústní, písemné i názorně vedené odpovědi na totéž učivo. Jaký závěr by zde byl nejméně přiměřený?",
        "options": [
          "Důležité je neplést modalitu výkonu s podstatou potíže.",
          "Že bariéra bude jen ve formě výstupu.",
          "Že samotná změna výstupního kanálu problém nevyřešila.",
          "Že je potřeba znovu sledovat, co zůstává stabilní napříč modalitami."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 2,
        "text": "V ústním zkoušení žák látku vystihne přesně, ale v ručně psané odpovědi ji nerozvine a text je sotva čitelný. Které vysvětlení situace je nejpřesnější?",
        "options": [
          "Obtíž souvisí spíše s formě písemného výstupu než nutně v obsahu znalosti.",
          "Obtíž souvisí především s celkovým neporozumění učivu, které se jen více odhalí písemně.",
          "Obtíž souvisí s nízkou ochotou pracovat, protože ústně je výkon vždy snadnější.",
          "Nejpřesnější je vycházet z toho, že žák zvládá učivo jen tehdy, když může odpovídat zkratkovitě."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 3,
        "text": "Žák při práci s pomůckami a při ústním vedení postup chápe, ale v rychle vyplňovaném pracovním listu se ztrácí. Co z toho plyne nejpřesněji?",
        "options": [
          "Že se ukazuje spíše rozdíl mezi podmínkami a formou výkonu než jistý důkaz nepochopení principu.",
          "Že je nejpravděpodobnější globální rozumové oslabení bez ohledu na formu úkolu, a právě tato rovina by měla být považována za rozhodující.",
          "Že je vhodné uzavřít situaci primárně jako problém motivace k samostatné práci, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Že je rozhodující zejména to, že pomůcky úkol zjednodušují bez vztahu ke skutečné kompetenci, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 4,
        "text": "Který znak nejvíce ukazuje, že změna výstupního kanálu opravdu zasáhla hlavní bariéru?",
        "options": [
          "Že výkon se zlepší jen v jedné mimořádně podpůrné hodině, a právě tato rovina by měla být považována za rozhodující.",
          "Že po změně formy odpovědi se posune výkon právě v cílové situaci, kde žák dříve selhával.",
          "Že učitel získá lepší dojem ze spolupráce žáka než předtím, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Že nová forma výkonu působí odborněji a je snazší ji administrativně popsat, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 5,
        "text": "Žák při diktátu zřetelně zaostává, ale při ústním převyprávění a při výběru správného řešení z možností drží obsah přesně. Která interpretace je nejméně přesná?",
        "options": [
          "Potíž se nemusí týkat obsahu učiva v té podobě, v jaké ji ukazuje diktát, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Je třeba odlišit obsahovou znalost od nároků konkrétní formy výkonu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Výsledek diktátu sám o sobě nestačí k závěru, že žák látce nerozumí, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Diktát tu pravděpodobně odhaluje stejnou hlavní potíž jako všechny ostatní formy výkonu."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 6,
        "text": "Žák mluví srozumitelně, ale při výkladu z větší vzdálenosti bez vizuální opory ztrácí obsah. Který rámec je bližší?",
        "options": [
          "Logopedický rámec je bližší, protože podstata leží v kvalitě výslovnosti žáka, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Surdopedický rámec dostupnosti mluvené informace je bližší než logopedický rámec produkce.",
          "Psychologický rámec je bližší, protože potíž vzniká hlavně v koncentraci pozornosti bez ohledu na podmínky.",
          "Sociální rámec je bližší, protože problém je především v organizaci třídy a ne v přístupu ke sdělení."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 7,
        "text": "Kdy je nepřesné vykládat situaci primárně logopedicky?",
        "options": [
          "Když se deformace hlásek objevuje stabilně i v klidu a při zachovaném porozumění, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Když je řeč málo plynulá a dítě potřebuje práci s komunikační produkcí, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Když přetrvává nápadná artikulační nepřesnost bez vazby na hluk nebo vzdálenost, a právě tato rovina by měla být považována za rozhodující.",
          "Když se obsah rozpadá hlavně v hluku, na vzdálenost a bez vizuální opory, přestože výslovnost je funkční."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 8,
        "text": "Který znak ukazuje spíše k dostupnosti sdělení než k poruše samotné mluvní produkce?",
        "options": [
          "Výslovnost je stabilně nepřesná i v klidném individuálním rozhovoru, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Obsah se rozpadá zejména tehdy, když je sdělení rychlé, vzdálené nebo bez vizuální opory.",
          "Řeč je dlouhodobě neplynulá bez zjevné vazby na komunikační podmínky, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Chyby ve hláskách se objevují i při pomalém opakování po učiteli, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 9,
        "text": "Žák po poslechu textu odpoví věcně správně, ale z hustě sázeného materiálu s nízkým kontrastem selhává. Jaké vysvětlení je nejpřesnější?",
        "options": [
          "Obtíž souvisí spíše s přístupu k vizuálně podané informaci než v obecném porozumění textu.",
          "Nejpřesnější je vycházet z toho, že žák nerozumí stejnému obsahu bez ohledu na formu podání, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Obtíž souvisí s odporem k samostatné práci s textem, nikoli ve vizuálních podmínkách, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Nejpřesnější je vycházet z toho, že poslech vždy uměle nadhodnocuje skutečnou kompetenci, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 10,
        "text": "Po zvětšení textu a zvýšení kontrastu se výkon v cílovém úkolu výrazně zlepší. Který závěr by z toho byl nejméně přesný?",
        "options": [
          "Změna dostupnosti materiálu pravděpodobně zasáhla hlavní bariéru, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Původní výkon nebyl nutně přímým ukazatelem obsahové neznalosti, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde nejspíše o důkaz, že žák bez podpory textu nerozumí bez ohledu na podobu podání.",
          "Je vhodné odlišit bariéru ve vizuálním přístupu od závěru o celkové úrovni znalosti."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 11,
        "text": "V tiché individuální práci žák běžné instrukci rozumí, ale ve skupinové práci v hluku opakovaně mine začátek úkolu. Která interpretace je bližší?",
        "options": [
          "Potíž je v dostupnosti sdělení v náročných podmínkách více než v skutečném neporozumění zadání.",
          "Potíž je stejná napříč situacemi a hluk jen odkryl všeobecně slabé chápání, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Potíž je především v neochotě reagovat na autoritu učitele ve skupině, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Potíž je hlavně v tom, že individuální práce bývá vždy jednodušší než skupinová, bez dalšího významu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 12,
        "text": "Jak nejlépe vyložit situaci, kdy dítě provede úkol správně po krátké demonstraci, ale samotný rychle vyslovený slovní pokyn v tělocvičně často mine?",
        "options": [
          "Obtíž souvisí spíše s dostupnosti a formě podání instrukce než v samotném pohybovém principu úkolu.",
          "Obtíž souvisí s celkově slabém chápání pohybového zadání bez ohledu na formu instrukce, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Nejpřesnější je vycházet z toho, že dítě v tělocvičně systematicky vzdoruje verbálním pokynům, a právě tato rovina by měla být považována za rozhodující.",
          "Nejpřesnější je vycházet z toho, že demonstrace je vždy nepřiměřenou nápovědou a výsledek proto nelze spolehlivě posoudit, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 13,
        "text": "Žák čte nahlas plynule, ale u otázek na vztahy mezi postavami a motivy jejich jednání opakovaně tápe. Co z toho plyne nejspíše?",
        "options": [
          "Selhává hlavně inferenční a vztahové porozumění textu, ne nutně samotné čtení nahlas.",
          "Selhává hlavně mluvní produkce, protože nahlas čte bez jistoty významu každého slova.",
          "Selhává hlavně zraková ostrost, protože vztahy v textu bývají náročné na sledování řádků.",
          "Selhává hlavně disciplína při práci s literárním textem, protože odpovědi nejsou doslovné."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 14,
        "text": "Co by v této situaci bylo nejméně přesné tvrdit, když dítě po názoru a rozfázování úkolu postup zvládne, ale bez této opory se ztratí už v prvních krocích?",
        "options": [
          "Je vhodné odlišit dostupnost a strukturu zadání od závěru o celkové nekompetenci, a právě tato rovina by měla být považována za rozhodující.",
          "Potíž může ležet v pochopení a adaptivním zvládnutí úkolu, ne nutně v odporu ke spolupráci.",
          "Podpůrná struktura mění podmínky výkonu a ukazuje, co je pro dítě rozhodující, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde především o důkaz, že dítě problém zvládá zcela bez obtíží a podpora je zbytečná."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 15,
        "text": "Dítě rozumí doslovnému obsahu, ale selhává v narážce, ironii a změně perspektivy partnera. Které vysvětlení je bližší?",
        "options": [
          "Jde spíše o sociálně-pragmatické porozumění než o pouhý deficit ve slyšení nebo artikulaci.",
          "Jde spíše o potíž ve výslovnosti, protože význam narážky závisí hlavně na mluvní produkci, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde spíše o vizuální bariéru, protože změna perspektivy bývá založena na vnímání neverbálních znaků.",
          "Jde spíše o nízkou ochotu spolupracovat, protože dítě doslovný obsah chápe, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 16,
        "text": "Žák chápe princip příkladu, když je zadání rozfázované a zvýrazněné, ale v kompaktně zapsaném bloku instrukcí opakovaně vynechá rozhodující krok. Co z toho plyne nejpřesněji?",
        "options": [
          "Problém leží spíše v přístupu ke struktuře zadání než jistě v nepochopení matematického principu.",
          "Problém leží jistě v tom, že žák nerozumí postupu a podpora jen zakrývá skutečný deficit, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Problém leží hlavně v nízké sebekontrole při práci s písemným materiálem, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Problém leží v tom, že kompaktně zapsané zadání má vždy vyšší obsahovou náročnost, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 17,
        "text": "Která informace nejlépe pomůže učiteli rozlišit přístup k informaci od jejího porozumění?",
        "options": [
          "Zda se výkon mění po změně podmínek podání stejného obsahu.",
          "Zda je žák celkově klidnější než na začátku pololetí, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Zda se obtíž objevila i v jiném předmětu bez bližšího popisu situace.",
          "Zda rodič považuje domácí přípravu za dostatečnou, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 18,
        "text": "Který znak nejvíce ukazuje na stabilní podstatu obtíže napříč situacemi?",
        "options": [
          "Žák selhává hlavně při jednom typu úkolu a po změně podmínek se výkon vrací.",
          "Stejný typ chyby přetrvává i po změně formy odpovědi, tempa i míry názoru.",
          "Obtíž se objevuje jen tehdy, když je zadání dlouhé a hlučné prostředí rušivé.",
          "Výkon kolísá podle toho, zda učitel pracuje individuálně nebo frontálně, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 19,
        "text": "Co by zde bylo nejméně přesné vyvozovat, když se potíž objevuje jen při rychlém tempu a po zpomalení stejného zadání mizí?",
        "options": [
          "Výkon je citlivý na tempo podání, ne nutně stejně oslabený ve všech podmínkách.",
          "Před závěrem o stabilní globální potíži je vhodné zohlednit roli podmínek výkonu.",
          "Jde o potvrzení, že stejná bariéra přetrvává bez ohledu na podobu zadání.",
          "Tempo může fungovat jako rozhodující podmínka dostupnosti úkolu, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 20,
        "text": "Když žák chybuje stejně u tichého čtení, poslechu i praktické ukázky, který závěr je bližší?",
        "options": [
          "Potíž bude pravděpodobně stabilnější a méně vázané na konkrétní kanál podání.",
          "Potíž bude pravděpodobně jen ve vizuálním přístupu k informaci, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Potíž bude pravděpodobně jen v produkci odpovědi, ne v porozumění, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Potíž bude pravděpodobně jen v akustických podmínkách výuky, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 21,
        "text": "Žák v každém předmětu selhává až tehdy, když má odpověď zapsat vlastní rukou; ústně nebo na počítači drží obsah výrazně lépe. Jaké vysvětlení je zde nejpřesnější?",
        "options": [
          "Stabilní je spíše bariéra ve specifické formě výkonu než jistě v obsahu kompetence napříč předměty.",
          "Stabilní je obsahová neznalost, která se ústně jen hůř odhaluje, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Stabilní je porucha porozumění zadání bez ohledu na způsob odpovědi, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Stabilní je hlavně motivační odmítání školní práce jako takové, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 22,
        "text": "Která interpretace je předčasná, když výkon kolísá podle hluku, vzdálenosti a vizuální opory?",
        "options": [
          "Že jde o potíž úplně nezávislou na podmínkách prostředí.",
          "Že je potřeba sledovat, které podmínky výkon zhoršují a které jej zpřístupňují.",
          "Že změna prostředí může být pro čtení obtíže stejně důležitá jako změna úkolu.",
          "Že kolísání výkonu samo o sobě brání rychlému závěru o stejné obtíži napříč situacemi."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 23,
        "text": "Který indikátor nejvíc svědčí pro to, že opatření míří mimo hlavní bariéru?",
        "options": [
          "Že vyžaduje od týmu krátké zaučení před zavedením, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Že mění něco nápadného, ale výkon se v cílové situaci téměř neposouvá.",
          "Že je potřeba ho popsat konkrétněji než obecnou větou, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Že se jeho účinek musí sledovat v čase, ne jen po jedné hodině, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 24,
        "text": "Který ukazatel nejlépe umožní vyhodnotit, zda navržená podpora opravdu funguje?",
        "options": [
          "Že škola hodnotí spolupráci rodiny jako lepší než dřív, a právě tato rovina by měla být považována za rozhodující.",
          "Že výstup odborně zní a obsahuje přesnou terminologii, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Že je předem jasné, v jaké situaci a podle čeho se změna pozná.",
          "Že se po předání zprávy už nikdo nevrací s doplňujícími dotazy."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 25,
        "text": "Na co je při první konzultaci s učitelem nejdůležitější se zeptat pro další poradenský postup?",
        "options": [
          "Který žák ve třídě vás zatěžuje nejvíc obecně?",
          "Jakou kategorii obtíží byste od poradny potřeboval potvrdit?",
          "V jaké konkrétní situaci se obtíž ukazuje a co ji zhoršuje nebo mírní?",
          "Kdy byste chtěl mít hotový formální dokument?"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 26,
        "text": "Škola zavede úpravu, po níž žák působí klidněji, ale v situaci, kvůli níž byla podpora nastavena, výkon zůstává beze změny. Co z toho plyne nejpřesněji?",
        "options": [
          "Podpora pravděpodobně zasáhla vedlejší projev, ne nutně hlavní bariéru.",
          "Podpora je funkční, protože zlepšila celkovou atmosféru práce, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Podpora je jistě nefunkční, protože každá účinná změna musí odstranit obtíž úplně.",
          "Podpora je dostačující, pokud učitel subjektivně cítí menší zátěž, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 27,
        "text": "Jak nejlépe vyložit situaci, kdy se výkon zlepší jen při velmi silně vedené individuální opoře, ale v běžné třídní situaci se potíž vrací?",
        "options": [
          "Opatření už prokázalo účinnost v cílové školní situaci, protože žák umí úkol s podporou zvládnout.",
          "Je třeba odlišit zlepšení v podpůrně nasycené situaci od skutečného posunu v běžné cílové situaci.",
          "Zlepšení v individuální situaci stačí k závěru, že hlavní bariéra byla odstraněna, a právě tato rovina by měla být považována za rozhodující.",
          "Pokud se výkon zlepší někde, není už důvod sledovat, kde se potíž vrací, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 28,
        "text": "Žák při diktátu selhává, ale při pomalém přečtení téhož textu s možností průběžně se vracet drží obsah i pravopis lépe. Co by z toho bylo nejméně přesné vyvozovat?",
        "options": [
          "Potíž může souviset i s tempem a podmínkami zpracování, ne jen s výsledným počtem chyb.",
          "Je vhodné odlišit výkon v jednorázově plynoucím sdělení od výkonu s možností návratu k informaci.",
          "Diktát sám o sobě nemusí přesně ukazovat rozsah hlavní obtíže, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Výsledek potvrzuje, že stejná potíž trvá beze změny bez ohledu na podmínky zpracování."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 29,
        "text": "Dítě při samostatném řešení působí jako nepozorné, ale po rozfázování úkolu a konkrétní opoře se ukáže, že hlavní bariéra byla v pochopení a adaptivním zvládání kroku. Co je přesnější?",
        "options": [
          "Přesnější je rychlé etopedické uzavření, protože nápadný projev bývá hlavním odborným signálem.",
          "Přesnější je posoudit obtíž vývojově a funkčně, ne vycházet jen z viditelného sekundárního projevu.",
          "Přesnější je uzavřít situaci jako čistě motivační problém, protože po opoře se dítě uklidnilo, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Přesnější je hledat hlavně školní nekázeň, protože úkol působil na dítě náročně, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 30,
        "text": "Při stejné instrukci dítě potřebuje konkrétní názornost, více času a podporu v běžných adaptivních krocích; problém nespočívá hlavně ve vzdoru. Co je bližší?",
        "options": [
          "Bližší je etopedický rámec, protože potřeba času obvykle kryje problém v regulaci chování.",
          "Bližší je psychopedický nebo širší vývojový rámec než rychlé etopedické vysvětlení.",
          "Bližší je logopedický rámec, protože názornost kompenzuje hlavně řečovou produkci.",
          "Bližší je sociální rámec, protože jde především o podporu rodiny mimo školu, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 31,
        "text": "Kdy je nepřesné vykládat situaci primárně tyflopedicky?",
        "options": [
          "Když dítě potřebuje kontrast, zvětšení a přesnější orientaci v ploše textu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Když výkon kolísá podle vizuální vzdálenosti a kvality zobrazení, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Když textový obsah po zpřístupnění drží, ale bariéra zůstává ve vizuálním přístupu, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Když hlavní bariéra leží v poloze, úchopu, manipulaci a motorickém provedení, ne ve vizuálním přístupu."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 32,
        "text": "Kdy je chyba vyvodit z pomalého zápisu zrakovou bariéru?",
        "options": [
          "Když je přítomné i zhoršení v kontrastu, v dálce a ve sledování řádků, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Když po zvětšení a kontrastu stále přetrvává hlavní bariéra ve fyzickém provedení a vytrvalosti ruky.",
          "Když dítě používá vizuální orientační body a pomůcky pro čtení, a právě tato rovina by měla být považována za rozhodující.",
          "Když je potřeba upravit vizuální přístup k materiálu, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 33,
        "text": "Jaké vysvětlení situace je nejpřesnější, když dítě rozumí otázce při individuální práci, ale ve třídě opakovaně odpovídá mimo, protože zachytí jen část zadání?",
        "options": [
          "Obtíž souvisí spíše s dostupnosti zadání v konkrétních podmínkách než nutně ve stejném neporozumění v každé situaci.",
          "Obtíž souvisí s stabilně slabé obsahové kompetenci bez ohledu na to, jak je sdělení podané, a právě tato rovina by měla být považována za rozhodující.",
          "Nejpřesnější je vycházet z toho, že individuální práce neukazuje skutečnou úroveň žáka, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Obtíž souvisí hlavně s neochotě sledovat výuku ve větší skupině, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 34,
        "text": "Žák po úpravě pracovního místa a zjednodušení vizuálního pole drží pozornost i obsah úkolu lépe, přestože předtím působil jako roztěkaný. Co by z toho bylo nejméně přesné vyvozovat?",
        "options": [
          "Úprava podmínek mohla zasáhnout hlavní bariéru, ne jen chování na povrchu, a právě tato rovina by měla být považována za rozhodující.",
          "Je vhodné odlišit sekundární neklid od příčiny, která neklid vyvolávala, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Zlepšení po úpravě podmínek svědčí proti rychlému moralizujícímu uzavření, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde především o potvrzení, že původní problém byl v osobnostním nastavení a úprava prostředí je vedlejší."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 35,
        "text": "Co z toho plyne nejpřesněji, když se chyba objevuje jen při delším vícefázovém zadání, ale u krátkých jednoznačných pokynů dítě postup zvládá?",
        "options": [
          "Rozhodující je odlišit délku a strukturu zadání od závěru, že dítě nerozumí i jednoduchému principu.",
          "Rozhodující je uzavřít situaci jako stejné neporozumění bez ohledu na složitost instrukce, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Rozhodující je vykládat potíž především jako nezájem o náročnější úkoly, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Rozhodující je předpokládat, že delší zadání jen více odhalilo problém chování, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 36,
        "text": "Který údaj má pro funkční posouzení obtíže nejmenší vypovídací hodnotu?",
        "options": [
          "Přesně popsaná situace, v níž se chyba objevuje, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Popis toho, co výkon zhoršuje nebo naopak zpřístupňuje, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Obecné označení, že žák je dlouhodobě problémový bez vazby na konkrétní úkol.",
          "Srovnání výkonu při odlišných podmínkách téhož úkolu, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 37,
        "text": "Co by v této situaci bylo nejméně vhodné tvrdit, když dítě po zopakování a zpomalení stejné instrukce postup provede správně?",
        "options": [
          "Původní selhání nemusí automaticky znamenat, že dítě principu nerozumělo, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Rozhodující může být tempo a dostupnost sdělení, ne nutně samotný obsah úkolu.",
          "Je vhodné sledovat, zda se zlepšení objeví i v běžné rychlosti a bez dodatečné opory.",
          "Jde o jasný důkaz, že podmínky nejsou pro výkon důležité a chyba byla náhodná."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 38,
        "text": "Dítě v testu s výběrem možností odpovídá přesněji než v otevřené produkci, přestože typ učiva zůstává stejný. Co z toho plyne nejpřesněji?",
        "options": [
          "Rozdíl může ležet ve formě vyjádření výkonu, ne nutně v tom, že by učivo umělo jen v jedné podobě.",
          "Rozdíl dokazuje, že uzavřené úlohy nikdy nemají vypovídací hodnotu, a právě tato rovina by měla být považována za rozhodující.",
          "Rozdíl ukazuje, že otevřená produkce je jediným spolehlivým zdrojem o skutečné znalosti, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Rozdíl automaticky potvrzuje, že žák nerozumí, jen náhodně poznává správné možnosti, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 39,
        "text": "Jaké vysvětlení je nejbližší, když se výkon po úpravě podmínek zlepšuje jen v těch úkolech, které jsou na dané podmínky citlivé, ale v ostatních zůstává stejný?",
        "options": [
          "Podpora pravděpodobně cílí specificky na část bariéry, která je vázaná na konkrétní podmínky úkolu.",
          "Podpora je plošně neúčinná, protože nezlepšila výkon ve všech oblastech najednou, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Podpora je zbytečná, protože některé výkony byly uspokojivé i bez ní, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Podpora dokazuje, že hlavní bariéra byla čistě obsahová a podmínky s ní nesouvisejí, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 40,
        "text": "Jak nejlépe vyložit situaci, kdy učitel popisuje, že žák „to někdy umí a někdy ne“, ale podrobnější rozbor ukáže jasnou vazbu na hluk, vzdálenost a hustotu materiálu?",
        "options": [
          "Kolísání výkonu je zde vhodnější vysvětlit podle podmínek než jako důkaz zcela nevyzpytatelné kompetence.",
          "Kolísání výkonu je zde nejspíše náhodné a bez analytické hodnoty, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Kolísání výkonu je zde nejspíše potvrzením stejné hlavní obtíže bez vazby na situaci, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Kolísání výkonu je zde hlavně známkou neochoty žáka pracovat soustavně, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 41,
        "text": "Žák po vysvětlení úkolu odpoví správně, ale při samostatném převodu stejného zadání do pracovního postupu se ztratí. Které vysvětlení situace je zde nejbližší?",
        "options": [
          "Rozhodující může být přechod od porozumění k samostatnému organizačnímu zvládnutí úkolu.",
          "Rozhodující je uzavřít situaci jako jisté nepochopení už na úrovni instrukce, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Rozhodující je hledat hlavně poruchu chování, protože při samostatnosti chyba narůstá, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Rozhodující je to, že učitel při vysvětlení úkol mimoděk zjednodušil do nepoužitelné podoby."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 42,
        "text": "Které tvrzení má největší vypovídací hodnotu pro posouzení, zda podpora opravdu mění podstata obtíže?",
        "options": [
          "Že je popsáno, v jaké konkrétní situaci se měl výkon posunout, a právě tam se skutečně posunul.",
          "Že učitel má po zavedení podpory lepší pocit z práce celé třídy, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Že žák působí s novým opatřením klidněji, i když cíl podpory mířil jinam, a právě tato rovina by měla být považována za rozhodující.",
          "Že opatření lze odborně pojmenovat několika přesnými termíny, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 43,
        "text": "Co by v této situaci bylo nejméně vhodné tvrdit, když žák při stejném obsahu selhává pouze tehdy, když musí odpověď formulovat bez nabídky opory, ale při strukturované opoře se drží věcně správně?",
        "options": [
          "Je vhodné odlišit náročnost samostatné produkce od závěru o úplné neznalosti.",
          "Rozdíl může ležet ve formě vybavení a uspořádání odpovědi, ne pouze v obsahu učiva.",
          "Jde o potvrzení, že žák stejný obsah ve skutečnosti nezvládá ani s oporou.",
          "Je třeba sledovat, zda se podpora přenáší i do méně vedených situací, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 44,
        "text": "V jednom typu hodin žák působí neobratně a zpomaleně, ale při stejné obsahové náročnosti v jiném uspořádání činnosti drží výkon výrazně lépe. Které vysvětlení je přesnější?",
        "options": [
          "Je vhodné hledat, které uspořádání činnosti bariéru zesiluje a které ji naopak tlumí.",
          "Je vhodné uzavřít, že podstata potíže leží mimo podmínky a rozdíl mezi hodinami nemá význam.",
          "Je vhodné uzavřít, že lepší výkon v jedné situaci je pouze náhodný výkyv bez diagnostické hodnoty.",
          "Je vhodné vysvětlit rozdíl hlavně jako důsledek vztahu žáka k vyučujícímu, bez zkoumání úkolu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 45,
        "text": "Jaké vysvětlení je nejpřesnější, když se po rozdělení úkolu na menší kroky zlepší provedení, ale ne nutně rychlost?",
        "options": [
          "Podpora může zpřístupnit organizaci a pochopení postupu, aniž by okamžitě odstranila všechny sekundární dopady.",
          "Podpora je neúčinná, protože nevedla k celkovému zrychlení výkonu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Podpora je účinná jen tehdy, když se současně zlepší i chování, tempo a samostatnost bez výjimky, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Podpora potvrzuje, že původní problém byl pouze v nedostatku snahy, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 46,
        "text": "Jaké vysvětlení je nejbližší, když dítě po přesunu blíže k učiteli a po zřetelnějším kontaktu s tváří drží obsah podstatně lépe, zatímco artikulace zůstává stejná?",
        "options": [
          "Obtíž souvisí spíše s dostupnosti sdělení než v samotné kvalitě mluvní produkce dítěte.",
          "Obtíž souvisí spíše s artikulaci, protože pozice ve třídě mění hlavně její vnímání učitelem.",
          "Obtíž souvisí spíše s tom, že dítě po přesazení více poslouchá z kázně, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Obtíž souvisí spíše s obsahu učiva, který se po přesunu blíže automaticky zjednodušil, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 47,
        "text": "Na kterou otázku je pro první funkční zpřesnění potíže nejméně užitečné se ptát?",
        "options": [
          "Ve které konkrétní situaci se obtíž ukazuje nejzřetelněji?",
          "Co výkon zhoršuje a co jej naopak zpřístupňuje?",
          "Co zůstává stejné i po změně formy odpovědi nebo podmínek podání?",
          "Do které diagnostické kategorie bychom dítě nejrychleji zařadili, abychom mohli pokračovat?"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 48,
        "text": "Co z toho plyne nejpřesněji, když dítě při stejné látce selhává hlavně v dlouhých souvislých instrukcích, ale nikoli v krátkých postupných krocích?",
        "options": [
          "Obtíž může souviset se zpracováním struktury a délky zadání, ne nutně v tom, že dítě látce nerozumí vůbec.",
          "Obtíž by musela souviset s totálním nepochopení látky, protože dlouhé instrukce jen odhalují pravdu, a právě tato rovina by měla být považována za rozhodující.",
          "Obtíž by musela souviset se vzdorem, protože dlouhé instrukce vyžadují více sebekontroly, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Obtíž by musela souviset s nečitelností materiálu, protože delší instrukce bývají graficky náročnější, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 49,
        "text": "Co by v této situaci bylo nejméně vhodné vyvozovat, když výkon po úpravě podmínek stoupne jen v některých hodinách, které sdílejí stejný typ bariéry?",
        "options": [
          "Podpora pravděpodobně cílí na specifickou část problému, která se neprojevuje všude stejně.",
          "Je třeba rozlišit mezi přenosem podpory a jejím účinkem v přesně vymezených situacích.",
          "To, že se účinek ukázal jen tam, kde byla bariéra aktivní, samo o sobě neznamená neúčinnost.",
          "Jestliže se zlepšení neobjevilo všude, podpora mířila jistě mimo hlavní bariéru."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 50,
        "text": "Který závěr je při funkčním posouzení obtíže nejpřesvědčivější, když se po změně podmínek nezmění celkový dojem z žákova chování, ale změní se přesně ten výkon, na který byla podpora zaměřena?",
        "options": [
          "Že rozhodující je efekt v cílové oblasti, ne to, zda se současně proměnil i celkový dojem z chování.",
          "Že bez proměny celkového chování nelze podporu považovat za účinnou, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Že změna v cílové oblasti je vedlejší, pokud se současně nezmění i všechny ostatní projevy, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Že účinek podpory lze hodnotit až tehdy, když se zlepší klima celé třídy, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 25,
        "B": 10,
        "C": 6,
        "D": 9
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 2,
    "label": "Baterie 2",
    "title": "Psychologie, ontogeneze, komunikace a poradenské minimum",
    "subtitle": "Krátké diferenční položky na psychologické pojmy, autory, ontogenezi, aplikované oblasti psychologie a komunikaci v poradenském kontextu.",
    "purpose": "Ověřit, zda uchazeč přesně rozliší psychologické pojmy, přiřazení autorů k pojmům, vývojové souvislosti, aplikované psychologické oblasti a komunikační techniky relevantní pro poradenský a speciálněpedagogický rámec přijímacího testu.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "psychologické pojmy a jemné pojmové rozdíly",
      "obranné mechanismy a psychoanalytické minimum",
      "autoři, směry a modely",
      "ontogeneze a vývojové souvislosti",
      "aplikované psychologické oblasti",
      "komunikace a poradenské minimum"
    ],
    "breakdown": [
      "14× pojmové dvojice a čtveřice",
      "12× autoři × směry × modely",
      "8× ontogeneze a vývojové souvislosti",
      "8× aplikované psychologické oblasti",
      "8× komunikace a poradenské minimum"
    ],
    "tags": [
      "psychologie",
      "ontogeneze",
      "komunikace",
      "poradenské minimum",
      "UPOL"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Dítě nové situace opakovaně vykládá starým schématem, i když tím realitu částečně zkresluje. Který proces zde převažuje?",
        "options": [
          "akomodace",
          "asimilace",
          "internalizace",
          "introjekce"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 2,
        "text": "Jak se nazývá proces, při němž nové zkušenosti vedou k přestavbě dosavadního kognitivního schématu?",
        "options": [
          "asimilace",
          "identifikace",
          "akomodace",
          "socializace"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 3,
        "text": "Které vymezení nejlépe odlišuje introjekci od identifikace?",
        "options": [
          "introjekce znamená přisuzování vlastních obsahů druhým, identifikace jejich vytěsnění",
          "introjekce je přejímání obsahů do vnitřního světa, identifikace je ztotožnění se s osobou nebo jejími rysy",
          "introjekce je vždy vědomé napodobení, identifikace vždy nevědomé přání",
          "introjekce je růstová motivace, identifikace deficitní motivace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 4,
        "text": "Který obranný mechanismus je nejpřesněji popsán tak, že jedinec připisuje vlastní nepřijatelné impulzy nebo tendence druhému člověku?",
        "options": [
          "sublimace",
          "racionalizace",
          "projekce",
          "regrese"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 5,
        "text": "Který obranný mechanismus odpovídá situaci, kdy se nepřijatelný pudový tlak uplatní ve společensky ceněné činnosti?",
        "options": [
          "represe",
          "sublimace",
          "introjekce",
          "fixace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 6,
        "text": "Jak se nazývá mechanismus, kdy se člověk v zátěži vrací k méně zralému způsobu reagování?",
        "options": [
          "regrese",
          "racionalizace",
          "kompenzace",
          "identifikace"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 7,
        "text": "Kdy je přesnější mluvit o racionalizaci než o represe?",
        "options": [
          "když nepřijatelný obsah zůstává mimo vědomí bez náhradního vysvětlení",
          "když člověk nevědomě vytěsní bolestnou vzpomínku",
          "když si své jednání dodatečně vysvětluje logicky znějícími důvody, které zakrývají skutečný motiv",
          "když agresivní impulz obrátí do sportovní činnosti"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 8,
        "text": "Které tvrzení nejpřesněji odlišuje potlačení od represe?",
        "options": [
          "potlačení je nevědomé vytěsnění, represe vědomé odložení",
          "potlačení je vědomé sebeovládání, represe je nevědomé vytěsnění",
          "potlačení i represe jsou v psychoanalytickém smyslu totéž",
          "potlačení je sociální proces, represe vývojový proces"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 9,
        "text": "Které vymezení nejlépe odlišuje frustraci od deprivace?",
        "options": [
          "frustrace je dlouhodobé strádání v základních potřebách, deprivace krátké blokování cíle",
          "frustrace je konflikt hodnot, deprivace obranný mechanismus",
          "frustrace je zmaření nebo blokace cíle, deprivace je dlouhodobější nedostatek potřebného podnětového nebo vztahového nasycení",
          "frustrace je výlučně biologická, deprivace výlučně sociální"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 10,
        "text": "Který pojem nejlépe vystihuje dlouhodobý nedostatek emočního a vztahového nasycení s vývojovým dopadem?",
        "options": [
          "stres",
          "deprivace",
          "konflikt",
          "adaptace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 11,
        "text": "Které vymezení nejlépe vystihuje internalizaci?",
        "options": [
          "přijetí norem, pravidel nebo hodnot do vnitřní regulace jednání",
          "mechanické osvojení senzomotorické reakce",
          "dočasné převzetí role kvůli vnějšímu tlaku bez vnitřního přijetí",
          "připisování vlastních obsahů druhým"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 12,
        "text": "Kdy je přesnější mluvit o socializaci než o internalizaci?",
        "options": [
          "když jde o přijetí jediné normy do svědomí",
          "když jde o širší proces začleňování do sociálních vztahů, rolí a kulturních pravidel",
          "když jde o vytěsnění nevhodného impulsu",
          "když jde o změnu kognitivního schématu po nové zkušenosti"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 13,
        "text": "Které vymezení nejpřesněji vystihuje empatii v poradenském kontaktu?",
        "options": [
          "souhlas s klientovým výkladem situace",
          "emoční ztotožnění bez odstupu",
          "porozumění klientovu prožívání při zachování profesního odstupu a nezahlcení vlastním prožíváním",
          "rychlé pojmenování nejlepšího řešení za klienta"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 14,
        "text": "Která formulace odpovídá parafrázi?",
        "options": [
          "„Myslím, že váš problém ve skutečnosti souvisí s autoritou otce.“",
          "„Jestli tomu rozumím správně, nejvíc vás teď zatěžuje nejistota po změně školy.“",
          "„Takže postup bude následující: test, doporučení, kontrola.“",
          "„To, co popisujete, je typická maladaptivní reakce.“"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 15,
        "text": "Která složka Freudova modelu osobnosti je nejvíce spojena s tlakem internalizovaných norem a zákazů vůči jednání jedince?",
        "options": [
          "ego",
          "id",
          "superego",
          "self"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 16,
        "text": "Který autor je nejvíce spojován s pocity méněcennosti, životním stylem a kompenzací v rámci individuální psychologie?",
        "options": [
          "Carl Rogers",
          "Alfred Adler",
          "Jean Piaget",
          "Carl Gustav Jung"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 17,
        "text": "Se kterým autorem je nejvíce spojen pojem kolektivní nevědomí?",
        "options": [
          "Sigmund Freud",
          "Erik Erikson",
          "Carl Gustav Jung",
          "Abraham Maslow"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 18,
        "text": "Klientsky orientovaný přístup, bezpodmínečné přijetí a důraz na autenticitu jsou nejvíce spojovány s osobností:",
        "options": [
          "Carl Rogers",
          "Ivan Pavlov",
          "Alfred Adler",
          "Zdeněk Matějček"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 19,
        "text": "Hierarchie potřeb a pojem seberealizace jsou nejvíce spojovány s osobností:",
        "options": [
          "Erik Erikson",
          "Abraham Maslow",
          "Lev Vygotskij",
          "Carl Jung"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 20,
        "text": "Psychosociální vývoj a vývojové konflikty napříč životem jsou nejpřesněji spojovány s osobností:",
        "options": [
          "Sigmund Freud",
          "Erik Erikson",
          "Ivan Pavlov",
          "Jean Piaget"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 21,
        "text": "Se kterým autorem jsou nejvíce spojeny pojmy asimilace, akomodace a stadiální kognitivní vývoj?",
        "options": [
          "Lev Vygotskij",
          "Jean Piaget",
          "Carl Rogers",
          "Alfred Adler"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 22,
        "text": "Který autor je nejvíce spojován se sociokulturním pojetím vývoje a učení, včetně role jazyka a sociální interakce?",
        "options": [
          "Ivan Pavlov",
          "Abraham Maslow",
          "Lev Vygotskij",
          "Carl Jung"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 23,
        "text": "Podmíněný reflex a reflexologická linie učení jsou nejvíce spojovány s osobností:",
        "options": [
          "Ivan Pavlov",
          "Jean Piaget",
          "Carl Rogers",
          "Erik Erikson"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 24,
        "text": "Psychická deprivace dítěte a vývoj dítěte se zdravotním postižením jsou v českém prostředí nejvíce spojovány s osobností:",
        "options": [
          "Zdeněk Matějček",
          "Josef Slowík",
          "Milan Valenta",
          "Bohumír Popelář"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 25,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "Jung — podmíněné reflexy",
          "Adler — kolektivní nevědomí",
          "Vygotskij — sociokulturní vývoj",
          "Maslow — obranné mechanismy ega"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 26,
        "text": "Které tvrzení o vztahu Freud × Adler × Jung × Rogers vystihuje situaci nejlépe?",
        "options": [
          "Rogers je nejvíce spojován s kolektivním nevědomím, Adler s psychosociálními krizemi, a právě v tomto směru by se měla hledat hlavní příčina. Právě toto hledisko je zde rozhodující.",
          "Freud je nejvíce spojován s obrannými mechanismy, Adler s individuální psychologií, Jung s archetypy a Rogers s klientsky orientovaným přístupem",
          "Freud je nejvíce spojován se sociokulturním vývojem, Jung s podmíněnými reflexy, Adler se seberealizací a Rogers s psychickou deprivací, a právě tato rovina by měla být považována za rozhodující.",
          "Freud, Adler, Jung a Rogers zastupují v této otázce tentýž teoretický směr, a právě tento znak by měl nést hlavní interpretační váhu. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 27,
        "text": "Ontogenetická psychologie se nejpřesněji zabývá:",
        "options": [
          "jen psychickými poruchami dospělého věku",
          "jen dědičností inteligence",
          "změnami psychiky člověka v průběhu individuálního vývoje",
          "výhradně dějinami psychologických škol"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 28,
        "text": "Které vymezení nejlépe odlišuje zrání od učení?",
        "options": [
          "zrání je sociální vliv, učení biologický růst CNS",
          "zrání je vnitřně podmíněné dozrávání předpokladů, učení je změna vznikající zkušeností a činností",
          "zrání i učení jsou v psychologii totožné procesy",
          "zrání se týká jen motoriky, učení jen školních znalostí"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 29,
        "text": "Které vymezení nejlépe vystihuje vývojovou krizi?",
        "options": [
          "vždy jde o psychopatologický stav vyžadující léčbu",
          "jde o normativní nebo očekávatelně vznikající napětí spojené s vývojovým přechodem a novými nároky",
          "jde o totéž co deprivace",
          "jde výhradně o důsledek špatné výchovy"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 30,
        "text": "Který Eriksonův konflikt nejlépe odpovídá období, v němž dítě usiluje o samostatnost a současně naráží na riziko studu a pochybností?",
        "options": [
          "iniciativa × vina",
          "autonomie × stud a pochybnost",
          "snaživost × inferiorita",
          "intimita × izolace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 31,
        "text": "Ve kterém Piagetově stadiu dítě už používá symboly a řeč, ale jeho myšlení je stále výrazně vázáno na názornost a egocentrismus?",
        "options": [
          "senzomotorické",
          "formálně operační",
          "předoperační",
          "konkrétně operační"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 32,
        "text": "Kdy je přesnější uvažovat o hranici dané spíše zráním než nedostatkem učení?",
        "options": [
          "když dítě neumí po výkladu zopakovat jednu novou informaci",
          "když se i při opakovaném vedení nedaří výkon, pro nějž ještě nejsou zralé vývojové předpoklady",
          "když žák odmítá spolupracovat při hodině",
          "když si dítě neosvojilo terminologii konkrétního předmětu"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 33,
        "text": "Které tvrzení o ontogenezi vystihuje pojem nejlépe?",
        "options": [
          "ontogeneze znamená jen prenatální vývoj",
          "ontogeneze sleduje individuální vývoj jedince a vzniká v souhře zrání, učení a prostředí",
          "ontogeneze je v psychologii název pro vývoj celého druhu",
          "ontogeneze je v podstatě totéž co školní socializace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 34,
        "text": "Jak tento stav nejlépe označit, když dítě dlouhodobě vyrůstá bez stabilního citového vztahu a bez dostatečně podnětného kontaktu s pečující osobou?",
        "options": [
          "jde nejspíše o krátkodobou frustraci bez vývojového významu",
          "jde výhradně o konflikt mezi normami a impulzy",
          "jde o stav blízký psychické deprivaci s možným ontogenetickým dopadem",
          "jde hlavně o akomodaci kognitivního schématu"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 35,
        "text": "Které tvrzení nejpřesněji vystihuje vztah pojmů forenzní psychologie a soudní psychologie?",
        "options": [
          "forenzní psychologie je užší část pedagogické psychologie",
          "soudní psychologie je starší název pro vývojovou psychologii",
          "v běžném vymezení jde o blízké nebo překrývající se označení psychologické práce v právním a soudním kontextu",
          "jde o dvě zcela nesouvisející oblasti"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 36,
        "text": "Která oblast psychologie je nejvíce zaměřena na psychologické procesy učení, vyučování a školního prostředí?",
        "options": [
          "pedagogická psychologie",
          "forenzní psychologie",
          "klinická psychologie",
          "sociální psychologie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 37,
        "text": "Která oblast psychologie se uplatní nejspíše tehdy, když je hlavním cílem pomoci klientovi v orientaci, rozhodování, adaptaci a zvládání životní situace bez redukce jen na diagnózu?",
        "options": [
          "patologická psychologie",
          "poradenská psychologie",
          "experimentální psychologie",
          "fyziologická psychologie"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 38,
        "text": "Která oblast psychologie je nejvíce zaměřena na psychické poruchy, diagnostiku, klinický obraz a psychologickou práci v kontextu duševního onemocnění?",
        "options": [
          "klinická psychologie",
          "sociální psychologie",
          "pedagogická psychologie",
          "forenzní psychologie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 39,
        "text": "Do které oblasti psychologie patří nejspíše zkoumání vlivu skupiny, sociální percepce, postojů nebo konformity?",
        "options": [
          "poradenská psychologie",
          "sociální psychologie",
          "ontogenetická psychologie",
          "klinická psychologie"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 40,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "pedagogická psychologie — psychologické aspekty vyučování a učení",
          "klinická psychologie — skupinová konformita a postoje",
          "forenzní psychologie — výhradně školní adaptace dítěte",
          "sociální psychologie — diagnostika psychických poruch v léčebném kontextu"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 41,
        "text": "Která formulace odpovídá otevřené otázce v poradenském rozhovoru?",
        "options": [
          "„Takže za vaše potíže může hlavně změna školy, že?“",
          "„Kolik přesně vám bylo, když to začalo?“",
          "„Jak jste tehdy celé situaci rozuměl a co pro vás bylo nejtěžší?“",
          "„Souhlasíte, že bylo nejlepší odejít?“"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 42,
        "text": "Která odpověď nejlépe odpovídá reflektivnímu naslouchání?",
        "options": [
          "„Tohle jste měl řešit mnohem dřív.“",
          "„Jestli tomu rozumím správně, nejvíc vás vyčerpává, že se situace doma i ve škole táhne bez jasného řešení.“",
          "„Takže si teď sepíšeme tři administrativní kroky.“",
          "„Podle mého je hlavní problém v tom, že jste příliš citlivý.“"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 43,
        "text": "Které tvrzení nejlépe vystihuje rozdíl mezi rozhovorem a dialogem?",
        "options": [
          "dialog více zdůrazňuje vzájemnost a společné vytváření porozumění, zatímco rozhovor může mít i širší či formálnější podobu",
          "dialog je vždy terapeutický, rozhovor vždy neformální",
          "rozhovor je vždy direktivní, dialog vždy výslechový",
          "mezi rozhovorem a dialogem nelze v odborném kontextu vůbec rozlišovat"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Která odpověď nejspíše představuje interpretaci, nikoli parafrázi nebo čisté reflektivní shrnutí?",
        "options": [
          "„Slyším, že vás nejvíc zraňuje nejistota a dlouhé čekání.“",
          "„Jestli tomu rozumím správně, po přestupu jste začal ztrácet jistotu v tom, co dřív fungovalo.“",
          "„Mohlo by za tím být i to, že v autoritách znovu prožíváte starší zkušenost z rodiny.“",
          "„Takže hlavní jsou teď dvě oblasti: škola a vztahy doma.“"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 45,
        "text": "Která formulace nejlépe odpovídá empatické, ale současně profesně udržené reakci?",
        "options": [
          "„Úplně přesně vím, co prožíváte, mně se stalo totéž.“",
          "„To přejde, nesmíte si to tolik brát.“",
          "„Z toho, co říkáte, je cítit velké napětí a osamění; dává smysl, že je pro vás teď těžké hledat další krok.“",
          "„Hlavně se zkuste uklidnit a nevracet se k tomu.“"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 46,
        "text": "Který postup je v poradenské komunikaci nejméně blízký aktivnímu naslouchání?",
        "options": [
          "ověřování, zda poradce klientovi správně rozumí",
          "zachycení obsahové i emoční roviny sdělení",
          "průběžné direktivní přerušování kvůli rychlému posunu k závěru",
          "práce s parafrází a shrnutím"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 47,
        "text": "Která formulace nejvíce připomíná sugestivní otázku?",
        "options": [
          "„Jak jste té situaci tehdy rozuměl?“",
          "„Co se podle vás změnilo po přechodu do nové třídy?“",
          "„To jste tehdy musel cítit hlavně křivdu, že ano?“",
          "„Které momenty byly pro vás v té době nejzatěžující?“"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 48,
        "text": "Které vymezení nejlépe odlišuje shrnutí od parafráze?",
        "options": [
          "shrnutí vybírá a spojuje více důležitých bodů do celku, parafráze vrací kratší úsek sdělení vlastními slovy",
          "shrnutí je vždy interpretace, parafráze vždy rada",
          "shrnutí se používá jen v diagnostice, parafráze jen v terapii",
          "mezi shrnutím a parafrází není věcný rozdíl"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 49,
        "text": "Která formulace odpovídá shrnutí nejlépe?",
        "options": [
          "„Takže když to spojím, obtíž pro vás vznikla po změně školy, zesílila v rodině a teď nejvíc potřebujete znovu získat pocit orientace.“",
          "„To zní, jako byste byl vlastně jen přecitlivělý.“",
          "„Takže škola.“",
          "„Myslím, že za tím bude potlačený konflikt s autoritou.“"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 50,
        "text": "Jaká reakce poradce je nejpřiměřenější, když chce zachovat porozumění, ale nepřebírat klientovu zkušenost za svou?",
        "options": [
          "„Vaše pocity mohu plně převzít, protože je prožívám stejně.“",
          "„Nemusíme se zdržovat emocemi, pojďme rovnou k řešení.“",
          "„Mohu se pokusit porozumět tomu, co prožíváte, ale potřebuji zároveň držet odstup, aby naše práce zůstala užitečná.“",
          "„Nejdůležitější je, abyste přijal můj výklad situace.“"
        ],
        "correct": 2,
        "correctLetter": "C"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 11,
        "B": 20,
        "C": 19,
        "D": 0
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 3,
    "label": "Baterie 3",
    "title": "Historie speciální pedagogiky, terminologický vývoj a české osobnosti",
    "subtitle": "Krátké diferenční položky na přiřazení termínů k autorům, vývoj oborového jazyka, periodizaci a českou tradici speciální pedagogiky.",
    "purpose": "Ověřit, zda uchazeč přesně rozliší historické a terminologické souvislosti oboru, významné české a evropské osobnosti, vývojové etapy a proměny jazyka speciální pedagogiky bez sklouzávání k obecným nebo encyklopedickým poučkám.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "přiřazení termínů k autorům v dějinách oboru",
      "české osobnosti a jejich oblasti",
      "historické přístupy a periodizace",
      "proměna oborového jazyka",
      "pomocné školství a česká tradice",
      "přechod od deficitního k funkčnímu pojetí"
    ],
    "breakdown": [
      "18× termín × autor",
      "12× osobnost × přínos × oblast",
      "10× periodizace a historické přístupy",
      "5× česká oborová tradice a pomocné školství",
      "5× proměna jazyka a cíle oboru"
    ],
    "tags": [
      "historie speciální pedagogiky",
      "terminologický vývoj",
      "české osobnosti",
      "termín-autor",
      "UPOL"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Které spojení historického termínu s osobností je správné?",
        "options": [
          "pedagogická patologie — Adolf Strümpell",
          "pedagogická patologie — Jan Mauer",
          "pedagogická patologie — Bohumír Popelář",
          "pedagogická patologie — Miloš Sovák"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 2,
        "text": "Které přiřazení pojmu k osobnosti odpovídá historickému vývoji oboru?",
        "options": [
          "léčebná pedagogika — Josef Zeman",
          "léčebná pedagogika — Adolf Dannemann",
          "léčebná pedagogika — František Čáda",
          "léčebná pedagogika — Ludvík Edelsberger"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 3,
        "text": "Která dvojice termín–osobnost je doložena správně?",
        "options": [
          "pedopatologie — František Čáda",
          "pedopatologie — Jan Mauer",
          "pedopatologie — Miloš Sovák",
          "pedopatologie — Bohumír Popelář"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 4,
        "text": "S okruhem duševně úchylné děti je v české tradici nejvíce spojován:",
        "options": [
          "Bohumír Popelář",
          "František Čáda",
          "Josef Slowík",
          "Ludvík Edelsberger"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 5,
        "text": "Pojem nápravná pedagogika je v českém vývoji nejpřesněji navázán na osobnost:",
        "options": [
          "Jan Mauer",
          "František Štampach",
          "František Kábele",
          "Josef Zeman"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 6,
        "text": "Které spojení termínu a osobnosti je v historickém kontextu správné?",
        "options": [
          "sociální pedagogika — Ludvík Edelsberger",
          "sociální pedagogika — František Štampach",
          "sociální pedagogika — František Ludvík",
          "sociální pedagogika — Marie Vítková"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 7,
        "text": "Pojem zvláštní péče je v české tradici nejvíce spojován s osobností:",
        "options": [
          "František Ludvík",
          "František Kábele",
          "Adolf Dannemann",
          "Milan Valenta"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 8,
        "text": "Které přiřazení termínu a autora je správné?",
        "options": [
          "speciální pedagogika defektologická — Ludvík Edelsberger",
          "speciální pedagogika defektologická — Josef Slowík",
          "speciální pedagogika defektologická — Bohumír Popelář",
          "speciální pedagogika defektologická — Jan Mauer"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 9,
        "text": "S pojmem defektologie je v československém vývoji nejvíce spojován:",
        "options": [
          "Miloš Sovák",
          "Josef Zeman",
          "Bohumír Popelář",
          "František Čáda"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 10,
        "text": "S kterou osobností je v českém prostředí nejvíce spojován pojem speciální pedagogika?",
        "options": [
          "Bohumír Popelář",
          "Miloš Sovák",
          "Josef Zeman",
          "Ludvík Edelsberger"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 11,
        "text": "S pojmem integrativní speciální pedagogika je v českém prostředí nejvíce spojována:",
        "options": [
          "Libuše Ludíková",
          "Marie Vítková",
          "Jarmila Pipeková",
          "Josef Slowík"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 12,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "defektologie — Bohumír Popelář",
          "nápravná pedagogika — Jan Mauer",
          "léčebná pedagogika — František Kábele",
          "sociální pedagogika — Miloš Sovák"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 13,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "zvláštní péče — František Ludvík",
          "speciální pedagogika — Josef Zeman",
          "duševně úchylné děti — Ludvík Edelsberger",
          "pedopatologie — Bohumír Popelář"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 14,
        "text": "Která čtveřice obsahuje pouze správné historické vazby?",
        "options": [
          "Strümpell — pedagogická patologie; Mauer — nápravná pedagogika; Popelář — speciální pedagogika; Sovák — defektologie",
          "Strümpell — léčebná pedagogika; Mauer — defektologie; Popelář — nápravná pedagogika; Sovák — speciální pedagogika, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Strümpell — sociální pedagogika; Mauer — speciální pedagogika defektologická; Popelář — defektologie; Sovák — pedopatologie",
          "Strümpell — integrativní speciální pedagogika; Mauer — pedagogická patologie; Popelář — léčebná pedagogika; Sovák — sociální pedagogika"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 15,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "František Čáda — nápravná pedagogika",
          "Adolf Dannemann — léčebná pedagogika",
          "Bohumír Popelář — defektologie",
          "Josef Zeman — integrativní speciální pedagogika"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 16,
        "text": "Které tvrzení o historické terminologii vystihuje věc nejlépe?",
        "options": [
          "výraz defektologie je v české tradici přesněji spojen s Bohumírem Popelářem než s Milošem Sovákem",
          "výraz speciální pedagogika defektologická se v české tradici nepojí s Ludvíkem Edelsbergerem",
          "výraz speciální pedagogika se v české terminologii pojí s Bohumírem Popelářem",
          "výraz nápravná pedagogika je v českém vývoji bližší Adolfu Dannemannovi než Janu Mauerovi"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 17,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "sociální pedagogika — František Ludvík",
          "léčebná pedagogika — Adolf Dannemann",
          "duševně úchylné děti — Jan Mauer",
          "integrativní speciální pedagogika — Libuše Ludíková"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 18,
        "text": "Která vazba je nejméně přesná?",
        "options": [
          "pedagogická patologie — Adolf Strümpell",
          "speciální pedagogika — Bohumír Popelář",
          "defektologie — Miloš Sovák",
          "nápravná pedagogika — František Kábele"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 19,
        "text": "Která osobnost je nejvíce spojována s moderním formováním československé defektologie a logopedie?",
        "options": [
          "František Štampach",
          "Miloš Sovák",
          "František Ludvík",
          "Jan Mauer"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 20,
        "text": "Kdo je v československém kontextu po roce 1919 nejvýrazněji spojen s budováním moderní defektologie a pomocného školství?",
        "options": [
          "Josef Zeman",
          "Bohumír Popelář",
          "Ludvík Edelsberger",
          "Adolf Dannemann"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 21,
        "text": "Která osobnost je v novodobém českém prostředí nejvíce spojena s psychopedií?",
        "options": [
          "Milan Valenta",
          "Libuše Ludíková",
          "V. Lechta",
          "J. Jesenský"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 22,
        "text": "Která osobnost je v českém prostředí nejvíce spojena se speciální pedagogikou osob se zrakovým postižením?",
        "options": [
          "Marie Vítková",
          "Libuše Ludíková",
          "František Kábele",
          "Adolf Meyer"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 23,
        "text": "S kterou osobností je nejvíce spojen pojem narušená komunikační schopnost?",
        "options": [
          "Karel Neubauer",
          "Viktor Lechta",
          "Miloš Sovák",
          "Josef Slowík"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 24,
        "text": "Která osobnost je nejvíce spojena s logopedickými klasifikačními a paradigmatickými souvislostmi v českém a slovenském prostoru?",
        "options": [
          "Karel Neubauer",
          "Jan Mauer",
          "Bohumír Popelář",
          "J. Jesenský"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 25,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "J. Jesenský — školská integrace a její české souvislosti",
          "Josef Slowík — první české pomocné školství, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Libuše Ludíková — defektologie",
          "Miloš Sovák — integrativní speciální pedagogika, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 26,
        "text": "Které tvrzení o novodobých českých osobnostech odpovídá nejlépe?",
        "options": [
          "Marie Vítková je přesněji spojována se zrakovým postižením než Libuše Ludíková, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Josef Slowík je spojován spíše se širším a inkluzivním pojetím speciální pedagogiky než s historickým pojmem nápravná pedagogika",
          "Milan Valenta je typičtěji spojován se zrakovým postižením než s psychopedií, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Karel Neubauer je typičtěji spojován s pomocným školstvím než s logopedickým paradigmatem, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 27,
        "text": "Která osobnost je nejvíce spojena se širším a inkluzivním pojetím speciální pedagogiky?",
        "options": [
          "Josef Slowík",
          "František Čáda",
          "Adolf Strümpell",
          "František Ludvík"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 28,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "František Kábele — logopedie a somatopedie",
          "Bohumír Popelář — psychopedie",
          "Jan Mauer — integrativní speciální pedagogika",
          "Libuše Ludíková — defektologie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 29,
        "text": "Která osobnost je nejvíce spojena s integrativní speciální pedagogikou, nikoli primárně s pomocným školstvím nebo historickou defektologií?",
        "options": [
          "Josef Zeman",
          "Miloš Sovák",
          "Marie Vítková",
          "Adolf Dannemann"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 30,
        "text": "Které přiřazení osobnosti a oblasti je správné?",
        "options": [
          "Karel Neubauer — logopedické klasifikační rámce",
          "Josef Slowík — pedagogická patologie",
          "František Čáda — integrativní speciální pedagogika",
          "J. Jesenský — léčebná pedagogika"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 31,
        "text": "Které vymezení nejlépe odpovídá represivnímu pojetí péče o osoby s postižením?",
        "options": [
          "důraz na společné vzdělávání, participaci a podporu v běžném prostředí",
          "důraz na vyloučení, odmítání nebo tvrdé omezování jedince kvůli odlišnosti",
          "důraz na školní integraci při zachování oddělených služeb podpory",
          "důraz na reedukaci a kompenzaci v moderním speciálněpedagogickém rámci"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 32,
        "text": "Které vymezení nejlépe odpovídá charitativnímu pojetí?",
        "options": [
          "převaha právního nároku na podporu a participaci v běžném prostředí",
          "důraz na humanitní pomoc a opatrování bez silného akcentu na vzdělávací emancipaci",
          "důraz na standardní školní inkluzi se systémovou podporou",
          "důraz na speciálněpedagogické poradenství a individualizaci výkonu"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 33,
        "text": "Který znak ukazuje spíše na institucionální pojetí než na pojetí integrační nebo inkluzivní?",
        "options": [
          "snaha zajistit podporu v běžném prostředí při zachování účasti",
          "přesun pomoci do specializovaných ústavů a oddělených zařízení",
          "důraz na dostupnost běžné školy a úpravu podmínek",
          "důraz na překonávání bariér v prostředí místo izolace jedince"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 34,
        "text": "Které tvrzení nejlépe vystihuje integrační pojetí?",
        "options": [
          "jedinec je zcela vyloučen ze vzdělávacího a sociálního prostoru",
          "hlavním cílem je především opatrování bez nároku na vzdělávání",
          "jedinec vstupuje do běžného prostředí, ale podpora je ještě často chápána jako přidávaná k existujícímu systému",
          "běžné prostředí je považováno za nevhodné a nahrazuje se oddělenou institucionální sítí"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 35,
        "text": "Které tvrzení nejlépe vystihuje inkluzivní pojetí?",
        "options": [
          "odlišnost je důvodem pro oddělení do samostatné vzdělávací větve",
          "hlavním řešením je charitativní ochrana mimo běžné prostředí",
          "systém se má přizpůsobovat tak, aby běžné prostředí bylo dostupné i pro odlišné potřeby",
          "cílem je ponechat podporu jen ve specializovaných ústavech"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 36,
        "text": "Který posun nejlépe odpovídá přechodu od deficitního k funkčnímu a podpůrnému pojetí oboru?",
        "options": [
          "od zkoumání bariér v prostředí k silnějšímu soustředění na „vadu“ jako označení člověka",
          "od označování osoby primárně podle nedostatku k většímu důrazu na potřeby, podporu a fungování v kontextu",
          "od školní práce k rezignaci na vzdělávací možnosti jedince",
          "od pedagogiky k výlučně medicínskému řízení situace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 37,
        "text": "Které tvrzení nejlépe vystihuje participační pojetí?",
        "options": [
          "akcentuje hlavně oddělenou péči ve specializovaném systému",
          "akcentuje především pasivní ochranu jedince bez důrazu na jeho roli",
          "akcentuje účast, zapojení a odstraňování bariér, které brání plnému začlenění",
          "akcentuje výhradně terminologické přejmenování oboru bez změny cíle"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 38,
        "text": "Které tvrzení o historické proměně oboru je nepřesné?",
        "options": [
          "historický vývoj zahrnuje i terminologické proměny a proměny cíle oboru",
          "starší názvosloví bývá často deficitněji orientované než novější podpůrné a participační pojetí",
          "integrační a inkluzivní pojetí jsou historicky zcela totožné a nelze je nijak rozlišit",
          "vývoj oboru nelze vykládat jen přes jednotlivé osobnosti, ale i přes proměnu zařízení, jazyka a cílů"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 39,
        "text": "Který popis je bližší inkluzivnímu než čistě integračnímu rámci?",
        "options": [
          "jedinec je připojován k již hotovému systému bez výraznější změny jeho pravidel",
          "systém je upravován tak, aby bariéry nebránily účasti různorodých žáků",
          "podpora je vyhrazena především odděleným institucím",
          "hlavní roli má opatrování mimo běžné vzdělávací prostředí"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 40,
        "text": "Která formulace nejpřesněji vystihuje proměnu cíle oboru?",
        "options": [
          "od podpory a participace zpět k převažující izolaci a ochraně",
          "od důrazu na možnosti a prostředí k dominantnímu zaměření na nálepku defektu",
          "od označení a segregace směrem k podpoře fungování, vzdělávání a účasti",
          "od pedagogického pojetí k úplnému opuštění vzdělávací roviny"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 41,
        "text": "Která osobnost je nejvíce spojena s budováním sítě pomocného školství v československém prostředí první republiky?",
        "options": [
          "Josef Zeman",
          "Marie Vítková",
          "Karel Neubauer",
          "Libuše Ludíková"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 42,
        "text": "Které tvrzení o pomocném školství odpovídá nejlépe historickému kontextu české tradice?",
        "options": [
          "šlo o výhradně vysokoškolskou větev podpory studentů se specifickými potřebami",
          "šlo o historicky budovanou síť speciálního školství navázanou na dobové chápání péče o děti se speciálními vzdělávacími potřebami",
          "šlo o souhrn sociálních služeb mimo jakoukoli vazbu na školství",
          "šlo výhradně o pozdní produkt inkluzivní reformy po roce 2000"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 43,
        "text": "Které spojení odpovídá historickému a oborovému kontextu nejlépe?",
        "options": [
          "Josef Zeman — ministerská a systémová role při rozvoji pomocného školství a defektologie",
          "Jan Mauer — významná osobnost zrakového postižení na UPOL, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Marie Vítková — historická léčebná pedagogika, a právě v tomto směru by se měla hledat hlavní příčina.",
          "František Čáda — strukturované učení TEACCH, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Který popis je historicky bližší české tradici pomocného školství než novodobému inkluzivnímu rámci?",
        "options": [
          "jednotný důraz na participaci v běžném systému bez samostatné sítě zařízení",
          "budování specifické školské větve pro určité skupiny žáků v rámci tehdejšího chápání speciální péče",
          "univerzitní systém podpory studentů se specifickými potřebami",
          "důraz na individuální plán a podpůrná opatření v současném právním rámci"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 45,
        "text": "Které tvrzení nejlépe vystihuje českou oborovou tradici první republiky?",
        "options": [
          "pomocné školství a defektologie nelze spojovat s Josefem Zemanem",
          "česká oborová tradice první republiky nesouvisela s rozvojem speciálního školství",
          "Josef Zeman je považován za klíčovou osobnost moderní československé defektologie a pomocného školství",
          "první republika je pro dějiny oboru důležitá pouze kvůli nástupu pojmu integrativní speciální pedagogika"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 46,
        "text": "Která formulace je z hlediska novějšího jazyka oboru nejméně vhodná?",
        "options": [
          "důraz na podporu potřeb a funkční fungování jedince",
          "důraz na přístupnost a účast v prostředí",
          "důraz na spoluúčast systému na odstraňování bariér",
          "důraz na označení člověka především přes jeho defekt nebo úchylku"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 47,
        "text": "Které tvrzení nejlépe vystihuje proměnu oborového jazyka?",
        "options": [
          "novější pojetí oboru se obvykle posouvá od nálepkování člověka k jazyku podpory, potřeb a participace",
          "novější pojetí oboru se obvykle vrací k silně deficitnímu a stigmatizujícímu označování",
          "proměna jazyka nemá žádnou souvislost s proměnou cíle oboru",
          "historické termíny a novější jazyk oboru jsou z hlediska cíle i významu plně zaměnitelné"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 48,
        "text": "Která dvojice nejlépe vystihuje rozdíl mezi starším a novějším pojetím oboru?",
        "options": [
          "starší pojetí — participace; novější pojetí — represivní vyloučení",
          "starší pojetí — deficitní označování; novější pojetí — podpůrné a funkční chápání",
          "starší pojetí — inkluze; novější pojetí — charitativní opatrování",
          "starší pojetí — přístupnost prostředí; novější pojetí — izolace v ústavu"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 49,
        "text": "Které tvrzení o vztahu jazyka a cíle oboru je nepřesné?",
        "options": [
          "změna jazyka obvykle souvisí se změnou toho, co obor považuje za svůj cíl",
          "deficitně orientovaný jazyk může odrážet starší pojetí jedince jako objektu péče",
          "podpůrný jazyk bývá spojen s důrazem na účast a dostupnost prostředí",
          "jazyk oboru je vůči cílům oboru neutrální a nic důležitého nevypovídá"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 50,
        "text": "Která formulace nejlépe uzavírá proměnu cíle speciální pedagogiky v novějším pojetí?",
        "options": [
          "hlavním cílem je především oddělit jedince od běžného prostředí",
          "hlavním cílem je především pečovat o jedince bez důrazu na jeho vzdělávací a sociální účast",
          "hlavním cílem je podpora vzdělávání, fungování a participace při respektu k potřebám a bariérám prostředí",
          "hlavním cílem je vrátit se k historickým deficitním označením, protože jsou přesnější"
        ],
        "correct": 2,
        "correctLetter": "C"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 19,
        "B": 19,
        "C": 9,
        "D": 3
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 4,
    "label": "Baterie 4",
    "title": "Disciplíny speciální pedagogiky, vnitřní termíny a klasifikační rozlišení",
    "subtitle": "Krátké diferenční položky na vnitřní terminologii, klasifikační rozlišení, diagnostické znaky a významné osobnosti uvnitř speciálněpedagogických disciplín.",
    "purpose": "Ověřit, zda uchazeč přesně rozliší vnitřní pojmy, klasifikační znaky, diagnostické okruhy, hraniční diferenciace a významné osobnosti uvnitř logopedie, surdopedie, tyflopedie, psychopedie, etopedie a příbuzných disciplín.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "logopedické vnitřní termíny a diagnostické znaky",
      "surdopedické a sluchové klasifikační rozlišení",
      "tyflopedické a oftalmopedické rozlišení",
      "psychopedické a etopedické diferenciace",
      "PAS a sociálně-pragmatická komunikace",
      "kongenitální a získané vady",
      "prelingvální a postlingvální rozlišení",
      "významné osobnosti a metody uvnitř jednotlivých pedií"
    ],
    "breakdown": [
      "10× logopedie × surdopedie × interní termíny",
      "8× tyflopedie / oftalmopedie × somatopedie",
      "8× psychopedie × etopedie",
      "6× PAS a sociálně-pragmatická komunikace",
      "8× klasifikační a terminologické rozlišení",
      "10× osobnosti a metody uvnitř jednotlivých pedií"
    ],
    "tags": [
      "disciplíny speciální pedagogiky",
      "logopedie",
      "surdopedie",
      "tyflopedie",
      "psychopedie",
      "etopedie",
      "UPOL"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Dítě zachytí, že někdo mluví, ale opakovaně zaměňuje hláskově blízké dvojice a nerozhodne mezi nimi ani v klidné individuální situaci. Které vyšetření je zde nejdůležitější?",
        "options": [
          "orientační vyšetření sluchu",
          "vyšetření fonematického sluchu",
          "vyšetření laterality",
          "vyšetření prozodických faktorů řeči"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 2,
        "text": "Který nález patří spíše do hodnocení prozodických faktorů řeči než do hodnocení artikulace nebo jazykových rovin?",
        "options": [
          "nepřiměřená melodie a přízvuk mluvy",
          "stabilní distorze sykavek",
          "nápadně omezená slovní zásoba",
          "zaměňování hláskově blízkých slov"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 3,
        "text": "Které přiřazení termínu a hlavní obtíže je správné?",
        "options": [
          "balbuties — porucha plynulosti s bloky, napětím a narušeným tokem řeči",
          "tumultus sermonis — úplná ztráta hlasu při zachovaném porozumění",
          "dysfonie — porucha rezonance s výraznou nosovostí",
          "rinolalie — výpadek fonematického rozlišování při zachované fonaci"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 4,
        "text": "Které srovnání nejpřesněji odlišuje balbuties od tumultus sermonis?",
        "options": [
          "Balbuties bývá spojena s bloky a napětím, zatímco tumultus sermonis spíše s překotným a málo kontrolovaným tempem řeči.",
          "Balbuties je porucha hlasu, zatímco tumultus sermonis je porucha rezonance, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Balbuties zasahuje jen psaný projev, zatímco tumultus sermonis jen mluvené porozumění, a právě tato rovina by měla být považována za rozhodující.",
          "Balbuties je výlučně symptomatická porucha řeči, zatímco tumultus sermonis výlučně sluchová porucha, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 5,
        "text": "Který klinický rámec nejspíše ukazuje na symptomaticky narušenou komunikační schopnost spíše než na izolovanou vývojovou poruchu řeči?",
        "options": [
          "narušená komunikace v kontextu DMO a širšího neurologického zatížení",
          "izolovaná sigmatická výslovnost při jinak typickém vývoji",
          "přechodná nejistota při školním vystupování bez dalších znaků",
          "samotná pravopisná chybovost při zachované mluvené řeči"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 6,
        "text": "Která pomůcka nejpřesněji míří na situaci, kdy je potřeba přenést hlas učitele přímo do sluchadla nebo přijímače žáka a omezit vliv třídy jako akustického prostředí?",
        "options": [
          "indukční smyčka",
          "FM systém",
          "Pichtův psací stroj",
          "televizní lupa"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 7,
        "text": "Žák při frontálním výkladu výrazně profituje z přímé zrakové opory obličeje mluvčího a při odvrácení řečníka ztrácí obsah, přestože jeho vlastní řečová produkce nepůsobí nápadně narušeně. Které vysvětlení by zde bylo nejméně přesné?",
        "options": [
          "Těžiště může ležet v dostupnosti auditivně-mluveného sdělení, ne v produkční logopedické obtíži.",
          "Důležitým vodítkem je závislost porozumění na vizuálním kontaktu s mluvčím.",
          "Situace může vyžadovat uvažování o surdopedické a komunikačně přístupové opoře.",
          "Hlavní podstata obtíže nejspíše leží v artikulační nepřesnosti samotného žáka."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 8,
        "text": "Které shrnutí nejpřesněji odpovídá postlingvální sluchové ztrátě?",
        "options": [
          "ke sluchové ztrátě došlo až po rozvoji řeči",
          "ke sluchové ztrátě došlo ještě před přirozeným rozvojem řeči",
          "sluchová ztráta je vždy vrozená a vždy hluboká",
          "sluchová ztráta se projevuje hlavně poruchou artikulace bez dopadu na porozumění"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 9,
        "text": "Které přiřazení diagnostické oblasti a její hlavní obtíže je správné?",
        "options": [
          "impresivní složka řeči — porozumění řeči",
          "expresivní složka řeči — orientační vyšetření sluchového prahu",
          "lateralita — kvalita hlasové fonace",
          "fonace — gramaticko-syntaktická stavba výpovědi"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 10,
        "text": "Dítě má dlouhodobě chraptivý, unavitelný hlas, ale artikulace i obsah řeči zůstávají relativně zachované. Jak situaci vyložit nejpřesněji?",
        "options": [
          "Těžiště leží spíše v poruše hlasu než v poruše rezonance nebo sluchové dostupnosti.",
          "Těžiště leží spíše v prelingvální sluchové ztrátě než v poruše hlasu, a právě tato rovina by měla být považována za rozhodující.",
          "Těžiště leží spíše v expresivně jazykové rovině než ve fonaci, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Těžiště leží spíše v symptomatické NKS než v hlasové oblasti, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 11,
        "text": "Která pomůcka nejméně stojí na funkčním využití zbytkového zraku a spíše přechází k hmatové cestě práce s textem?",
        "options": [
          "ruční lupa",
          "televizní lupa",
          "Braillovo písmo",
          "individuálně nastavené přisvětlení"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 12,
        "text": "Která úprava prostředí je zde nejvhodnější u světloplachosti?",
        "options": [
          "zvýšit lokální přisvětlení bez další úpravy",
          "pracovat se zastíněním a omezením oslnění",
          "zvýšit hlasitost mluvené instrukce",
          "přejít automaticky na hmatové písmo bez ohledu na funkční profil"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 13,
        "text": "Který předmět speciálněpedagogické péče je nejvíce spojen s tyflopedickou podporou zrakového postižení?",
        "options": [
          "prostorová orientace a samostatný pohyb",
          "nácvik odezírání a sluchové diferenciace",
          "bezprostřední postkrizová reflexe incidentu",
          "nácvik artikulačních pozic mluvních orgánů"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 14,
        "text": "Který prostředek je nejpřesněji spojen se zápisem v Braillově písmu, ne s optickým zvětšováním ani se sluchovou dostupností?",
        "options": [
          "FM systém",
          "televizní lupa",
          "Pichtův psací stroj",
          "indukční smyčka"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 15,
        "text": "Které srovnání nejpřesněji vystihuje rozdíl mezi Haüyem a Braillem?",
        "options": [
          "Haüy je spojen s novodobým školským vzděláváním nevidomých, Braille s písmem pro nevidomé.",
          "Haüy je spojen s oralismem neslyšících, Braille s krizovým rozhovorem, a právě tato rovina by měla být považována za rozhodující.",
          "Haüy je spojen s logopedickou diagnostikou, Braille s indukční smyčkou, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Haüy je spojen s psychopedií, Braille se sociálně-pragmatickou komunikací, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 16,
        "text": "Žák po zvětšení, kontrastu a přehlednějším grafickém uspořádání drží stejný obsah výrazně lépe, zatímco změna sedu a úchopu výkon téměř neposune. Jaké vysvětlení je zde nejpřesnější?",
        "options": [
          "Obtíž souvisí spíše s vizuální přístupnosti než v tělesné a motorické formě výkonu.",
          "Obtíž souvisí spíše s motorickém provedení než v přístupu k vizuální informaci, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Obtíž souvisí spíše s poruše hlasu než ve vizuální přístupnosti, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Obtíž souvisí spíše s postkrizové regulaci chování než ve vizuálním přístupu, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 17,
        "text": "Co by zde bylo nejméně přesné tvrdit, pokud dítě dobře vizuálně orientuje text i prostor, ale dlouhodobě potřebuje úpravu polohy, úchopu a manipulace s pomůckami?",
        "options": [
          "Těžiště nemusí ležet v tyflopedické dostupnosti informace, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Do popředí se může dostávat somatopedický rámec formy výkonu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Hlavní problém nemusí být v zrakové přístupnosti textu, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Nejpravděpodobnější obtíž se zde týká právě vizuální orientaci a kontrastu."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 18,
        "text": "Která pomůcka je optickou pomůckou, nikoli hmatovým nebo hlasovým technickým řešením?",
        "options": [
          "hmatový displej",
          "optická lupa",
          "Pichtův psací stroj",
          "počítač s hlasovým výstupem"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 19,
        "text": "Které tvrzení nejpřesněji vystihuje posun od MKN-10 k MKN-11 v oblasti mentálního postižení / poruchy intelektového vývoje?",
        "options": [
          "Novější přístup více zdůrazňuje adaptivní fungování a neopírá se jen o starší terminologii mentální retardace.",
          "Novější přístup ruší potřebu rozlišovat závažnost a ponechává jen globální popis bez funkčních dopadů, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Novější přístup nahrazuje edukativní hledisko čistě psychiatrickým bez vazby na podporu, a právě tato rovina by měla být považována za rozhodující.",
          "Novější přístup chápe intelektový vývoj výhradně jako otázku školního prospěchu, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 20,
        "text": "Která trojice nejpřesněji odpovídá oblastem adaptivního fungování?",
        "options": [
          "konceptuální, sociální, praktická",
          "percepční, sluchová, motorická",
          "fonologická, lexikální, pragmatická",
          "somatická, kognitivní, behaviorální"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 21,
        "text": "Který nález ukazuje spíše na oslabení adaptivního fungování než jen na izolovanou školní neúspěšnost?",
        "options": [
          "přetrvávající obtíže v praktické orientaci, sociálních pravidlech a každodenní samostatnosti",
          "jednorázový slabší výkon v jedné písemné práci",
          "izolovaná artikulační nepřesnost bez dalších dopadů",
          "nestabilní tempo řeči při zachovaném praktickém fungování"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 22,
        "text": "Který pedagogický princip je pro psychopedické vedení nejméně přesný?",
        "options": [
          "vysoká míra názornosti a konkrétnosti",
          "přiměřené tempo a opakování",
          "funkční vazba učiva k běžnému životu",
          "dominantně abstraktní výklad bez opory v konkrétní zkušenosti"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 23,
        "text": "Dítě potřebuje výraznou názornost, opakování, konkrétní návaznost na běžný život a realistické vzdělávací cíle. Jaké vysvětlení situace je nejpřesnější?",
        "options": [
          "Vysvětlení je bližší psychopedickému rámci než rychlému etopedickému uzavření.",
          "Vysvětlení je bližší surdopedickému rámci než psychopedickému, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Vysvětlení je bližší tyflopedickému rámci než psychopedickému, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Vysvětlení je bližší hlasové poruše než psychopedickému rámci, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 24,
        "text": "Na co je při rozlišení psychopedického a etopedického vysvětlení situace nejdůležitější se zeptat, když dítě nápadně odmítá úkol?",
        "options": [
          "Co se stane s výkonem po konkrétním modelování, názoru a snížení abstrakční zátěže?",
          "Jak rychle bude možné sepsat kázeňský záznam?",
          "Která sankce bude pro třídu nejsrozumitelnější?",
          "Který spolužák dítě nejvíce provokuje bez vazby na úkol?"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 25,
        "text": "Které shrnutí nejpřesněji vystihuje etopedický postkrizový rámec práce?",
        "options": [
          "zkoumat bezprostřední situaci, dětské pochopení události, dopad na druhé a možnosti opravy",
          "převést každou obtíž přímo na stupeň intelektového vývoje",
          "uzavřít každou konfliktovou situaci jako důkaz zrakové bariéry",
          "soustředit se výhradně na artikulační přesnost bez práce s událostí"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 26,
        "text": "Chování se vyhrocuje hlavně po vztahovém střetu, po porušení pravidel a v situacích sociálního konfliktu, nikoli po snížení abstrakce nebo zvýšení názornosti. Jaké vysvětlení je zde nejpřesnější?",
        "options": [
          "Do popředí se dostává spíše etopedický rámec než psychopedická potřeba názornosti.",
          "Do popředí se dostává spíše tyflopedický rámec než etopedický, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Do popředí se dostává spíše surdopedický rámec než etopedický, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Do popředí se dostává spíše porucha hlasu než etopedický rámec, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 27,
        "text": "Dítě rozumí slovům doslovně, ale opakovaně mine komunikační záměr nepřímé prosby nebo ironie. Jaké vysvětlení situace je nejpřesnější?",
        "options": [
          "Těžiště leží spíše v sociálně-pragmatickém porozumění než v prosté slovní zásobě.",
          "Těžiště leží spíše ve zrakové ostrosti než v pragmatickém porozumění, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Těžiště leží spíše v poruše hlasu než v sociálně-pragmatickém porozumění, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Těžiště leží spíše v grafomotorice než v sociálně-pragmatickém porozumění, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 28,
        "text": "Který znak je nejméně přesně vysvětlitelný čistě sluchovou nedostupností sdělení?",
        "options": [
          "stabilní selhávání ve sdílené pozornosti, reciproční výměně a pochopení komunikačního záměru i v klidném kontaktu",
          "ztráta části obsahu při větší vzdálenosti mluvčího a bez vizuální opory",
          "výrazná závislost porozumění na přímém pohledu na mluvčího",
          "rozpad obsahu při akusticky náročném frontálním výkladu"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 29,
        "text": "Který typ podpory nejpřesněji míří na obtíž v recipročním vedení rozhovoru a v odhadování komunikačního záměru partnera?",
        "options": [
          "explicitní nácvik pravidel reciprocity, střídání rolí a významu sociálních signálů",
          "přímé zesílení hlasu učitele do sluchadla",
          "zvětšení textu a zvýšení kontrastu pracovního listu",
          "nácvik hlasové hygieny a fonace"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 30,
        "text": "Který projev lépe odpovídá sociálně-pragmatickému komunikačnímu profilu než izolované logopedické produkční obtíži?",
        "options": [
          "formálně plynulá řeč bez recipročního přizpůsobení partnerovi a bez práce s perspektivou druhého",
          "stabilní sigmatická odchylka při zachovaném dialogickém ladění",
          "chraptivý hlas při zachovaném pochopení komunikační situace",
          "nosovost řeči při jinak přiměřené reciprocitě"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 31,
        "text": "Co by zde bylo nejméně přesné vyvozovat, když dítě rozumí doslovnému obsahu, ale opakovaně nezachytí, co druhý „myslel mezi řádky“?",
        "options": [
          "Může jít o oslabené pragmatické zpracování komunikačního záměru, a právě tato rovina by měla být považována za rozhodující.",
          "Může jít o bariéru v perspektivním a sociálně-kontextovém čtení sdělení.",
          "Může jít o obtíž, která přesahuje pouhou slovní zásobu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Nejspíše jde primárně o artikulační odchylku jednotlivých hlásek."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 32,
        "text": "Který projev nejspíše přetrvá i po zjednodušení obsahové náročnosti instrukce, pokud hlavní obtíž spočívá v sociálně-pragmatickém fungování?",
        "options": [
          "obtíž ve změně perspektivy partnera a v recipročním navazování",
          "obtíž ve čtení drobného písma na dálku",
          "obtíž v držení tužky a v posturálním nastavení",
          "obtíž v hlasové intenzitě při fonaci"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 33,
        "text": "Ke ztrátě sluchu došlo až poté, co se řeč přirozeně rozvinula. Jak se tato situace označuje nejpřesněji?",
        "options": [
          "postlingvální sluchová ztráta",
          "prelingvální sluchová ztráta",
          "kongenitální sluchová ztráta bez dalšího",
          "symptomatická porucha řeči"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 34,
        "text": "Které přiřazení termínu a hlavní obtíže je správné?",
        "options": [
          "dysfonie — patologická změna kvality hlasu",
          "afonie — překotné tempo řeči bez bloků",
          "fonastenie — porucha rezonance s nosovostí",
          "rinolalie — úplná ztráta hlasu"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 35,
        "text": "Která dvojice patří nejpřesněji do okruhu narušení zvuku řeči, nikoli primárně do poruch plynulosti nebo hlasu?",
        "options": [
          "rinolalie a palatolalie",
          "balbuties a tumultus sermonis",
          "dysfonie a afonie",
          "fonastenie a afonie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 36,
        "text": "Která trojice nejpřesněji odpovídá základním kategoriím poruch hlasu v logopedicko-foniatrickém rámci, jak jsou běžně vymezovány?",
        "options": [
          "dysfonie, afonie, fonastenie",
          "balbuties, rinolalie, dyslalie",
          "palatolalie, afázie, dysgrafie",
          "koktavost, mutismus, echolalie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 37,
        "text": "Jak toto shrnout nejpřesněji, pokud vada vznikla prenatálně, i když se její funkční dopad plně projeví až později?",
        "options": [
          "Stále může jít o vadu kongenitální.",
          "Jde automaticky o vadu získanou, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde automaticky o vadu postlingvální.",
          "Jde automaticky o symptomatickou poruchu řeči."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 38,
        "text": "Které vymezení nejlépe odpovídá symptomatické narušené komunikační schopnosti?",
        "options": [
          "komunikační obtíž je součástí širšího primárního postižení nebo onemocnění",
          "komunikační obtíž je vždy izolovaná a bez vazby na další stav",
          "komunikační obtíž je vždy jen otázkou nevhodného školního prostředí",
          "komunikační obtíž je totéž co každá pravopisná chyba"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 39,
        "text": "Které tvrzení o klasifikaci závažnosti poruchy intelektového vývoje vystihuje věc nejlépe?",
        "options": [
          "Klasifikace pomáhá orientačně, ale sama nevystihuje plně individuální vzdělávací potřeby.",
          "Klasifikace sama přesně určí celý vzdělávací plán bez potřeby dalšího funkčního čtení, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Klasifikace je v edukaci bezcenná a nemá žádný orientační význam, a právě tato rovina by měla být považována za rozhodující.",
          "Klasifikace se týká jen medicíny a v psychopedii se neuvažuje, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 40,
        "text": "Co by v psychopedickém a klasifikačním vysvětlení situace bylo nejméně přesné?",
        "options": [
          "Podpůrné a vzdělávací důsledky se nesmějí odvozovat jen mechanicky od diagnostické nálepky.",
          "Důležitou roli hraje i adaptivní fungování a reálný funkční profil, a právě tato rovina by měla být považována za rozhodující.",
          "Rozhodující může být vztah mezi stupněm obtíže a konkrétní podporou v běžném životě, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Pro vzdělávací závěr plně stačí samotný diagnostický štítek bez dalšího funkčního čtení."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 41,
        "text": "Jak nejlépe vystihnout rozdíl mezi Haüyem a Braillem?",
        "options": [
          "Haüy je spojen s první školou pro nevidomé, Braille s písmem pro nevidomé.",
          "Haüy je spojen s písmem pro nevidomé, Braille s první školou pro neslyšící.",
          "Haüy je spojen s FM systémem, Braille s logopedickou diagnostikou, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Haüy je spojen s bazální stimulací, Braille s krizovým rozhovorem, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 42,
        "text": "Jak nejpřesněji vymezit osobnost Pedra Ponce de León?",
        "options": [
          "významná historická osobnost vzdělávání neslyšících",
          "autor písma pro nevidomé",
          "významná česká osobnost psychopedie",
          "autor logoterapie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 43,
        "text": "Který odborník je nejbezpečněji spojován se stuttering modification v rámci práce s balbuties?",
        "options": [
          "Charles Van Riper",
          "Andreas D. Fröhlich",
          "Adolf Meyer",
          "Milan Valenta"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Které vymezení nejpřesněji odpovídá osobnosti Adolfa Meyera?",
        "options": [
          "psychobiologický přístup s důrazem na životní historii a prostředí",
          "novodobé vzdělávání nevidomých a první škola pro nevidomé",
          "logoterapie zaměřená na smysl a hodnoty",
          "bazální stimulace a bazální dialog"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 45,
        "text": "Jak nejlépe vystihnout vztah Lechty a Neubauera?",
        "options": [
          "Lechta je více spojen s pojmem narušená komunikační schopnost, Neubauer s logopedickou diagnostikou a klasifikačními rámci.",
          "Lechta je více spojen s tyflopedií, Neubauer se surdopedií, a právě tato rovina by měla být považována za rozhodující. Právě odtud by se proto mělo dále vycházet.",
          "Lechta je více spojen s defektologií, Neubauer s etopedií, a právě tento znak by měl nést hlavní interpretační váhu. Právě tento závěr by pak měl být považován za určující.",
          "Lechta je více spojen s bazální stimulací, Neubauer s logoterapií, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 46,
        "text": "Která osobnost je v českém prostředí nejvíce spojena s psychopedií jako teoreticky a oborově významná osobnost?",
        "options": [
          "Milan Valenta",
          "Libuše Ludíková",
          "Pedro Ponce de León",
          "Valentin Haüy"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 47,
        "text": "Která osobnost je nejpřesněji spojena se speciální pedagogikou osob se zrakovým postižením v prostředí UPOL?",
        "options": [
          "Libuše Ludíková",
          "Milan Valenta",
          "Viktor Lechta",
          "Fritz Redl"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 48,
        "text": "Které vymezení nejlépe odpovídá Františku Kábelemu v české speciální pedagogice?",
        "options": [
          "vazba na logopedii a somatopedii",
          "vazba na historii vzdělávání nevidomých ve Francii",
          "vazba na logoterapii a existenciální terapii",
          "vazba na psychobiologický model psychiatrie"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 49,
        "text": "Které vymezení nejpřesněji vystihuje Life Space Interview ve vazbě k Fritzovi Redlovi?",
        "options": [
          "bezprostřední práce s problematickou situací dítěte jako terapeutickým materiálem",
          "systém optických pomůcek pro slabozraké",
          "metoda náhradní komunikace s vizuálními symboly",
          "klasifikace poruch hlasu podle míry fonace"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 50,
        "text": "Které přiřazení osoby a jejího nejtypičtějšího oborového spojení je správné?",
        "options": [
          "Adolf Meyer — psychobiologický přístup s důrazem na životní historii a prostředí",
          "Charles Van Riper — první škola pro nevidomé, a právě tato rovina by měla být považována za rozhodující.",
          "Libuše Ludíková — logoterapie",
          "Milan Valenta — vzdělávání neslyšících v klášterní tradici, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 39,
        "B": 4,
        "C": 2,
        "D": 5
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 5,
    "label": "Baterie 5",
    "title": "Intervence, reedukace, kompenzace a podpůrné strategie",
    "subtitle": "Krátké diferenční položky na cíl zásahu, mechanismus podpory, AAK, TEACCH, bazální stimulaci a terapeuticko-formativní rámce.",
    "purpose": "Ověřit, zda uchazeč přesně rozliší reedukaci, kompenzaci, metodickou úpravu, pomůcku, AAK, strukturovanou podporu a terapeuticko-formativní přístupy podle cíle, mechanismu a funkčního účinku.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "reedukace a kompenzace",
      "rehabilitační a terapeuticko-formativní rámce",
      "ergoterapie a fyzioterapie",
      "bazální stimulace a bazální dialog",
      "AAK a funkční komunikace",
      "TEACCH a strukturované učení",
      "pomůcka, metoda a úprava prostředí",
      "účinek podpory v cílové situaci"
    ],
    "breakdown": [
      "14× reedukace × kompenzace × rehabilitační rámec",
      "14× terapeuticko-formativní přístupy, pojmy a osobnosti",
      "10× AAK a funkční komunikace",
      "6× pomůcka × metoda × úprava prostředí",
      "6× účinnost podpory podle cíle a účinku"
    ],
    "tags": [
      "intervence",
      "reedukace",
      "kompenzace",
      "AAK",
      "TEACCH",
      "bazální stimulace",
      "UPOL"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Jak nejlépe vyložit situaci, když podpora míří na zlepšení právě té oslabené funkce, v níž dítě dlouhodobě selhává?",
        "options": [
          "Jde spíše o reedukační logiku než o kompenzační.",
          "Jde spíše o kompenzační logiku než o reedukační.",
          "Jde spíše o substituční logiku než o reedukační.",
          "Jde spíše o facilitační logiku než o reedukační."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 2,
        "text": "Jak nejlépe vyložit situaci, když podpora zachovává cíl činnosti, ale obchází oslabenou funkci jinou zachovanou cestou?",
        "options": [
          "Jde spíše o reedukaci než o kompenzaci.",
          "Jde spíše o kompenzaci než o reedukaci.",
          "Jde spíše o rehabilitaci než o kompenzaci.",
          "Jde spíše o obecnou facilitaci než o kompenzaci."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 3,
        "text": "Který znak nejvíce ukazuje, že zvolený zásah je reedukační?",
        "options": [
          "Sleduje, zda oslabená funkce sama vykazuje postupný výkonový posun.",
          "Sleduje, zda je možné oslabenou funkci stabilně obcházet jiným kanálem.",
          "Sleduje, zda se snižuje nárok cílové činnosti, a právě tato rovina by měla být považována za rozhodující.",
          "Sleduje, zda se zvyšuje dopomoc okolí, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 4,
        "text": "Který znak nejvíce ukazuje, že zvolený zásah je kompenzační?",
        "options": [
          "Posuzuje, zda oslabená funkce sama vykazuje růst po nácviku, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Posuzuje, zda je cílová činnost zvládnutelná díky jiné zachované cestě.",
          "Posuzuje, zda dítě vydrží delší výkonové drily, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Posuzuje, zda se mění tempo nácviku oslabené dovednosti, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 5,
        "text": "Které vymezení nejpřesněji odpovídá reedukaci?",
        "options": [
          "cílený nácvik oslabené funkce s očekáváním jejího zlepšení",
          "využití jiné funkční cesty při zachování stejného účelu činnosti",
          "úprava prostředí tak, aby byla činnost přehlednější a méně zatěžující",
          "obecné snížení nároků bez vazby na typ bariéry"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 6,
        "text": "Které vymezení nejpřesněji odpovídá kompenzaci?",
        "options": [
          "opakovaný nácvik oslabené funkce ve stupňovaných krocích",
          "využití jiné zachované funkce nebo cesty k dosažení téhož účelu",
          "rozfázování úkolu tak, aby byl přehlednější",
          "obecné zjednodušení výkonu bez vazby na cíl"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 7,
        "text": "Kdy už není přesné mluvit o reedukaci?",
        "options": [
          "když cílem je přímo posílit oslabenou funkci",
          "když se sleduje posun právě v té dovednosti, která byla oslabená",
          "když se volí jiný kanál proto, aby činnost zůstala dostupná i bez přímého posunu oslabené funkce",
          "když zásah postupuje od jednodušších kroků k náročnějším"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 8,
        "text": "Které tvrzení nejpřesněji rozlišuje kompenzaci a prosté zjednodušení?",
        "options": [
          "Kompenzace zachovává cíl, ale mění cestu; prosté zjednodušení může změnit i samotný výkonový cíl.",
          "Kompenzace i zjednodušení vždy mění cíl činnosti, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Kompenzace je vždy dočasná, zjednodušení vždy trvalé, a právě tato rovina by měla být považována za rozhodující.",
          "Kompenzace je odborná, zjednodušení neodborné, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 9,
        "text": "Co by zde bylo nejméně přesné tvrdit, když je cílem úlohy porozumět textu a škola převede text do hlasové podoby?",
        "options": [
          "Jde spíše o kompenzační zpřístupnění než o reedukační nácvik čtení, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde spíše o zachování obsahového cíle než o přímou práci na oslabeném čtení.",
          "Jde spíše o změnu cesty k cíli než o změnu cíle samotného, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Jde spíše o reedukační nácvik čtenářské techniky než o kompenzační podporu."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 10,
        "text": "Jak situaci vyložit nejpřesněji, když je cílem zásahu zlepšit grafomotorické vedení tahu a přesnost písemného záznamu pravidelným nácvikem?",
        "options": [
          "Jde spíše o kompenzační postup.",
          "Jde spíše o reedukační postup.",
          "Jde spíše o substituční postup.",
          "Jde spíše o čistě organizační úpravu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 11,
        "text": "Který ukazatel nejlépe ukazuje, že postup je kompenzační, ne reedukační?",
        "options": [
          "Zlepšuje se přímo ta oslabená dovednost, která byla cílem nácviku, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Cílová činnost je funkčně zvládnutelná i bez přímého posunu v oslabené funkci.",
          "Snižuje se počet kroků v reedukačním cvičení, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Dítě vydrží delší trénink oslabené funkce, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 12,
        "text": "Který ukazatel nejlépe ukazuje, že postup je reedukační, ne kompenzační?",
        "options": [
          "Oslabená funkce sama vykazuje zlepšení v úloze, na kterou zásah mířil.",
          "Výkon je možný jen po zavedení jiné zachované cesty, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Zátěž činnosti se přesunula na pomůcku nebo na doprovod, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Dítě lépe snáší situaci, ale oslabená funkce se nemění, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 13,
        "text": "Co by zde bylo nejméně přesné tvrdit, když dítě píše obsahově lépe na klávesnici než rukou a cílem úkolu není grafomotorika?",
        "options": [
          "Klávesnice zde může mít kompenzační funkci, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o změnu cesty k zachování obsahového cíle, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Nejde nutně o přímou práci na zlepšení grafomotoriky, a právě tato rovina by měla být považována za rozhodující.",
          "Jde především o reedukační nácvik rukopisné složky výkonu."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 14,
        "text": "Jak nejlépe vystihnout vztah reedukace a rehabilitace?",
        "options": [
          "Reedukace může být jedním z konkrétnějších postupů uvnitř širšího rehabilitačního rámce.",
          "Rehabilitace je užší pojem a reedukace je jeho nadřazená kategorie, a právě tato rovina by měla být považována za rozhodující.",
          "Oba pojmy jsou v odborném použití plně zaměnitelné, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Rehabilitace patří jen zdravotnictví, reedukace jen školství, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 15,
        "text": "Které vymezení nejlépe odlišuje ergoterapii od fyzioterapie v kontextu podpory dítěte?",
        "options": [
          "ergoterapie míří více k funkčnímu využití činností a participaci, fyzioterapie primárně k pohybovým funkcím a motorickému systému",
          "ergoterapie míří více k pohybovým funkcím, fyzioterapie k nácviku denních činností",
          "ergoterapie i fyzioterapie jsou v dětské podpoře plně zaměnitelné",
          "ergoterapie i fyzioterapie se liší hlavně délkou sezení, ne cílem zásahu"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 16,
        "text": "Jak nejlépe vymezit ergoterapii?",
        "options": [
          "cílená práce s činnostmi směřující k soběstačnosti, funkčnímu výkonu a participaci",
          "cílená práce s artikulační přesností a hlasovým vedením",
          "cílená práce s vizuální strukturací prostředí u PAS",
          "cílená práce s existenciálním smyslem a hodnotou"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 17,
        "text": "Jak nejlépe vymezit fyzioterapii?",
        "options": [
          "práce s pohybovým systémem, motorikou a tělesnými funkcemi ve vztahu k pohybu a držení těla",
          "práce s výběrem symbolického systému AAK",
          "práce s náhradou mluvené komunikace",
          "práce s terapeutickým významem a existenciální orientací"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 18,
        "text": "Které tvrzení o ergoterapii je nepřesné?",
        "options": [
          "může pracovat s nácvikem běžných denních činností",
          "může sledovat funkční účast v reálných situacích",
          "její hlavní smysl spočívá jen v nácviku hlásek a hlasu",
          "souvisí s využitím činnosti jako prostředku podpory"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 19,
        "text": "Které tvrzení o fyzioterapii je nepřesné?",
        "options": [
          "může souviset s pohybovými funkcemi a motorickým systémem",
          "může mít význam pro držení těla, koordinaci nebo mobilitu",
          "její hlavní smysl spočívá primárně v podpoře funkční komunikace a náhradě řeči",
          "její cíl se váže k pohybové stránce fungování"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 20,
        "text": "Jak nejlépe vymezit bazální stimulaci?",
        "options": [
          "koncept práce s jednoduchými podněty, jistotou, orientací a kvalitou kontaktu",
          "koncept systematického nácviku oslabené čtecí dovednosti",
          "koncept strukturovaného učení a vizuálního rozvrhování dne",
          "koncept orientovaný primárně na hledání smyslu a hodnoty"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 21,
        "text": "Které tvrzení o bazální stimulaci je nepřesné?",
        "options": [
          "může podporovat orientaci, jistotu a kontakt",
          "bývá spojována i s pojmem bazální dialog",
          "je nejvíce spojena s Andreasem D. Fröhlichem",
          "jejím hlavním smyslem je především náhrada řeči symbolickým systémem"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 22,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "Andreas D. Fröhlich — bazální stimulace",
          "Viktor Frankl — strukturované učení TEACCH",
          "Viktor Lechta — logoterapie",
          "Karel Neubauer — bazální dialog"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 23,
        "text": "Které přiřazení pojmu a přístupu je správné?",
        "options": [
          "bazální dialog — bazální stimulace",
          "bazální dialog — logoterapie",
          "bazální dialog — AAK",
          "bazální dialog — reedukace čtení"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 24,
        "text": "Který přístup je nejvíce spojován se strukturovanou podporou osob s PAS?",
        "options": [
          "logoterapie",
          "TEACCH",
          "bazální stimulace",
          "stuttering modification"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 25,
        "text": "Jak nejlépe vymezit TEACCH?",
        "options": [
          "strukturování prostředí, času a činností s důrazem na předvídatelnost a vizuální oporu",
          "hledání smyslu a hodnoty v prožívání a rozhodování",
          "práce s jednoduchými kontaktními a smyslovými podněty",
          "systematický nácvik oslabené funkce ve stupňovaných krocích"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 26,
        "text": "Jak nejlépe vyložit situaci, když dítě s PAS zvládá činnost výrazně lépe po jasném prostorovém členění, vizuálním rozvrhu a předvídatelném sledu kroků?",
        "options": [
          "Situace je bližší strukturovanému učení než reedukačnímu nácviku samotné oslabené funkce.",
          "Situace je bližší logoterapii než strukturovanému učení, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Situace je bližší bazální stimulaci než strukturovanému učení, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Situace je bližší substituci řeči než práci s prostředím, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 27,
        "text": "Které tvrzení o logoterapii vystihuje věc nejlépe?",
        "options": [
          "je nejvíce spojena s Viktorem Franklem a s důrazem na smysl",
          "je nejvíce spojena s Andreasem D. Fröhlichem a bazální stimulací",
          "je nejvíce spojena s TEACCH a strukturací prostředí",
          "je nejvíce spojena s AAK a náhradou mluvené řeči"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 28,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "Viktor Frankl — logoterapie",
          "Andreas D. Fröhlich — sociální příběhy",
          "Charles Van Riper — bazální stimulace",
          "Viktor Lechta — TEACCH"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 29,
        "text": "Jak nejlépe vyložit situaci, když je hlavním cílem práce otázka smyslu, osobního postoje a životní orientace člověka v těžké situaci?",
        "options": [
          "Je bližší logoterapeutický rámec než strukturované učení nebo reedukační trénink.",
          "Je bližší bazálně-stimulační rámec než logoterapeutický, a právě tato rovina by měla být považována za rozhodující.",
          "Je bližší AAK než logoterapeutický rámec, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Je bližší reedukaci pravopisu než logoterapeutickému rámci, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 30,
        "text": "Jak nejlépe vystihnout AAK?",
        "options": [
          "jde o soubor prostředků, které mohou řeč doplňovat nebo nahrazovat",
          "jde výhradně o technické komunikátory s hlasovým výstupem",
          "jde jen o podporu artikulace při lehkých vadách výslovnosti",
          "jde jen o přechodnou podporu do nástupu spontánní řeči"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 31,
        "text": "Které tvrzení nejpřesněji rozlišuje augmentativní a alternativní komunikaci?",
        "options": [
          "augmentativní komunikace řeč podporuje nebo rozšiřuje, alternativní ji může nahrazovat",
          "augmentativní komunikace je bez pomůcek, alternativní jen s pomůckami",
          "augmentativní komunikace je určena dětem, alternativní dospělým",
          "augmentativní komunikace je krátkodobá, alternativní vždy trvalá"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 32,
        "text": "Která volba je v této situaci nejvhodnější, pokud má dítě výrazně omezenou mluvenou produkci, ale potřebuje funkčně sdělovat volby a potřeby?",
        "options": [
          "uvažovat o AAK jako o funkční cestě ke sdělování potřeb a voleb",
          "ponechat komunikaci výhradně na odhadu okolí",
          "odložit veškerou komunikaci do doby zlepšení řeči",
          "soustředit se jen na grafický záznam bez komunikačního cíle"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 33,
        "text": "Které tvrzení o výběru AAK je nepřesné?",
        "options": [
          "systém má odpovídat motorickému, zrakovému a kognitivnímu profilu uživatele",
          "důležitá je i použitelnost systému v přirozeném prostředí",
          "hlavním kritériem má být to, zda systém působí moderněji než jiné varianty",
          "funkční použitelnost je důležitější než reprezentativní vzhled"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 34,
        "text": "Která volba nejpřesněji odpovídá augmentativnímu použití AAK?",
        "options": [
          "systém podporuje existující komunikaci a zvyšuje její použitelnost",
          "systém nahrazuje komunikaci tam, kde je běžná řeč plně funkční",
          "systém slouží jen k testování porozumění",
          "systém se používá jen tehdy, když je řeč zcela nepřítomná"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 35,
        "text": "Která volba nejpřesněji odpovídá alternativnímu použití AAK?",
        "options": [
          "systém doplňuje stávající řeč o další oporu",
          "systém nahrazuje mluvenou řeč tam, kde ta není funkčně použitelná",
          "systém slouží hlavně k nácviku výslovnosti",
          "systém slouží jen ke zklidnění prostředí"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 36,
        "text": "Který znak nejvíce ukazuje, že AAK byla zvolena funkčně správně?",
        "options": [
          "uživatel s ní dokáže v běžných situacích účinně sdělit potřebu, volbu nebo záměr",
          "systém je technicky nápadný a působí moderně",
          "dospělým se díky němu lépe organizuje režim bez ohledu na komunikaci uživatele",
          "systém má rozsáhlý manuál, i když se v praxi neužívá"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 37,
        "text": "Které tvrzení o komunikačním partnerovi v AAK vystihuje věc nejlépe?",
        "options": [
          "funkčnost systému závisí i na tom, zda okolí rozumí jeho použití a skutečně jej přijímá do komunikace",
          "partner má být spíše pasivní, aby nenarušoval samostatnost uživatele",
          "partner je důležitý jen u nízkotechnologických systémů",
          "komunikační partner nemá na účinnost AAK podstatný vliv"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 38,
        "text": "Která úprava je nejspíše metodickou úpravou prostředí, ne reedukací ani pomůckou?",
        "options": [
          "kratší, jasněji členěné zadání s vizuálním zvýrazněním klíčových kroků",
          "systematický nácvik oslabené dovednosti ve stupňovaných krocích",
          "využití komunikátoru s hlasovým výstupem",
          "využití klávesnice místo ručního psaní tam, kde cílem není grafomotorika"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 39,
        "text": "Jak nejlépe vyložit situaci, když škola zavede kontrastnější materiál, zvětšení a přehlednější grafické členění, aby žák mohl pracovat se stejným obsahem?",
        "options": [
          "Jde především o úpravu přístupnosti prostředí a materiálu.",
          "Jde především o reedukační nácvik oslabené funkce, a právě tato rovina by měla být považována za rozhodující.",
          "Jde především o bazální stimulaci, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Jde především o logoterapeutický zásah, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 40,
        "text": "Které tvrzení nejpřesněji rozlišuje pomůcku a metodu?",
        "options": [
          "Pomůcka je prostředek nebo nástroj, metoda je způsob práce s úkolem nebo učením.",
          "Pomůcka i metoda jsou v odborné praxi plně zaměnitelné, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Pomůcka je vždy odborný postup a metoda vždy technické zařízení, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Pomůcka se týká jen kompenzace, metoda jen reedukace, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 41,
        "text": "Které tvrzení o úpravě prostředí je nepřesné?",
        "options": [
          "může být klíčová tam, kde bariéra souvisí s přehledností, organizací nebo senzorickou zátěží",
          "nemusí sama o sobě přímo zlepšovat oslabenou funkci",
          "může zvýšit dostupnost činnosti bez změny jejího obsahového cíle",
          "vždy představuje reedukační postup"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 42,
        "text": "Který zásah je nejspíše pomůckou, ne metodickou úpravou?",
        "options": [
          "rozdělení dlouhé instrukce do kratších kroků",
          "zvýraznění klíčového místa v zadání",
          "komunikátor s hlasovým výstupem použitý k funkčnímu dorozumění",
          "úprava pořadí kroků v činnosti"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 43,
        "text": "Kdy je úprava prostředí nejvhodnější volbou?",
        "options": [
          "když hlavní bariéra souvisí s přehledností, senzorickou zátěží nebo organizací práce v daném prostředí",
          "když je cílem přímo posílit oslabenou funkci systematickým tréninkem",
          "když má být hlavním výsledkem zlepšení artikulace",
          "když je cílem určit hlavní osobnost spojenou s metodou"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Které tvrzení nejlépe vystihuje situaci, když žák zvládá stejný obsah po rozdělení úkolu do kratších kroků, ale ztrácí se v jeho celku?",
        "options": [
          "Rozfázování zde funguje jako metodická úprava přístupu k úkolu.",
          "Rozfázování zde dokazuje, že cíl úkolu má být trvale snížen, a právě tato rovina by měla být považována za rozhodující.",
          "Rozfázování zde představuje kompenzační pomůcku, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Rozfázování zde znamená, že hlavní bariéra leží v obsahu samotném."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 45,
        "text": "Jak nejlépe vystihnout vztah pomůcky a cíle?",
        "options": [
          "Vhodnost pomůcky se posuzuje podle toho, zda pomáhá dosáhnout cíle v situaci, pro kterou byla zavedena.",
          "Vhodnost pomůcky se posuzuje hlavně podle technické náročnosti, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Vhodnost pomůcky se posuzuje podle toho, zda zní odborněji než metoda, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Vhodnost pomůcky se posuzuje hlavně podle toho, zda snižuje obtížnost bez dalšího, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 46,
        "text": "Který ukazatel nejlépe ukazuje, že reedukační zásah opravdu funguje?",
        "options": [
          "zlepšuje se právě ta oslabená dovednost, na kterou zásah cílil",
          "dítě působí při práci klidněji než dřív",
          "zásah je rodiči hodnocen jako odborný",
          "zásah snižuje organizační náročnost okolí"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 47,
        "text": "Který ukazatel nejlépe ukazuje, že kompenzační zásah opravdu funguje?",
        "options": [
          "cílová činnost je funkčně zvládnutelná díky jiné zachované cestě",
          "zlepšuje se oslabená funkce sama o sobě bez změny cesty",
          "prodlužuje se čas reedukačního nácviku",
          "zvyšuje se počet použitých metod v plánu"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 48,
        "text": "Co by zde bylo nejméně přesné tvrdit, když nová pomůcka funguje jen v jedné silně vedené ukázce, ale v běžné cílové situaci ne?",
        "options": [
          "Je třeba hodnotit účinek v cílové situaci, pro kterou byla zavedena, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jedna ukázková hodina sama o sobě nestačí k potvrzení účinnosti, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Zatím není jisté, že pomůcka plní svou funkci v běžném provozu, a právě tato rovina by měla být považována za rozhodující.",
          "Pomůcka je tímto spolehlivě ověřena jako funkční pro běžné používání."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 49,
        "text": "Na co je při rozhodování mezi reedukací a kompenzací nejdůležitější se ptát?",
        "options": [
          "Máme realisticky posilovat oslabenou funkci, nebo hledat jinou funkční cestu ke stejnému cíli?",
          "Který postup je pro tým jednodušší na zápis?",
          "Který postup působí v dokumentaci odborněji?",
          "Který postup je ve škole používanější bez ohledu na cíl?"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 50,
        "text": "Který znak nejvíce ukazuje, že zvolená terapie, pomůcka nebo metoda míří mimo hlavní cíl?",
        "options": [
          "Posun se ukazuje jen v okrajových situacích, ale ne tam, kde měla podpora primárně fungovat.",
          "Postup vyžaduje opakované používání v čase, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Postup má více než jednu složku, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě tento závěr by pak měl být považován za určující.",
          "Postup pracuje i s přirozeným prostředím dítěte, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 0,
        "correctLetter": "A"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 33,
        "B": 7,
        "C": 5,
        "D": 5
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 6,
    "label": "Baterie 6",
    "title": "Školské poradenství, PO, dokumenty a role",
    "subtitle": "Krátké diferenční položky na podpůrná opatření, dokumenty školy a ŠPZ, role institucí a typy škol.",
    "purpose": "Ověřit, zda uchazeč přesně rozliší školní podporu, poradenský vstup, dokumenty školy a ŠPZ, role jednotlivých subjektů, typy škol a procesní kroky v rámci školského poradenství.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "PO1–PO5 a hranice školní podpory",
      "PLPP, IVP, zpráva a doporučení",
      "škola, ŠPP, ŠPZ, PPP, SPC, SVP",
      "typy škol a právní režimy",
      "procesní další krok školy",
      "implementace doporučení do praxe",
      "vyhodnocování podpory"
    ],
    "breakdown": [
      "18× PO1 a první školní kroky",
      "12× dokumenty a jejich role",
      "10× instituce a pracovníci ve školním poradenství",
      "6× typy škol a právní režim",
      "4× hraniční a snadno zaměnitelné školní situace"
    ],
    "tags": [
      "školské poradenství",
      "PO",
      "PLPP",
      "IVP",
      "PPP",
      "SPC",
      "ŠPZ",
      "UPOL"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Která dvojice tvoří školská poradenská zařízení?",
        "options": [
          "PPP a SPC",
          "PPP a SVP",
          "SPC a ŠPP",
          "ŠPP a SVP"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 2,
        "text": "Ve kterém případě je na místě, aby škola zatím zůstala v režimu vlastní podpory bez návazného doporučení ŠPZ?",
        "options": [
          "Škola ještě neumí přesně vymezit cílovou obtíž, ale chce rovnou žádat o asistenta pedagoga, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o mírnou obtíž, u níž škola umí sama upravit metody, organizaci a hodnocení a průběžně sledovat efekt.",
          "Škola chce co nejrychleji získat formální dokument pro rodiče bez vlastní práce s podporou, a právě tato rovina by měla být považována za rozhodující.",
          "Potíž od začátku vyžaduje tlumočníka českého znakového jazyka a specializované podpůrné opatření vyššího stupně."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 3,
        "text": "Kdy je z hlediska postupu školy na místě zpracovat plán pedagogické podpory?",
        "options": [
          "Až po vydání doporučení ŠPZ, a právě na tomto vysvětlení by měla stát celá interpretace. Právě tento závěr by pak měl být považován za určující.",
          "Když samotné zohlednění individuálních vzdělávacích potřeb žáka při vzdělávání nepostačuje.",
          "Jen tehdy, když škola současně připravuje IVP, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Pouze u žáků podle § 16 odst. 9 školského zákona, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 4,
        "text": "Které tvrzení nejpřesněji rozlišuje zprávu a doporučení?",
        "options": [
          "Zpráva i doporučení mají pro školu stejnou funkci; liší se jen délkou textu, a právě tato rovina by měla být považována za rozhodující.",
          "Zpráva popisuje odborná zjištění a jejich interpretaci, doporučení převádí závěry do podpůrných opatření pro vzdělávání.",
          "Zpráva je dokument školy a doporučení dokument ŠPZ, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě tento závěr by pak měl být považován za určující.",
          "Doporučení slouží hlavně rodičům, zatímco zpráva je určena pouze zřizovateli, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 5,
        "text": "Který subjekt je nejpřesněji hlavním nositelem každodenní realizace podpory po doručení doporučení ŠPZ?",
        "options": [
          "ŠPZ, protože stanovilo podpůrná opatření.",
          "Škola a její pedagogičtí pracovníci.",
          "Zřizovatel školy.",
          "OSPOD."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 6,
        "text": "Které tvrzení o podpůrných opatřeních prvního stupně vystihuje situaci nejlépe?",
        "options": [
          "Jsou vždy navázána na doporučení PPP nebo SPC, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Škola je poskytuje bez doporučení ŠPZ a průběžně vyhodnocuje jejich účinek.",
          "Jsou vyhrazena pouze nadaným žákům, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Slouží jen jako předstupeň převedení do školy podle § 16 odst. 9, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 7,
        "text": "Nejpozději po jaké době škola vyhodnocuje, zda podpůrná opatření prvního stupně vedou ke stanoveným cílům?",
        "options": [
          "po 1 měsíci",
          "po 2 měsících",
          "po 3 měsících",
          "po 6 měsících"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 8,
        "text": "Který dokument je nejvíce spojen se školním plánováním a vyhodnocováním podpory prvního stupně?",
        "options": [
          "doporučení ŠPZ",
          "zpráva ŠPZ",
          "PLPP",
          "program poradenských služeb ve škole"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 9,
        "text": "Pokud škola po vyhodnocení vlastní podpory přechází k dalšímu poradenskému kroku, jaký postup je nejpřiměřenější?",
        "options": [
          "bez dalšího převést žáka do školy podle § 16 odst. 9",
          "doporučit využití poradenské pomoci ŠPZ",
          "nechat další rozhodnutí jen na rodiči bez školního podkladu",
          "automaticky zpracovat IVP bez poradenského vstupu"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 10,
        "text": "Které tvrzení nejlépe vystihuje rozdíl mezi PPP a SPC?",
        "options": [
          "PPP je školské poradenské zařízení, SPC je orgán veřejné správy, a právě tento znak by měl nést hlavní interpretační váhu.",
          "PPP i SPC jsou ŠPZ, ale SPC je více navázáno na specializovanou podporu konkrétních typů postižení.",
          "PPP je určeno jen pro nadané, SPC jen pro žáky s poruchami chování, a právě v tomto směru by se měla hledat hlavní příčina.",
          "PPP vydává doporučení, SPC pouze zprávu, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 11,
        "text": "Která možnost nejpřesněji vystihuje roli školního poradenského pracoviště?",
        "options": [
          "Je totožné se ŠPZ, jen působí uvnitř školy, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o školní poradenskou strukturu, nikoli o školské poradenské zařízení.",
          "Je to správní orgán školy pro rozhodování o podpůrných opatřeních, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Funguje jen v základních školách speciálních, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 12,
        "text": "Který pracovník je typickou součástí ŠPP, nikoli ŠPZ?",
        "options": [
          "speciální pedagog školy",
          "pracovník SPC",
          "pracovník PPP",
          "sociální pracovník centra"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 13,
        "text": "Které tvrzení nejlépe vystihuje roli IVP?",
        "options": [
          "IVP je automatickým výstupem každého poradenského vyšetření, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "IVP je dokument školy, který může vznikat v návaznosti na doporučení a vzdělávací potřeby žáka.",
          "IVP vydává výhradně ŠPZ a škola jej jen archivuje, a právě tento znak by měl nést hlavní interpretační váhu.",
          "IVP nelze využít u nadaného žáka, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 14,
        "text": "Která dvojice nejpřesněji rozlišuje dokument školy a standardní výstup ŠPZ?",
        "options": [
          "PLPP — dokument školy; doporučení — výstup ŠPZ",
          "doporučení — dokument školy; PLPP — výstup ŠPZ",
          "IVP — výstup ŠPZ; zpráva — dokument školy, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "zpráva — dokument školy; doporučení — výstup školy"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 15,
        "text": "Která situace nejpřesněji odpovídá tomu, že škola ještě neumí odborně vyhodnotit vlastní podporu?",
        "options": [
          "Umí doložit cílovou obtíž, zvolenou podporu i kritérium efektu, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Sleduje změnu právě v situaci, pro kterou byla podpora nastavena, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Má jen obecný dojem zlepšení, ale nemá přesně určeno, co vlastně chtěla podporou ovlivnit.",
          "V cílové situaci dochází k pozvolnému naplňování cíle, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 16,
        "text": "Které opatření nejspíše nepatří do podpůrných opatření prvního stupně?",
        "options": [
          "úprava metod a organizace výuky v režii školy",
          "průběžné pedagogické zohlednění vzdělávacích potřeb",
          "normovaná personální podpora typu asistent pedagoga na základě doporučení",
          "úprava hodnocení v rámci vlastní práce školy"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 17,
        "text": "Které tvrzení o SVP odpovídá nejlépe jeho postavení v systému?",
        "options": [
          "SVP je typ školského poradenského zařízení vedle PPP a SPC, a právě v tomto směru by se měla hledat hlavní příčina.",
          "SVP patří do systému preventivně-výchovné péče, nikoli mezi ŠPZ.",
          "SVP je zvláštní typ základní školy podle § 16 odst. 9, a právě tato rovina by měla být považována za rozhodující.",
          "SVP je součást ŠPP na škole, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 18,
        "text": "Která situace nejpřesněji ukazuje na potřebu doporučení ŠPZ, nikoli jen na pokračování v PO1?",
        "options": [
          "Škola má lehkou obtíž, kterou umí sama cíleně podchytit a sleduje zlepšení, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Škola po vyhodnocení vlastní podpory zjišťuje, že cíl není naplňován a další krok potřebuje odborné posouzení.",
          "Učitel chce mít rychle formální dokument bez předchozí práce s podporou, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Rodič se ptá, jaké služby škola obecně nabízí, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě tento závěr by pak měl být považován za určující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 19,
        "text": "Která možnost nejpřesněji vystihuje vztah školy a doporučení ŠPZ?",
        "options": [
          "Doporučení nahrazuje pedagogické plánování školy, a právě tato rovina by měla být považována za rozhodující.",
          "Doporučení vymezuje podpůrný rámec, který škola převádí do každodenní praxe.",
          "Doporučení je interní dokument školy pro první stupeň podpory, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Doporučení má význam jen při změně školy, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 20,
        "text": "Kdo rozhoduje o přijetí dítěte do základní školy speciální?",
        "options": [
          "zřizovatel",
          "ředitel školy v souladu s právním rámcem",
          "PPP",
          "OSPOD"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 21,
        "text": "Jak nejlépe vymezit školu podle § 16 odst. 9 školského zákona?",
        "options": [
          "Jde o každou školu, která vzdělává alespoň jednoho žáka s podpůrnými opatřeními, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o školu nebo třídu zřízenou pro žáky se speciálními vzdělávacími potřebami podle právního rámce.",
          "Jde o jiný název pro základní školu speciální, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde o školské poradenské zařízení, a právě tato rovina by měla být považována za rozhodující. Právě tento závěr by pak měl být považován za určující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 22,
        "text": "Která možnost nejpřesněji vystihuje vztah ZŠ speciální a školy podle § 16 odst. 9?",
        "options": [
          "Jsou to vždy zcela totožné pojmy, a právě v tomto směru by se měla hledat hlavní příčina.",
          "ZŠ speciální je konkrétní typ školy; škola podle § 16 odst. 9 je širší právní kategorie.",
          "ZŠ speciální je zařízení preventivně-výchovné péče, a právě tato rovina by měla být považována za rozhodující.",
          "Škola podle § 16 odst. 9 je jen dočasný režim PO1, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 23,
        "text": "Která skupina je podle vyhlášky o poradenských službách přímo mezi adresáty poradenských služeb?",
        "options": [
          "děti, žáci, studenti, zákonní zástupci, školy a školská zařízení",
          "jen děti a jejich rodiče",
          "jen školy a jejich zřizovatelé",
          "jen žáci se speciálními vzdělávacími potřebami"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 24,
        "text": "Která dvojice nejpřesněji rozlišuje zprávu a doporučení vzhledem ke škole?",
        "options": [
          "zpráva je dokument školy, doporučení dokument ŠPZ",
          "zpráva přináší odborný obraz a interpretaci, doporučení je přímější podklad pro nastavování podpory ve vzdělávání",
          "zpráva se vydává jen rodiči, doporučení jen zřizovateli",
          "zpráva i doporučení plní shodnou funkci, liší se jen rozsahem"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 25,
        "text": "Které tvrzení o nadaném žákovi odpovídá nejlépe?",
        "options": [
          "Nadaný žák stojí zcela mimo poradenský systém, a právě tento znak by měl nést hlavní interpretační váhu.",
          "IVP může být v právním rámci využit i u nadaného žáka.",
          "Nadaný žák nikdy nepracuje s doporučením ŠPZ, a právě v tomto směru by se měla hledat hlavní příčina.",
          "U nadaného žáka nelze použít žádný individuální postup školy."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 26,
        "text": "Který subjekt není školským poradenským zařízením?",
        "options": [
          "pedagogicko-psychologická poradna",
          "speciálně pedagogické centrum",
          "středisko výchovné péče",
          "PPP i SPC"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 27,
        "text": "Která situace nejpřesněji odlišuje PLPP od IVP?",
        "options": [
          "PLPP je dokument ŠPZ a IVP dokument školy, a právě v tomto směru by se měla hledat hlavní příčina. Právě tento závěr by pak měl být považován za určující.",
          "PLPP patří k vlastní školní podpoře prvního stupně, IVP je odlišný dokument školy navázaný na jiný podpůrný rámec.",
          "PLPP se používá jen u nadaných, IVP jen u žáků se smyslovým postižením, a právě tato rovina by měla být považována za rozhodující.",
          "PLPP i IVP vydává ředitel školy bez vztahu k poradenskému procesu, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 28,
        "text": "Který pracovník je typickou součástí školního poradenského pracoviště?",
        "options": [
          "výchovný poradce",
          "pracovník SPC",
          "pracovník PPP",
          "soudní kurátor"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 29,
        "text": "Která situace nejpřesněji odpovídá tomu, že škola záměnila vedlejší efekt podpory za její hlavní účinek?",
        "options": [
          "Žák se v cílové situaci zlepšil a škola to umí doložit předem určeným kritériem, a právě tato rovina by měla být považována za rozhodující.",
          "Učitelé popisují lepší atmosféru, ale hlavní vzdělávací problém se v běžné výuce nepohnul.",
          "Škola přesně rozlišila cíl podpory a vedlejší změny, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Žák dosahuje stabilnějšího výkonu právě v situaci, na kterou byla podpora cílena, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 30,
        "text": "Která možnost nejpřesněji vystihuje postavení školy po doručení doporučení ŠPZ?",
        "options": [
          "Škola už dál nic odborně neřeší, protože dokument podpory je hotový, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Škola musí doporučení převést do konkrétních pedagogických kroků a každodenní realizace podpory.",
          "Škola smí jen archivovat doporučení a čekat na další návštěvu ŠPZ, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Škola doporučení přebírá jen tehdy, pokud s ním souhlasí zřizovatel, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 31,
        "text": "Která možnost nejpřesněji vystihuje vztah školy a ŠPZ při vyhodnocování podpory?",
        "options": [
          "Vyhodnocování podpory je vždy jen věcí školy, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě toto hledisko je zde rozhodující.",
          "Škola průběžně sleduje a vyhodnocuje podporu; při doporučených opatřeních vstupuje do procesu i ŠPZ v rámci své role.",
          "Vyhodnocování provádí výhradně zřizovatel.",
          "Vyhodnocování je jen administrativní krok bez dopadu na další podporu, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 32,
        "text": "Které pracoviště je nejvhodnějším partnerem pro žáka s těžkým zrakovým postižením a potřebou specifických kompenzačních a organizačních opatření?",
        "options": [
          "PPP",
          "SPC",
          "SVP",
          "OSPOD"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 33,
        "text": "Které pracoviště je nejvhodnějším partnerem pro žáka s výraznými obtížemi ve čtení a psaní bez dominantního smyslového postižení?",
        "options": [
          "SPC",
          "PPP",
          "SVP",
          "dětský domov se školou"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 34,
        "text": "Které tvrzení o vysokých školách a školském poradenském systému odpovídá nejlépe?",
        "options": [
          "Vysoká škola standardně postupuje v režimu PO1–PO5 stejně jako ZŠ, a právě tato rovina by měla být považována za rozhodující.",
          "Vysoká škola má vlastní systém podpory studentů se specifickými potřebami, nikoli školní režim PO1–PO5.",
          "Student VŠ musí mít vždy doporučení SPC, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě tento závěr by pak měl být považován za určující.",
          "PLPP je základním dokumentem podpory na vysoké škole, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 35,
        "text": "Která varianta nejpřesněji zachycuje hranici mezi vlastním pedagogickým zohledněním a PLPP?",
        "options": [
          "Jakmile učitel udělá jakoukoli úpravu, musí vždy vzniknout PLPP, a právě tento znak by měl nést hlavní interpretační váhu.",
          "PLPP se zpracovává tehdy, když nepostačuje samotné zohlednění individuálních vzdělávacích potřeb při vzdělávání.",
          "PLPP je třeba vždy až po vydání doporučení, a právě v tomto směru by se měla hledat hlavní příčina. Právě toto hledisko je zde rozhodující.",
          "PLPP slouží jen pro vyšší stupně podpory, a právě na tomto vysvětlení by měla stát celá interpretace. Právě odtud by se proto mělo dále vycházet."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 36,
        "text": "Která možnost nejpřesněji vystihuje, proč není správné ztotožnit ŠPP a ŠPZ?",
        "options": [
          "Protože ŠPP patří mezi orgány veřejné správy, kdežto ŠPZ nikoli, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Protože ŠPP je školní poradenská struktura uvnitř školy, zatímco ŠPZ je externí školské poradenské zařízení.",
          "Protože ŠPP může vydávat doporučení a ŠPZ nikoli, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Protože ŠPP slouží jen nadaným a ŠPZ jen žákům se SVP, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 37,
        "text": "Které tvrzení nejlépe vystihuje funkci doporučení?",
        "options": [
          "Je to dokument školy pro vedení první podpory, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Je to výstup ŠPZ, který navrhuje podpůrná opatření pro vzdělávání.",
          "Jde o interní poznámku učitele po první konzultaci, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o správní rozhodnutí zřizovatele, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 38,
        "text": "Která možnost nejpřesněji vystihuje, co je na PO1 odborně nejnáročnější?",
        "options": [
          "Naučit se používat zkratku PO1 místo slova podpora, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Přesně vymezit cílovou obtíž, nastavit školní podporu a vyhodnotit její účinek v cílové situaci.",
          "Přesvědčit rodiče, aby souhlasili s přeřazením do jiného typu školy, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Bez další práce si připravit argumentaci pro asistenta pedagoga, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 39,
        "text": "Jak nejlépe rozlišit další krok, pokud škola po vyhodnocení PO1 uvažuje o návazném postupu?",
        "options": [
          "Pokud se zlepší klima třídy, není třeba řešit, zda se pohnula hlavní vzdělávací obtíž.",
          "Rozhodující je, zda se posunula právě ta obtíž, na kterou škola podporu cílila.",
          "Jakýkoli dílčí pozitivní efekt znamená, že podpora byla dostačující, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "O dalším kroku rozhoduje až zřizovatel, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 40,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "IVP — výstup ŠPZ; doporučení — dokument školy, a právě tento znak by měl nést hlavní interpretační váhu.",
          "PLPP — dokument školy; doporučení — výstup ŠPZ",
          "doporučení — dokument školy; zpráva — výstup zřizovatele",
          "zpráva — dokument školy; PLPP — výstup ŠPZ"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 41,
        "text": "Která možnost nejpřesněji vystihuje roli rodiče nebo zletilého žáka v procesu doporučení?",
        "options": [
          "Nemá žádnou roli, protože jde výhradně o interní vztah školy a ŠPZ, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Je informován o obsahu zprávy a doporučení a poradenský proces se ho procesně týká.",
          "Vydává souhlas místo školy s každodenní realizací výuky, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Rozhoduje místo ředitele o přijetí do ZŠ speciální, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 42,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "PPP — specializované centrum pro žáky se zrakovým postižením; SPC — široká poradna pro školní zralost, a právě v tomto směru by se měla hledat hlavní příčina.",
          "PPP — širší poradenská práce včetně obtíží učení a chování; SPC — specializovaná speciálněpedagogická podpora podle typu postižení",
          "PPP — zařízení preventivně-výchovné péče; SPC — školní poradenské pracoviště, a právě tato rovina by měla být považována za rozhodující.",
          "PPP — orgán veřejné správy; SPC — sociální služba, a právě tento znak by měl nést hlavní interpretační váhu. Právě tento závěr by pak měl být považován za určující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 43,
        "text": "Co by o škole a doporučení bylo nepřesné tvrdit?",
        "options": [
          "Škola musí podporu převést do běžné pedagogické praxe, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Doporučení samo neodvádí každodenní práci za školu, a právě tato rovina by měla být považována za rozhodující.",
          "Škola po doručení doporučení už jen pasivně plní externě hotový režim bez vlastní pedagogické práce.",
          "Škola nese odpovědnost za realizaci podpory ve výuce, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 44,
        "text": "Který dokument nelze přesně chápat jako přímý nástroj školy pro první stupeň podpory?",
        "options": [
          "PLPP",
          "doporučení",
          "školní pedagogická práce s cílem a vyhodnocením",
          "vlastní úpravy metod a organizace výuky"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 45,
        "text": "Která možnost nejpřesněji vystihuje vztah PO1 a doporučení ŠPZ?",
        "options": [
          "PO1 je vždy důsledkem doporučení ŠPZ, a právě tento znak by měl nést hlavní interpretační váhu. Právě tento závěr by pak měl být považován za určující.",
          "PO1 je prostor školy pro vlastní podporu bez doporučení; doporučení je významné pro další podpůrný rámec.",
          "PO1 i doporučení plní stejnou funkci, liší se jen názvem, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Doporučení nahrazuje vyhodnocování podpory školou, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 46,
        "text": "Která varianta nejpřesněji vystihuje, proč je otázka „do jaké školy žák chodí“ sama o sobě pro poradenský krok často nedostačující?",
        "options": [
          "Protože typ školy nikdy nemá pro podporu žádný význam, a právě odtud by bylo vhodné vyvozovat hlavní závěr. Právě toto hledisko je zde rozhodující.",
          "Protože bez znalosti konkrétní obtíže, podmínek jejího projevu a dosavadní podpory nelze správně určit procesní další krok.",
          "Protože další krok určuje vždy jen zřizovatel, a právě na tomto vysvětlení by měla stát celá interpretace. Právě tento závěr by pak měl být považován za určující.",
          "Protože typ školy je důležitý jen u VŠ, a právě tato rovina by měla být považována za rozhodující. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 47,
        "text": "Která možnost nejpřesněji rozlišuje školní řád a doporučení ŠPZ?",
        "options": [
          "Oba dokumenty mají stejnou funkci, jen vznikají na jiném místě, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Školní řád upravuje vnitřní pravidla školy; doporučení je poradenský výstup k nastavení podpory ve vzdělávání.",
          "Školní řád je dokument ŠPZ a doporučení dokument školy, a právě tato rovina by měla být považována za rozhodující.",
          "Doporučení nahrazuje školní řád u žáků se SVP, a právě tento znak by měl nést hlavní interpretační váhu. Právě odtud by se proto mělo dále vycházet."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 48,
        "text": "Jaký další krok je procesně správný, když škola po vyhodnocení PO1 zjistí, že podpůrná opatření nejsou dostačující?",
        "options": [
          "bezodkladně doporučit využití poradenské pomoci ŠPZ",
          "automaticky zpracovat IVP bez dalšího",
          "trvale snížit nároky bez návazného kroku",
          "ponechat další postup výhradně na rodiči"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 49,
        "text": "Který subjekt nelze přesně považovat za hlavního partnera pro řešení preventivně-výchovných obtíží ve školském systému?",
        "options": [
          "SVP",
          "PPP",
          "SPC",
          "Úřad práce"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 50,
        "text": "Která možnost nejpřesněji vystihuje, co má škola před návazným krokem k ŠPZ umět odborně doložit?",
        "options": [
          "hlavně to, že obtíž působí ve třídě nepříjemně",
          "jaká obtíž byla cílena, jaká školní podpora byla zavedena a podle čeho škola posoudila její efekt",
          "že rodič požaduje co nejrychlejší externí dokument",
          "že se žák liší od spolužáků víc než dříve bez přesnější vazby na vzdělávací situaci"
        ],
        "correct": 1,
        "correctLetter": "B"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 6,
        "B": 37,
        "C": 6,
        "D": 1
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  },
  {
    "id": 7,
    "label": "Baterie 7",
    "title": "Sociální systém, veřejná správa, zařízení a nároky",
    "subtitle": "Krátké diferenční položky na dávky OZP, průkazy, orgány, zařízení a meziresortní hranice.",
    "purpose": "Ověřit přesnou orientaci v dávkách OZP, průkazech, orgánech veřejné správy, typech zařízení a zákonných podmínkách nároku tak, aby uchazeč bezpečně rozlišoval dávku, průkaz, službu, zařízení a rozhodující instituci.",
    "difficulty": "vysoká",
    "dominant": [
      "dávky OZP a jejich podmínky",
      "průkazy TP, ZTP a ZTP/P",
      "příspěvek na mobilitu",
      "příspěvek na zvláštní pomůcku",
      "orgány veřejné správy a rozhodovací kompetence",
      "zařízení ústavní a ochranné výchovy",
      "meziresortní hranice školství, sociální oblasti a veřejné správy"
    ],
    "breakdown": [
      "30× dávky OZP, průkazy a zákonné podmínky",
      "10× orgány veřejné správy a zařízení ústavní péče",
      "10× meziresortní hranice, dětská skupina a školské poradenství"
    ],
    "tags": [
      "hard mode",
      "sociální systém",
      "veřejná správa",
      "dávky OZP",
      "zařízení",
      "nároky"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Kdo rozhoduje v prvním stupni o příspěvku na mobilitu, příspěvku na zvláštní pomůcku a o průkazu osoby se zdravotním postižením?",
        "options": [
          "obecní úřad obce s rozšířenou působností podle místa bydliště",
          "krajská pobočka Úřadu práce ČR, v Praze pobočka pro hlavní město Prahu",
          "Česká správa sociálního zabezpečení podle trvalého pobytu žadatele, a právě tato rovina by měla být považována za rozhodující.",
          "Ministerstvo práce a sociálních věcí jako dávkový orgán prvního stupně"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 2,
        "text": "Kdo rozhoduje o odvolání proti rozhodnutí ve věcech dávek podle zákona č. 329/2011 Sb.?",
        "options": [
          "příslušný krajský úřad podle místa trvalého pobytu",
          "Česká správa sociálního zabezpečení jako nadřízený orgán",
          "Ministerstvo práce a sociálních věcí jako odvolací orgán",
          "správní soud v běžném odvolacím řízení"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 3,
        "text": "Které vymezení nejlépe odpovídá základním podmínkám nároku na příspěvek na mobilitu?",
        "options": [
          "Osoba musí mít průkaz TP nebo ZTP, být starší 1 roku a doložit alespoň jednu pravidelnou cestu měsíčně, a právě tato rovina by měla být považována za rozhodující.",
          "Osoba musí mít průkaz ZTP nebo ZTP/P, být starší 1 roku a v kalendářním měsíci se opakovaně za úhradu dopravovat nebo být dopravována.",
          "Osoba musí mít invaliditu třetího stupně, být starší 1 roku a pravidelně využívat placenou dopravní službu, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Osoba musí mít průkaz ZTP/P, být starší 1 roku a současně pobírat příspěvek na péči alespoň ve III. stupni, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 4,
        "text": "Které tvrzení o pobytových službách a příspěvku na mobilitu je nejpřesnější?",
        "options": [
          "Příspěvek na mobilitu nelze přiznat osobě, která v průběhu roku kdykoli využila pobytovou sociální službu, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Příspěvek na mobilitu nelze přiznat osobě, která byla byť jen krátce hospitalizována nebo jinak mimo domácí prostředí, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Příspěvek na mobilitu nelze přiznat osobě, které jsou po celý kalendářní měsíc poskytovány vymezené pobytové sociální služby nebo ústavní zdravotní péče.",
          "Příspěvek na mobilitu nelze přiznat osobě, která vedle dopravy využívá dlouhodobě jakoukoli institucionální podporu, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 5,
        "text": "Jak se standardně prokazuje splnění podmínky opakovaného dopravování za úhradu u příspěvku na mobilitu?",
        "options": [
          "čestným prohlášením žadatele o opakovaném dopravování",
          "potvrzením dopravce o pravidelném využívání placené dopravy",
          "soupisem cest předloženým škole nebo zaměstnavateli",
          "posudkem registrujícího praktického lékaře o dopravní potřebě"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 6,
        "text": "Jaká je základní měsíční výše příspěvku na mobilitu?",
        "options": [
          "800 Kč měsíčně jako paušální opakující se dávka",
          "1 000 Kč měsíčně jako standardní dávka na dopravu",
          "2 000 Kč měsíčně jako jednotná částka pro držitele ZTP a ZTP/P",
          "900 Kč měsíčně jako opakující se nároková dávka"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 7,
        "text": "Ve kterém případě náleží zvýšená částka 2 900 Kč měsíčně u příspěvku na mobilitu?",
        "options": [
          "když osoba využívá po celý kalendářní měsíc dlouhodobou domácí oxygenoterapii nebo domácí plicní umělou ventilaci",
          "když je osoba držitelem průkazu ZTP/P, je nezletilá a pravidelně je dopravována druhou osobou",
          "když osoba prokáže pravidelné používání specializované dopravy pro osoby se závažným zdravotním postižením",
          "když osoba doloží mimořádně vysoké výdaje na dopravu do zdravotnického zařízení nebo do školy"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 8,
        "text": "Které tvrzení o příspěvku na zvláštní pomůcku je nejpřesnější?",
        "options": [
          "Jde o opakující se měsíční dávku určenou na běžný provoz kompenzačních pomůcek, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o jednorázovou dávku, která má pomoci s pořízením zvláštní pomůcky při splnění zákonných podmínek.",
          "Jde o dávku určenou výhradně dětem se speciálními vzdělávacími potřebami, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o dávku poskytovanou jen tehdy, pokud pomůcku doporučí zdravotní pojišťovna, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 9,
        "text": "Které věkové rozlišení je u příspěvku na zvláštní pomůcku správné?",
        "options": [
          "Motorové vozidlo, úprava bytu i vodicí pes od 1 roku, ostatní pomůcky bez zvláštní věkové hranice.",
          "Všechny zvláštní pomůcky od 3 let, vodicí pes od 18 let a motorové vozidlo až od zahájení školní docházky.",
          "Motorové vozidlo, úprava bytu a plošiny od 3 let, vodicí pes od 15 let, ostatní pomůcky od 1 roku.",
          "Motorové vozidlo od 15 let, vodicí pes od 3 let a úprava bytu jen u osob starších 18 let, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 10,
        "text": "Které vymezení nejlépe odpovídá účelu zvláštní pomůcky pro účely dávky?",
        "options": [
          "Pomůcka musí sloužit výhradně k léčbě zdravotního stavu a nesmí mít vztah k práci, vzdělávání ani kontaktu s okolím, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Pomůcka musí být určena hlavně pro domácí použití a jen okrajově může souviset se školou nebo zaměstnáním, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Pomůcka má umožnit sebeobsluhu nebo být potřebná k práci, přípravě na povolání, získávání informací, vzdělávání nebo styku s okolím.",
          "Pomůcka je uznatelná tehdy, pokud nahrazuje sociální službu nebo snižuje potřebu podpory další osoby, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 11,
        "text": "Které tvrzení o využitelnosti zvláštní pomůcky je správné?",
        "options": [
          "Stačí, aby byla pomůcka obecně vhodná pro daný typ postižení, i když ji konkrétní osoba využít nemůže.",
          "Využitelnost pomůcky se zkoumá jen u dětí, nikoli u dospělých žadatelů, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Využitelnost pomůcky se neposuzuje, pokud její cena přesáhne 10 000 Kč, a právě tato rovina by měla být považována za rozhodující.",
          "Podmínkou je, že osoba může zvláštní pomůcku využívat, nikoli jen že pro ni teoreticky existuje."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 12,
        "text": "Které vymezení podmínky zdravotnického prostředku je nejpřesnější?",
        "options": [
          "Pomůcka nesmí být zdravotnickým prostředkem pořízeným mimo smluvní síť zdravotních zařízení nebo doporučeným soukromým dodavatelem, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Pomůcka nesmí být zdravotnickým prostředkem, který by se používal ve škole, v zaměstnání nebo v rámci běžného sociálního fungování, a právě tato rovina by měla být považována za rozhodující.",
          "Pomůcka nesmí být zdravotnickým prostředkem, pokud současně souvisí s úpravou bytu, pohybem v terénu nebo motorovým vozidlem, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Pomůcka nesmí být zdravotnickým prostředkem hrazeným z veřejného zdravotního pojištění ani prostředkem zapůjčeným zdravotní pojišťovnou."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 13,
        "text": "Které tvrzení o motorovém vozidle jako zvláštní pomůcce je správné?",
        "options": [
          "Rozhodující je pouze to, že osoba sama motorové vozidlo aktivně řídí, a právě tato rovina by měla být považována za rozhodující.",
          "Rozhodující je pouze to, že osoba vlastní řidičské oprávnění skupiny B, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Podmínkou je opakované dopravování v kalendářním měsíci a schopnost vozidlo řídit nebo být jím převážena.",
          "Podmínkou je současné přiznání příspěvku na péči alespoň ve II. stupni, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 14,
        "text": "Které tvrzení o pomůcce, která není výslovně uvedena ve vyhlášce, je nejpřesnější?",
        "options": [
          "Na takovou pomůcku lze dávku poskytnout, pokud ji krajská pobočka Úřadu práce uzná za srovnatelnou s pomůckou ve vyhlášce uvedenou.",
          "Na takovou pomůcku nelze dávku poskytnout, protože seznam druhů a typů pomůcek je pro správní řízení uzavřený, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Na takovou pomůcku lze dávku poskytnout jen po předchozím souhlasu soudu, zdravotní pojišťovny a posudkového lékaře, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Na takovou pomůcku lze dávku poskytnout jen tehdy, když ji doporučí školské poradenské zařízení nebo odborný lékař, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 15,
        "text": "Jak se stanoví spoluúčast u zvláštní pomůcky s cenou nižší než 10 000 Kč?",
        "options": [
          "Spoluúčast činí 1 000 Kč, přičemž příjmově se uplatní hranice osminásobku životního minima, s možností výjimky ze zvláštních důvodů.",
          "Spoluúčast činí vždy 10 % z ceny pomůcky a nepřihlíží se k příjmu ani k sociální situaci žadatele, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Spoluúčast se u pomůcky do 10 000 Kč nestanoví, protože zákon počítá s plnou úhradou drobných kompenzačních pomůcek, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Spoluúčast činí 5 % z ceny pomůcky, nejvýše však 2 000 Kč při nižším příjmu společně posuzovaných osob, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 16,
        "text": "Jak se standardně stanoví spoluúčast u zvláštní pomůcky s cenou vyšší než 10 000 Kč?",
        "options": [
          "Spoluúčast činí pevně 1 000 Kč bez ohledu na cenu pomůcky, její druh nebo příjmovou situaci žadatele, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Spoluúčast činí 5 % z ceny pomůcky, pokud nejde o motorové vozidlo nebo o úpravu bytu, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Spoluúčast činí 15 % z ceny pomůcky, ale u dětí a studentů může být správním orgánem zcela prominuta, a právě tato rovina by měla být považována za rozhodující.",
          "Spoluúčast činí 10 % z předpokládané nebo již zaplacené ceny pomůcky a vychází z hodnoty konkrétní pomůcky."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 17,
        "text": "Které tvrzení o snížení spoluúčasti u dražší zvláštní pomůcky je správné?",
        "options": [
          "Spoluúčast lze snížit jen u nezletilých osob, studentů a držitelů průkazu ZTP/P, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Spoluúčast lze snížit, pokud osoba nemá dostatek prostředků, nejméně však na 1 000 Kč.",
          "Spoluúčast lze snížit jen tehdy, pokud s tím vysloví souhlas soud, opatrovník nebo orgán sociálně-právní ochrany dětí.",
          "Spoluúčast nelze snížit, jakmile cena pomůcky přesáhne hranici 10 000 Kč nebo jde o pomůcku dlouhodobé povahy."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 18,
        "text": "Jaká je obecná maximální výše příspěvku na zvláštní pomůcku?",
        "options": [
          "350 000 Kč s tím, že zvláštní vyšší hranice se uplatní jen u zákonem stanovených zvedacích plošin.",
          "285 000 Kč s tím, že vyšší částka připadá v úvahu jen u motorového vozidla a jeho provozních úprav.",
          "500 000 Kč bez dalšího rozlišení druhu pomůcky, pokud je prokázána dlouhodobá potřeba jejího užívání.",
          "800 000 Kč v rámci jednoho správního řízení, pokud je pomůcka spojena s pohyblivostí nebo přístupností bydlení."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 19,
        "text": "Jaká maximální výše platí pro příspěvek na pořízení svislé zdvihací plošiny nebo šikmé zvedací plošiny?",
        "options": [
          "350 000 Kč jako shodný limit, který zákon stanoví pro všechny dražší zvláštní pomůcky bez výjimky",
          "500 000 Kč jako zvláštní vyšší hranice, která se u těchto plošin uplatní oproti obecnému limitu",
          "400 000 Kč jako zvýšený limit pro stavebně náročné pomůcky spojené s přístupností bydlení",
          "800 000 Kč jako souhrnná částka, kterou lze na plošiny čerpat v jednom kalendářním roce"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 20,
        "text": "Které tvrzení o příspěvku na pořízení motorového vozidla je správné při příjmu vyšším než dvacetinásobek životního minima?",
        "options": [
          "Výše příspěvku činí 285 000 Kč, protože jde o nejnižší příjmové pásmo s nejsilnější veřejnou podporou.",
          "Výše příspěvku činí 245 000 Kč, protože jde o třetí pásmo navazující na hranici šestnáctinásobku životního minima.",
          "Výše příspěvku činí 205 000 Kč, protože jde o předposlední pásmo odstupňované podle rozhodného příjmu.",
          "Výše příspěvku činí 185 000 Kč, protože jde o nejvyšší příjmové pásmo v zákonném odstupňování."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 21,
        "text": "Které tvrzení o příspěvku na pořízení motorového vozidla je správné při příjmu nižším nebo rovném šestnáctinásobku životního minima?",
        "options": [
          "Výše příspěvku činí 185 000 Kč, protože jde o základní vstupní částku pro každou fyzickou osobu, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Výše příspěvku činí 225 000 Kč, protože jde o nejnižší příjmové pásmo podle zákonného členění, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Výše příspěvku činí 350 000 Kč, protože u motorového vozidla se neuplatní samostatné příjmové odstupňování, a právě tato rovina by měla být považována za rozhodující.",
          "Výše příspěvku činí 285 000 Kč, přičemž stejná částka platí i pro nezletilou osobu bez příjmového posuzování."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 22,
        "text": "Co platí, pokud bylo motorové vozidlo pořízeno ještě před podáním žádosti a jeho cena byla nižší než částka, která by jinak vycházela podle zákonného pásma?",
        "options": [
          "Výše příspěvku se stanoví vždy podle zákonného pásma, protože skutečná kupní cena nemá pro správní řízení význam, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Výše příspěvku se stanoví ve výši ceny, za kterou bylo vozidlo skutečně zakoupeno, i když je nižší než zákonná pásmová částka.",
          "Výše příspěvku se v takovém případě automaticky sníží o pevnou spoluúčast ve výši dvaceti procent z ceny vozidla, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Výše příspěvku se vůbec nestanoví, protože nákup vozidla před podáním žádosti vylučuje možnost dávku přiznat, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 23,
        "text": "Které tvrzení o souhrnném limitu vyplacených příspěvků na zvláštní pomůcku v období 60 po sobě jdoucích měsíců je správné?",
        "options": [
          "Součet nesmí přesáhnout 500 000 Kč bez ohledu na druh pomůcky, zdravotní stav ani stavební úpravy bydlení.",
          "Součet nesmí přesáhnout 350 000 Kč, protože jde o obecnou nejvyšší částku pro jednu zvláštní pomůcku.",
          "Součet nesmí přesáhnout 285 000 Kč, pokud byl mezi pořízenými pomůckami motorový vůz nebo úprava bytu.",
          "Součet nesmí přesáhnout 800 000 Kč, nebo 850 000 Kč, pokud byla v tomto období poskytnuta plošina."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 24,
        "text": "Které tvrzení o předchozím souhlasu soudu u nezletilého dítěte nebo osoby s omezenou svéprávností je nejpřesnější?",
        "options": [
          "Předchozí souhlas soudu se nevyžaduje při rozhodování o nároku a výši příspěvku ani při samotném nákupu pomůcky a navazujících řízeních.",
          "Předchozí souhlas soudu je nutný vždy, protože jde o majetkové plnění poskytované v oblasti veřejné sociální podpory, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Předchozí souhlas soudu se nevyžaduje jen u pomůcek s cenou do 10 000 Kč nebo u pomůcek pořizovaných školou, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Předchozí souhlas soudu se nevyžaduje pouze tehdy, když za osobu jedná zákonný zástupce společně s opatrovníkem, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 25,
        "text": "Které vymezení nejlépe odpovídá průkazu TP?",
        "options": [
          "Jde o průkaz pro těžké funkční postižení pohyblivosti nebo orientace, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o průkaz pro zvlášť těžké nebo úplné postižení s potřebou průvodce, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o průkaz určený výhradně osobám s úplnou ztrátou pohyblivosti, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde o průkaz pro středně těžké funkční postižení pohyblivosti nebo orientace."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 26,
        "text": "Které vymezení nejlépe odpovídá průkazu ZTP?",
        "options": [
          "Jde o průkaz pro středně těžké funkční postižení pohyblivosti nebo orientace.",
          "Jde o průkaz pro těžké funkční postižení pohyblivosti nebo orientace.",
          "Jde o průkaz pro zvlášť těžké postižení s potřebou průvodce v každém případě.",
          "Jde o průkaz, který je právně totožný s průkazem TP/P, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 27,
        "text": "Které vymezení nejlépe odpovídá průkazu ZTP/P?",
        "options": [
          "Jde o průkaz pro těžké funkční postižení pohyblivosti nebo orientace bez dalšího rozlišení, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde o průkaz pro jakékoli zrakové postižení bez ohledu na funkční závažnost, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o průkaz pro zvlášť těžké nebo úplné postižení pohyblivosti nebo orientace s potřebou průvodce.",
          "Jde o průkaz, který se přiznává automaticky při nároku na příspěvek na mobilitu, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 28,
        "text": "Které rozlišení průkazů TP, ZTP a ZTP/P je nejpřesnější?",
        "options": [
          "Rozlišují se podle stupně funkčního omezení pohyblivosti nebo orientace, u ZTP/P navíc s potřebou průvodce.",
          "Rozlišují se hlavně podle věku žadatele a teprve v druhém kroku podle závažnosti zdravotního stavu, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Rozlišují se hlavně podle typu diagnózy a až následně podle praktického fungování v oblasti pohybu a orientace.",
          "Rozlišují se podle toho, zda osoba současně pobírá příspěvek na mobilitu, příspěvek na péči nebo obě dávky."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 29,
        "text": "Které tvrzení o dočasném průkazu a běžném průkazu osoby se zdravotním postižením je nejpřesnější?",
        "options": [
          "Oba se vydávají jako polykarbonátové karty shodného charakteru, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Dočasný průkaz vydává soud a běžný průkaz Úřad práce, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Dočasný průkaz je určen pouze dětem a běžný průkaz pouze dospělým, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Dočasný průkaz je papírová karta zatavená do fólie, běžný průkaz je polykarbonátová karta."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 30,
        "text": "Které tvrzení o vztahu mezi průkazem OZP a dávkou podle zákona č. 329/2011 Sb. je nejpřesnější?",
        "options": [
          "Průkaz OZP je dávka a příspěvek na mobilitu představuje pouze potvrzení o zdravotním stavu a dopravní potřebě, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Průkaz OZP je samostatný nárokový status, zatímco příspěvek na mobilitu a příspěvek na zvláštní pomůcku jsou dávky.",
          "Průkaz OZP vzniká jen tehdy, když osoba současně pobírá příspěvek na mobilitu i příspěvek na zvláštní pomůcku, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Průkaz OZP je jen přílohou rozhodnutí o dávce a bez přiznání peněžité dávky samostatně nevzniká, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 31,
        "text": "Které vymezení nejlépe odpovídá OSPOD?",
        "options": [
          "Jde o školské poradenské zařízení zaměřené na poruchy chování a rodinnou spolupráci, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o sociální službu určenou rodinám dětí se zdravotním postižením, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde o orgán sociálně-právní ochrany dětí, nikoli o školské zařízení nebo sociální službu.",
          "Jde o zařízení, které vykonává ústavní a ochrannou výchovu u nezletilých, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 32,
        "text": "Které rozlišení role OSPOD a soudu je nejpřesnější?",
        "options": [
          "OSPOD rozhoduje o ústavní výchově a soud následně jen určuje konkrétní zařízení, kde bude dítě umístěno, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "OSPOD může v ochraně dítěte významně působit a podávat návrhy, ale o ústavní či ochranné výchově rozhoduje soud.",
          "OSPOD vede správní řízení o dávkách a soud do této oblasti standardně nevstupuje ani při ohrožení dítěte, a právě tento znak by měl nést hlavní interpretační váhu.",
          "OSPOD i soud mají v těchto věcech stejnou rozhodovací pravomoc a liší se jen formou vedeného řízení, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 33,
        "text": "Které vymezení nejlépe odpovídá ochranné výchově?",
        "options": [
          "Jde o opatření určené jen pro děti bez rodinného zázemí, když není k dispozici pěstounská péče, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o běžnou preventivní práci školy a OSPOD bez potřeby soudního rozhodnutí, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Jde o dobrovolný pobyt ve školském zařízení na základě žádosti zákonného zástupce, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o ochranné opatření ukládané soudem, které má jiný právní i funkční charakter než ústavní výchova."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 34,
        "text": "Které vymezení nejlépe odpovídá diagnostickému ústavu?",
        "options": [
          "Jde o dlouhodobé cílové zařízení pro děti bez poruch chování, které nemají vlastní rodinné zázemí, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Jde o školské poradenské zařízení, které vydává doporučení ke vzdělávání žáků se speciálními vzdělávacími potřebami, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o vstupní a rozřazovací článek systému, kde probíhá diagnostické, výchovné, vzdělávací a sociální posouzení dítěte.",
          "Jde o pobytovou sociální službu pro rodiny dětí s problémovým chováním a zvýšenými výchovnými obtížemi, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 35,
        "text": "Které tvrzení o dětském domově je nejpřesnější?",
        "options": [
          "Jde o zařízení primárně pro starší děti s uloženou ochrannou výchovou a výraznou agresivitou, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o zařízení, které nahrazuje pedagogicko-psychologickou poradnu v obtížných rodinných případech, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o zařízení primárně pro děti s nařízenou ústavní výchovou, které nemají závažné poruchy chování.",
          "Jde o druh sociální služby, který je obsahově totožný s dětským domovem se školou, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 36,
        "text": "Které tvrzení o dětském domově se školou je nejpřesnější?",
        "options": [
          "Jde o zařízení pro děti s nařízenou ústavní výchovou nebo uloženou ochrannou výchovou se závažnějšími poruchami chování.",
          "Jde o zařízení pro děti se smyslovým postižením, které potřebují internátní vzdělávání a trvalou speciálněpedagogickou podporu.",
          "Jde o školské poradenské zařízení se zaměřením na poradenství v oblasti chování a spolupráci se školou, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o běžný dětský domov doplněný o vlastní základní školu, aniž by se měnila cílová skupina zařízení, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 37,
        "text": "Které tvrzení o výchovném ústavu je nejpřesnější?",
        "options": [
          "Jde o zařízení pro výkon ústavní nebo ochranné výchovy, typicky pro starší děti se závažnými poruchami chování.",
          "Jde o vstupní diagnostické zařízení pro krátkodobé rozřazení dětí do dalších typů péče, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o poradenské zařízení preventivně-výchovné péče bez pobytového režimu, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Jde o správní orgán, který rozhoduje o omezení osobní svobody dítěte, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 38,
        "text": "Které vymezení nejlépe odpovídá středisku výchovné péče (SVP)?",
        "options": [
          "Jde o školské poradenské zařízení téhož typu jako PPP a SPC, a právě tato rovina by měla být považována za rozhodující.",
          "Jde o školské zařízení preventivně-výchovné péče, které pracuje mimo rámec ústavní a ochranné výchovy.",
          "Jde o sociální službu poskytovanou výhradně podle zákona o sociálních službách, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Jde o orgán veřejné správy, který ukládá výchovná opatření dítěti a rodině, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 39,
        "text": "Které formy práce SVP nejlépe odpovídají studijním podkladům?",
        "options": [
          "jen ambulantní a pobytová, bez dalšího členění",
          "jen terénní a krizová telefonická intervence",
          "ambulantní, celodenní a internátní služby",
          "pouze internátní pobyt s následnou diagnostikou"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 40,
        "text": "Které rozlišení je z hlediska systému nejpřesnější?",
        "options": [
          "PPP, SPC a SVP jsou tři rovnocenné typy školských poradenských zařízení, které se liší hlavně cílovou skupinou, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "PPP je orgán veřejné správy, SPC zdravotnické zařízení a SVP sociální služba poskytovaná podle jiného zákona, a právě v tomto směru by se měla hledat hlavní příčina.",
          "SVP je zvláštní dávkový orgán, který navazuje na OSPOD a na rozhodnutí soudu v oblasti výchovy dítěte, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "PPP a SPC patří mezi školská poradenská zařízení, zatímco SVP má zvláštní postavení v oblasti preventivně-výchovné práce."
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 41,
        "text": "Které vymezení nejlépe odpovídá službě péče o dítě v dětské skupině?",
        "options": [
          "Jde o předškolní vzdělávání podle školského zákona realizované v menším kolektivu dětí s pružnější organizací, a právě v tomto směru by se měla hledat hlavní příčina.",
          "Jde o sociální službu určenou primárně dětem se zdravotním postižením do sedmi let věku a jejich rodinám, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Jde o neziskovou pravidelnou péči o dítě od 6 měsíců věku do zahájení povinné školní docházky, poskytovanou mimo obydlí dítěte ve skupině dětí.",
          "Jde o neformální hlídání dítěte bez veřejnoprávního rámce a bez zákonem vymezených organizačních podmínek, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 42,
        "text": "Které rozlišení dětské skupiny a mateřské školy je nejpřesnější?",
        "options": [
          "Dětská skupina je druhem mateřské školy a liší se jen menší kapacitou, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "Dětská skupina je služba péče o dítě podle zvláštního zákona, nikoli škola podle školského zákona.",
          "Mateřská škola je sociální služba a dětská skupina je školské poradenské zařízení, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Oba instituty jsou právně totožné, liší se jen zřizovatelem a názvem zařízení, a právě odtud by bylo vhodné vyvozovat hlavní závěr."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 43,
        "text": "Komu jsou podle vyhlášky č. 72/2005 Sb. poskytovány poradenské služby ve školách a školských poradenských zařízeních?",
        "options": [
          "dětem, žákům, studentům, jejich zákonným zástupcům, školám a školským zařízením",
          "pouze dětem, žákům a studentům, nikoli jejich zákonným zástupcům nebo samotným školám",
          "pouze školám a školským zařízením, protože jde především o institucionální podporu výuky a vzdělávání",
          "pouze zletilým žákům a studentům, protože jde o odbornou konzultační službu s individuálním souhlasem"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Které tvrzení o bezplatnosti standardních poradenských služeb je nejpřesnější?",
        "options": [
          "Bezplatné jsou jen služby poskytované přímo ve škole, nikoli služby poskytované ve školském poradenském zařízení, a právě tento znak by měl nést hlavní interpretační váhu.",
          "Bezplatné jsou jen služby určené mimořádně nadaným žákům, nikoli žákům se speciálními vzdělávacími potřebami, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Bezplatné jsou standardní poradenské služby v ŠPZ a ve školách zřizovaných státem, krajem, obcí nebo svazkem obcí.",
          "Bezplatné jsou jen služby poskytované na základě soudního rozhodnutí, revize doporučení nebo krizového zásahu, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 45,
        "text": "Které rozlišení souhlasu s poradenskou službou je nejpřesnější?",
        "options": [
          "V ŠPZ je u psychologické, speciálně pedagogické nebo logopedické služby vyžadován písemný souhlas; ve škole se standardně nevyžaduje, nestanoví-li jiný právní předpis jinak.",
          "Ve škole i v ŠPZ je vždy nutný písemný souhlas zákonného zástupce bez ohledu na druh služby a právní rámec, a právě v tomto směru by se měla hledat hlavní příčina. Právě odtud by se proto mělo dále vycházet.",
          "Písemný souhlas se vyžaduje jen u zletilého žáka, nikoli u zákonného zástupce nezletilého dítěte nebo žáka, a právě na tomto vysvětlení by měla stát celá interpretace. Právě tento závěr by pak měl být považován za určující.",
          "Písemný souhlas se nevyžaduje nikdy, pokud škola službu považuje za potřebnou a odůvodněnou, a právě tato rovina by měla být považována za rozhodující. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 46,
        "text": "Které tvrzení o informování žáka a zákonného zástupce před poskytnutím poradenské služby v ŠPZ je nejpřesnější?",
        "options": [
          "ŠPZ má předem srozumitelně informovat o povaze, rozsahu, trvání, cílech, postupech, očekávaném prospěchu i právech a povinnostech spojených se službou.",
          "ŠPZ má informovat jen o době a místě vyšetření, protože ostatní náležitosti jsou součástí výsledné zprávy, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "ŠPZ má informovat především školu, protože ta je hlavním adresátem navržených podpůrných opatření a doporučení, a právě tato rovina by měla být považována za rozhodující.",
          "ŠPZ má informovat jen tehdy, pokud už dříve existovalo doporučení z jiného poradenského zařízení nebo školy, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 47,
        "text": "V jaké lhůtě má být poradenská služba v ŠPZ zahájena, nestanoví-li jiný právní předpis jinak?",
        "options": [
          "bezodkladně, nejpozději do 30 dnů od podání žádosti",
          "nejpozději do 6 měsíců od přijetí žádosti, pokud žadatel doložil školní dokumentaci",
          "v nejbližším školním pololetí po ukončení vstupního posouzení případu",
          "bez zbytečného odkladu, nejpozději do 3 měsíců od přijetí žádosti"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 48,
        "text": "Které tvrzení o krizové poradenské intervenci je nejpřesnější?",
        "options": [
          "Zahajuje se ve stejné lhůtě jako běžná poradenská služba, tedy nejpozději do tří měsíců od přijetí žádosti, a právě tato rovina by měla být považována za rozhodující.",
          "Poskytuje se bezodkladně v naléhavých ohrožujících situacích a standardní pravidlo souhlasu se zde neuplatní stejným způsobem.",
          "Poskytuje se až po vystavení zprávy a doporučení, aby byl znám další školní i poradenský postup, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Poskytuje se jen ve škole a nelze ji poskytovat ve školském poradenském zařízení jako samostatný typ služby, a právě v tomto směru by se měla hledat hlavní příčina."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 49,
        "text": "Které rozlišení služby, orgánu a zařízení je nejpřesnější?",
        "options": [
          "OSPOD je orgán veřejné správy, dětská skupina je služba péče o dítě a SVP je školské zařízení.",
          "OSPOD je sociální služba, dětská skupina je orgán veřejné správy a SVP je dávkový institut, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "OSPOD je školské zařízení, dětská skupina je poradenské zařízení a SVP je sociální dávka, a právě v tomto směru by se měla hledat hlavní příčina.",
          "OSPOD je zařízení preventivně-výchovné péče, dětská skupina je škola a SVP je soudní opatření."
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 50,
        "text": "Které obecné pravidlo je pro orientaci v této oblasti nejspolehlivější?",
        "options": [
          "Rozhodující je hlavně to, zda pojem zní sociálně, školsky nebo zdravotnicky; přesná kompetence už bývá až druhotná, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "Nejspolehlivější je vždy současně rozlišit, zda jde o dávku, průkaz, orgán veřejné správy, službu nebo zařízení, a teprve potom řešit konkrétní podmínku či limit.",
          "U většiny položek stačí poznat, zda jde o dítě nebo dospělého; další členění systému už nebývá podstatné, a právě na tomto vysvětlení by měla stát celá interpretace.",
          "V této oblasti bývá správná ta možnost, která shrnuje co nejvíce různých institucí, dokumentů a nároků do jedné odpovědi, a právě tato rovina by měla být považována za rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 13,
        "B": 13,
        "C": 12,
        "D": 12
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": true
    }
  },
  {
    "id": 8,
    "label": "Baterie 8",
    "title": "Finální selekční baterie",
    "subtitle": "Krátké rozlišovací položky z pojmů, institucí, dokumentů, psychologie, historie a sociálně-legislativní orientace.",
    "purpose": "Ověřit skutečně selekční připravenost na UPOL: přesně rozlišit blízké pojmy, instituce, dokumenty, role, historické orientační body a sociálně-legislativní minimum bez stylistických vodítek.",
    "difficulty": "velmi vysoká",
    "dominant": [
      "definiční rozlišení a blízké pojmové rodiny",
      "instituce, role a zařízení",
      "školský proces, PO a dokumenty",
      "sociálně-legislativní orientace",
      "psychologicko-poradenské minimum",
      "historie oboru a osobnosti",
      "krátká aplikace bez dominující kazuistiky"
    ],
    "breakdown": [
      "13× definice a jemné pojmové rozlišení",
      "11× instituce, role a zařízení",
      "8× školský proces, PO a dokumenty",
      "6× sociálně-legislativní orientace a dávky",
      "6× psychologicko-poradenské minimum",
      "4× historické osobnosti a periodizace",
      "2× krátká aplikace"
    ],
    "tags": [
      "finální selekce",
      "pojmové rodiny",
      "instituce",
      "historie oboru",
      "psychologie",
      "UPOL",
      "poradenství"
    ],
    "durationMinutes": 30,
    "questionCount": 50,
    "questions": [
      {
        "number": 1,
        "text": "Který pojem označuje začlenění nové zkušenosti do již existujícího kognitivního schématu, aniž by bylo nutné toto schéma zásadně přestavět?",
        "options": [
          "akomodace",
          "asimilace",
          "ekvilibrace",
          "internalizace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 2,
        "text": "Jaká je základní měsíční výše příspěvku na mobilitu podle aktuálních údajů MPSV v roce 2026?",
        "options": [
          "800 Kč",
          "900 Kč",
          "1 000 Kč",
          "2 900 Kč"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 3,
        "text": "Nejpozději po jaké době škola vyhodnocuje, zda podpůrná opatření prvního stupně vedou ke stanoveným cílům?",
        "options": [
          "po 1 měsíci",
          "po 2 měsících",
          "po 3 měsících",
          "po 6 měsících"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 4,
        "text": "S pojmem speciální pedagogika bývá v českém prostředí nejvíce spojován:",
        "options": [
          "Josef Zeman",
          "Ludvík Edelsberger",
          "Bohumír Popelář",
          "Miloš Sovák"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 5,
        "text": "Který termín nejlépe označuje cílené využití jiné zachované funkce při trvalejší bariéře funkce oslabené?",
        "options": [
          "reedukace",
          "kompenzace",
          "substituce",
          "facilitace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 6,
        "text": "Zvýšená měsíční sazba příspěvku na mobilitu 2 900 Kč se váže zejména na osoby:",
        "options": [
          "s domácí oxygenoterapií nebo domácí plicní ventilací",
          "se všemi držiteli průkazu ZTP/P bez další podmínky",
          "s jakoukoli invaliditou třetího stupně",
          "s mimořádným nadáním a dopravní bariérou"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 7,
        "text": "Která složka Freudova modelu osobnosti reprezentuje internalizované normy, zákazy a nároky?",
        "options": [
          "ego",
          "self",
          "id",
          "superego"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 8,
        "text": "Pojem nápravná pedagogika bývá v českém vývoji spojován zejména s osobností:",
        "options": [
          "František Čáda",
          "Jan Mauer",
          "Miloš Sovák",
          "Bohumír Popelář"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 9,
        "text": "Který termín nejlépe označuje cílený nácvik oslabené funkce s očekáváním jejího zlepšení?",
        "options": [
          "kompenzace",
          "reedukace",
          "rehabilitace",
          "facilitace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 10,
        "text": "Ontogenetická psychologie se nejpřesněji zabývá:",
        "options": [
          "výhradně dějinami psychologických směrů",
          "změnami psychiky člověka v průběhu individuálního vývoje",
          "pouze dědičností rozumových schopností",
          "výlučně psychopatologií dospělého věku"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 11,
        "text": "Který výstup školského poradenského zařízení převádí zjištění do návrhu podpůrných opatření pro školní praxi?",
        "options": [
          "zpráva",
          "doporučení",
          "plán pedagogické podpory",
          "individuální vzdělávací plán"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 12,
        "text": "Které přiřazení limitů příspěvku na zvláštní pomůcku je správné?",
        "options": [
          "obecně 285 000 Kč; u plošiny 350 000 Kč",
          "obecně 350 000 Kč; u plošiny 500 000 Kč",
          "obecně 350 000 Kč; u motorového vozidla 500 000 Kč",
          "obecně 500 000 Kč; u plošiny 800 000 Kč"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 13,
        "text": "Pojem defektologie bývá v českém vývoji spojován zejména s osobností:",
        "options": [
          "Bohumír Popelář",
          "Miloš Sovák",
          "Ludvík Edelsberger",
          "Jan Mauer"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 14,
        "text": "Který pojem nejlépe odpovídá vytěsnění nepřijatelných obsahů mimo vědomí?",
        "options": [
          "racionalizace",
          "represe",
          "projekce",
          "sublimace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 15,
        "text": "Kdo v běžném veřejném školství zpravidla nese investiční výdaje spojené s rekonstrukcí a architektonickými úpravami školy?",
        "options": [
          "ředitel školy z běžného provozního rozpočtu",
          "zřizovatel",
          "školské poradenské zařízení",
          "MŠMT jako přímý správce každé školy"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 16,
        "text": "S novodobým vzděláváním nevidomých bývá nejvíce spojován:",
        "options": [
          "Pedro Ponce de León",
          "Valentin Haüy",
          "Louis Braille",
          "Viktor Lechta"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 17,
        "text": "Integrativní speciální pedagogika je v českém prostředí nejvíce spojována s osobností:",
        "options": [
          "Marie Vítková",
          "Libuše Ludíková",
          "Milan Valenta",
          "Josef Slowík"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 18,
        "text": "Maximální výše příspěvku na zvláštní pomůcku na pořízení motorového vozidla činí k březnu 2026:",
        "options": [
          "185 000 Kč",
          "250 000 Kč",
          "285 000 Kč",
          "350 000 Kč"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 19,
        "text": "Který autor je nejvíce spojován se sociokulturním pojetím vývoje a učení?",
        "options": [
          "Jean Piaget",
          "Lev Vygotskij",
          "Ivan Pavlov",
          "Abraham Maslow"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 20,
        "text": "Kdo rozhoduje o přijetí dítěte do základní školy speciální?",
        "options": [
          "zřizovatel na návrh školy",
          "rodič bez další právní vazby",
          "ředitel školy v souladu s právním rámcem a s příslušnými podklady",
          "OSPOD po konzultaci s PPP, a právě tento znak by měl nést hlavní interpretační váhu."
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 21,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "Andreas D. Fröhlich — úprava koktavosti",
          "Viktor Frankl — logoterapie",
          "Charles Van Riper — bazální stimulace",
          "Fritz Redl — oftalmopedie"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 22,
        "text": "Jak je při ceně zvláštní pomůcky vyšší než 10 000 Kč nejpřesněji stanovena spoluúčast žadatele?",
        "options": [
          "spoluúčast se nestanovuje",
          "činí vždy 25 % z ceny pomůcky",
          "činí 10 % z ceny pomůcky; na žádost může být snížena, nejméně však na 1 000 Kč",
          "činí vždy pevně 10 000 Kč"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 23,
        "text": "Klientsky orientovaný přístup je nejvíce spojován s osobností:",
        "options": [
          "Carl Rogers",
          "Alfred Adler",
          "Viktor Frankl",
          "Abraham Maslow"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 24,
        "text": "Který subjekt je typičtější první externí oporou pro žáka se zrakovým postižením a potřebou specifických podpůrných opatření?",
        "options": [
          "speciálně pedagogické centrum",
          "pedagogicko-psychologická poradna",
          "orgán sociálně-právní ochrany dětí",
          "středisko výchovné péče"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 25,
        "text": "Zakladatelská osobnost moderní československé defektologie bývá nejvíce spojována s jménem:",
        "options": [
          "Josef Zeman",
          "Adolf Strümpell",
          "Bohumír Popelář",
          "František Kábele"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 26,
        "text": "Významná historická osobnost vzdělávání neslyšících je nejvíce spojována s osobností:",
        "options": [
          "Pedro Ponce de León",
          "Valentin Haüy",
          "Viktor Lechta",
          "Andreas D. Fröhlich"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 27,
        "text": "Které tvrzení nejlépe vystihuje rozdíl mezi rozhovorem a dialogem?",
        "options": [
          "dialog zdůrazňuje obousměrnost a společné vytváření porozumění",
          "rozhovor je vždy jen administrativní sběr dat",
          "dialog je nutně forma výslechu",
          "rozhovor je nutně neformální a dialog nutně terapeutický"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 28,
        "text": "Která možnost nejpřesněji vystihuje vztah individuálního vzdělávacího plánu a nadaného žáka?",
        "options": [
          "IVP je vyhrazen výhradně žákům se zdravotním postižením",
          "IVP může být v právním rámci využit i u nadaného žáka",
          "IVP nelze použít mimo druhý až pátý stupeň podpory, a právě v tomto směru by se měla hledat hlavní příčina.",
          "IVP u nadaného žáka vydává zřizovatel, a právě na tomto vysvětlení by měla stát celá interpretace."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 29,
        "text": "O umístění dítěte do konkrétního zařízení ústavní nebo ochranné výchovy rozhoduje:",
        "options": [
          "škola podle školního řádu",
          "OSPOD samostatně",
          "soud",
          "školské poradenské zařízení"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 30,
        "text": "Pokud škola potřebuje převést závěry poradenského vyšetření do konkrétní podoby podpůrných opatření pro výuku, rozhodující je především:",
        "options": [
          "školní řád",
          "zpráva",
          "doporučení",
          "třídní kniha"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 31,
        "text": "Dětský domov se školou je určen primárně pro děti:",
        "options": [
          "bez soudního rozhodnutí a bez závažnějších poruch chování",
          "s nařízenou ústavní výchovou nebo uloženou ochrannou výchovou se závažnějšími poruchami chování",
          "výhradně se smyslovým postižením",
          "jen pro žáky s mimořádným nadáním"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 32,
        "text": "Která situace nejpřesněji odpovídá tomu, kdy má škola doporučit využití služeb školského poradenského zařízení?",
        "options": [
          "když po vyhodnocení podpůrných opatření prvního stupně není naplňován jejich cíl a škola sama nedokáže potřebu podpory dostatečně pokrýt",
          "když se učitel rozhodne, že by bylo administrativně jednodušší přejít rovnou na IVP",
          "když škola chce bez dalšího podkladu požádat o přeřazení žáka do školy podle § 16 odst. 9",
          "když rodič žádá o formální dokument ještě před jakoukoli školní podporou"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 33,
        "text": "Která dvojice nejpřesněji rozlišuje dokument školy a standardní výstup školského poradenského zařízení?",
        "options": [
          "PLPP — dokument školy; doporučení — výstup ŠPZ",
          "IVP — dokument školy; doporučení — výstup ŠPZ, a právě odtud by bylo vhodné vyvozovat hlavní závěr.",
          "doporučení — dokument školy; PLPP — výstup ŠPZ",
          "zpráva — dokument školy; doporučení — výstup školy"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 34,
        "text": "Které opatření nejpřesněji odpovídá prvnímu stupni podpůrných opatření bez nutnosti doporučení ŠPZ?",
        "options": [
          "úprava metod, organizace a hodnocení vzdělávání zachycená školou v rámci vlastní podpory",
          "přiznání asistenta pedagoga s normovanou finanční náročností",
          "zajištění tlumočníka českého znakového jazyka jako podpůrného opatření vyššího stupně",
          "systémové stavební úpravy budovy školy jako individuální podpůrné opatření žáka"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 35,
        "text": "Jak nejlépe vystihnout rozdíl mezi podporou ve škole a na vysoké škole?",
        "options": [
          "vysoká škola standardně používá stejný systém PO1–PO5 jako základní škola, jen bez PLPP",
          "vysoká škola zajišťuje podporu studentů se specifickými potřebami vlastním univerzitním systémem, nikoli režimem školních podpůrných opatření PO1–PO5",
          "student vysoké školy může čerpat podporu jen tehdy, pokud má doporučení SPC",
          "základním dokumentem podpory na vysoké škole je doporučení PPP pro IVP"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 36,
        "text": "Škola u žáka po dobu několika týdnů cíleně upravuje způsob zadávání, organizaci práce i průběžné hodnocení. Učitelé ale současně zjišťují, že bez přesnějšího vymezení vzdělávacích dopadů obtíže nedokážou rozhodnout, zda má škola dál setrvat jen u vlastní podpory, nebo už doporučit návaznost na školské poradenské zařízení. Který podklad je pro toto rozhodnutí nejdůležitější?",
        "options": [
          "obecný dojem, že žák při individuálním vedení působí jistěji než dříve",
          "přesný popis cílové školní situace, zavedené podpory a kritéria, podle nichž bude možné posoudit její efekt",
          "předběžná úvaha školy, jaký typ diagnózy by mohl být u žáka nejpravděpodobnější",
          "souhrn toho, které předměty žák zvládá lépe a které hůře bez vazby na podmínky výuky"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 37,
        "text": "Které srovnání nejpřesněji vystihuje situaci, kdy škola ještě pracuje v rámci vlastní podpory, a situaci, kdy už její další postup stojí na kvalitním poradenském vstupu?",
        "options": [
          "dokud škola mění jen metody práce, jde vždy o PO1; jakmile mění i hodnocení, už musí mít doporučení ŠPZ",
          "dokud škola umí sama cíleně nastavovat a vyhodnocovat podporu vzhledem k popsané obtíži, zůstává v rovině vlastní školní práce; jakmile potřebuje odborně opřít další podpůrná opatření o poradenské posouzení, opírá se o výstup ŠPZ",
          "škola může bez ŠPZ upravovat organizaci výuky, ale nikdy nesmí sama měnit způsob zadávání nebo rozsah opory",
          "rozdíl spočívá hlavně v tom, že školní podpora je neformální, zatímco poradenský vstup je důležitý jen kvůli archivaci dokumentace"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 38,
        "text": "Které tvrzení nejlépe vystihuje vztah mezi doporučením ŠPZ a konkrétní školní realizací podpory?",
        "options": [
          "doporučení už samo o sobě určuje přesnou každodenní podobu výuky, takže škola jen mechanicky přebírá hotový režim práce",
          "doporučení vymezuje podpůrný rámec pro vzdělávání, ale škola ještě musí tento rámec převést do konkrétních pedagogických kroků v běžné výuce",
          "doporučení je pro školu významné hlavně tehdy, když chce žáka převést do jiného typu školy",
          "jakmile škola obdrží doporučení, plánování podpory už není pedagogickou otázkou, ale čistě administrativním úkolem"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 39,
        "text": "Která situace nejspíše neukazuje na potřebu přechodu od školní podpory v rámci PO1 k návazné práci se ŠPZ?",
        "options": [
          "škola umí přesně popsat obtíž, zavedla cílenou podporu, ale ani po jejím vyhodnocení nedochází k posunu v cílové situaci",
          "škola sice zaznamenala dílčí zlepšení v okrajové situaci, ale hlavní vzdělávací problém přetrvává v běžné výuce",
          "škola ještě neumí doložit, jakou obtíž chtěla ovlivnit, podle čeho bude změnu sledovat a jaký konkrétní efekt od podpory očekávala",
          "škola průběžně sleduje přesně vymezený cíl podpory a v cílové situaci dochází k pozvolnému, ale zřetelnému naplňování tohoto cíle"
        ],
        "correct": 3,
        "correctLetter": "D"
      },
      {
        "number": 40,
        "text": "Které rozlišení podpůrných opatření je zde věcně nejpřesnější?",
        "options": [
          "individuální vzdělávací plán je vázán jen na žáky s postižením, zatímco plán pedagogické podpory lze použít i u nadaných",
          "plán pedagogické podpory je nástrojem školy pro zachycení a sledování podpory v rámci její vlastní práce, zatímco individuální vzdělávací plán je zvláštní forma organizace vzdělávání navázaná na odlišný právní a podpůrný rámec",
          "IVP i PLPP jsou z hlediska školy zaměnitelné dokumenty; rozdíl je jen v tom, kdo je formálně podepisuje, a právě tato rovina by měla být považována za rozhodující. Právě odtud by se proto mělo dále vycházet. Právě tento závěr by pak měl být považován za určující.",
          "PLPP se používá tehdy, když škola už má doporučení ŠPZ, zatímco IVP si škola vytváří bez poradenského vstupu, a právě tento znak by měl nést hlavní interpretační váhu. Právě tento závěr by pak měl být považován za určující. Právě toto hledisko je zde rozhodující."
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 41,
        "text": "Po jaké době lze podle aktuálního právního rámce opětovně poskytnout příspěvek na zvláštní pomůcku na pořízení motorového vozidla?",
        "options": [
          "po 36 kalendářních měsících",
          "po 60 kalendářních měsících",
          "po 84 kalendářních měsících",
          "po 120 kalendářních měsících"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 42,
        "text": "Kdo rozhoduje a vyplácí dávky osobám se zdravotním postižením včetně příspěvku na mobilitu a příspěvku na zvláštní pomůcku?",
        "options": [
          "orgán sociálně-právní ochrany dětí",
          "krajská pobočka Úřadu práce ČR",
          "Česká správa sociálního zabezpečení",
          "speciálně pedagogické centrum"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 43,
        "text": "Která dvojice je přiřazena správně?",
        "options": [
          "raná péče — sociální služba",
          "pedagogicko-psychologická poradna — sociální služba",
          "orgán sociálně-právní ochrany dětí — školské poradenské zařízení",
          "speciálně pedagogické centrum — orgán veřejné správy"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 44,
        "text": "Pojem prelingvální neslyšící označuje nejpřesněji osobu:",
        "options": [
          "u níž ke ztrátě sluchu došlo po plném rozvoji řeči",
          "u níž sluchová ztráta vznikla před přirozeným rozvojem řeči",
          "která ztratila sluch pouze v období puberty",
          "která má lehkou nedoslýchavost bez podstatného dopadu na vývoj řeči"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 45,
        "text": "S pojmem narušená komunikační schopnost je nejvíce spojován:",
        "options": [
          "Milan Valenta",
          "Karel Neubauer",
          "Viktor Lechta",
          "Libuše Ludíková"
        ],
        "correct": 2,
        "correctLetter": "C"
      },
      {
        "number": 46,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "pedagogická patologie — Adolf Strümpell",
          "léčebná pedagogika — Bohumír Popelář",
          "sociální pedagogika — Jan Mauer",
          "zvláštní péče — Miloš Sovák"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 47,
        "text": "Fritz Redl je nejvíce spojován s:",
        "options": [
          "logoterapií",
          "krizovým terapeutickým rozhovorem po problematické situaci dítěte",
          "bazální stimulací",
          "metodou strukturovaného učení TEACCH"
        ],
        "correct": 1,
        "correctLetter": "B"
      },
      {
        "number": 48,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "speciální pedagogika defektologická — Ludvík Edelsberger",
          "sociální pedagogika — Jan Mauer",
          "zvláštní péče — Miloš Sovák",
          "defektologie — Bohumír Popelář"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 49,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "pedopatologie — František Čáda",
          "duševně úchylné děti — Bohumír Popelář",
          "integrativní speciální pedagogika — Libuše Ludíková",
          "léčebná pedagogika — Josef Zeman"
        ],
        "correct": 0,
        "correctLetter": "A"
      },
      {
        "number": 50,
        "text": "Které přiřazení termínu a osobnosti je správné?",
        "options": [
          "defektologie — Miloš Sovák",
          "speciální pedagogika — Jan Mauer",
          "integrativní speciální pedagogika — František Kábele",
          "nápravná pedagogika — Bohumír Popelář"
        ],
        "correct": 0,
        "correctLetter": "A"
      }
    ],
    "validation": {
      "questionCount": 50,
      "optionCount": 4,
      "answerDistribution": {
        "A": 15,
        "B": 24,
        "C": 9,
        "D": 2
      },
      "distributionTarget": "13/13/12/12 nebo jiná vyvážená distribuce bez převažujícího písmene",
      "distributionBalanced": false
    }
  }
];

window.SCIO_V4_RAW_BATTERIES_HARD.push({
  "id": 9,
  "label": "Baterie 9",
  "title": "Vývoj oboru, celoživotní edukace, nadání a tyflopedická podpora",
  "subtitle": "Historické mezníky a osobnosti, speciální andragogika a gerontagogika, právní a poradenský rámec nadání a systém podpory osob se zrakovým postižením.",
  "purpose": "Ověřit orientaci v historickém vývoji speciální pedagogiky, v pojetí celoživotní edukace (andragogika, gerontagogika), v problematice nadání a v systému podpory osob se zrakovým postižením.",
  "difficulty": "velmi vysoká",
  "durationMinutes": 30,
  "questions": [
    {
      "number": 1,
      "text": "Která událost je správně spojena s rokem 1807 v dějinách péče o osoby se zrakovým postižením?",
      "options": [
        "založení zařízení pro zaopatření a zaměstnávání nevidomých v Praze",
        "založení ústavu pro výchovu a léčení slepců v Praze",
        "zřízení pracoviště pro pracovní přípravu dospělých nevidomých v Praze",
        "otevření samostatného oddělení pro výuku nevidomých řemeslníků v Praze"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 2,
      "text": "Která událost je správně spojena s rokem 1832 v linii péče o nevidomé?",
      "options": [
        "založení zařízení pro zaopatření a zaměstnávání nevidomých v Praze",
        "založení ústavu pro výchovu a léčení slepců v Praze",
        "zřízení samostatného oddělení pro pracovní výcvik nevidomých dětí",
        "otevření zařízení pro přípravu nevidomých učitelů a vychovatelů"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 3,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "Alois Klár – síť pomocných škol, osnovy a učitelská příprava",
        "Josef Zeman – institucionální počátky péče o nevidomé",
        "Bohumír Popelář – pedologický a socializační pohled",
        "Karel Herfort – propojení lékařské diagnostiky a pedagogického vedení"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 4,
      "text": "Které tvrzení nejlépe vystihuje didaktické pojetí Karla Slavoje Amerlinga?",
      "options": [
        "vzdělávání má být vedeno hlavně léčebným režimem a soustavným dozorem",
        "vzdělávání má být postaveno hlavně na pracovní kázni a opakovacím drilu",
        "vzdělávání má vycházet z konkrétní zkušenosti a názorných úkolů",
        "vzdělávání má stát převážně na abstraktním verbálním výkladu a pojmech"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 5,
      "text": "Která vazba odpovídá Karlu Slavoji Amerlingovi?",
      "options": [
        "škola denní, týdenní, roční a sekulární",
        "škola nápravná, opatrovací, pracovní a léčebná",
        "škola obecná, přípravná, praktická a odborná",
        "škola internátní, útulková, pomocná a pokračovací"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 6,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "František Štampach – síť pomocných škol a učitelská příprava",
        "Bohumír Popelář – první užití termínu defektologie",
        "Jan Mauer – pedologický a socializační pohled",
        "František Čáda – pedologický a socializační pohled"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 7,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "Alois Klár – pomocné školství, osnovy a učitelská příprava",
        "Josef Zeman – pomocné školství, osnovy a učitelská příprava",
        "Karel Herfort – institucionální počátky péče o nevidomé",
        "František Štampach – rozvoj audioorální komunikace"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 8,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "Miloš Sovák – první užití názvu speciální pedagogika",
        "Bohumír Popelář – první užití termínu defektologie",
        "František Štampach – reedukace, ochranná a nápravná výchova",
        "Jan Mauer – moderní syntéza oboru a systematizace speciální pedagogiky"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 9,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "Bohumír Popelář – stabilizace názvu speciální pedagogika v českém prostředí",
        "František Čáda – institucionální počátky péče o nevidomé",
        "Alois Klár – rozvoj logopedie a kompenzační pedagogiky",
        "Rudolf Jedlička – pedologický a socializační pohled"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 10,
      "text": "Která dvojice osobnost – odborný akcent je přiřazena správně?",
      "options": [
        "Jan Mauer – rozvoj orientace v prostoru a tyflografiky",
        "Miloš Sovák – moderní syntéza oboru a systematizace speciální pedagogiky",
        "Josef Zeman – přechod od charity k zaměstnávání nevidomých",
        "Boris Titzl – systematizace logopedických diagnóz"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 11,
      "text": "Která událost je správně spojena s rokem 1871?",
      "options": [
        "vznik první pomocné třídy v Praze",
        "založení Jedličkova ústavu v Praze",
        "vznik Ernestina jako ústavu pro děti s mentálním postižením",
        "přijetí zákona o pomocných školách"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 12,
      "text": "Která událost je správně spojena s rokem 1896?",
      "options": [
        "vznik první pomocné třídy v Praze",
        "založení Jedličkova ústavu v Praze",
        "vznik prvních učebních osnov pro školy mládeže úchylné",
        "založení časopisu Úchylná mládež"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 13,
      "text": "Která událost je správně spojena s rokem 1913?",
      "options": [
        "vznik prvních učebních osnov pro školy mládeže úchylné",
        "přijetí zákona o pomocných školách",
        "založení časopisu Úchylná mládež",
        "založení Jedličkova ústavu v Praze"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 14,
      "text": "Která událost je správně spojena s rokem 1924?",
      "options": [
        "vznik prvních učebních osnov a směrnic pro školy mládeže úchylné",
        "založení časopisu Úchylná mládež",
        "první užití termínu defektologie",
        "vznik první pomocné třídy v Praze"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 15,
      "text": "Která událost je správně spojena s rokem 1928?",
      "options": [
        "vznik první pomocné třídy v Praze",
        "založení Jedličkova ústavu v Praze",
        "vznik prvních učebních osnov a směrnic pro školy mládeže úchylné",
        "první užití názvu speciální pedagogika"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 16,
      "text": "Která událost je správně spojena s rokem 1929?",
      "options": [
        "zákonné zařazení pomocných škol do školského systému",
        "první užití termínu defektologie",
        "založení časopisu Úchylná mládež",
        "vznik prvních učebních osnov pro školy mládeže úchylné"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 17,
      "text": "Který autor je spojen s periodizací represivní – zotročovací – charitativní – renesančně humanistické – rehabilitační – socializační – prevenční?",
      "options": [
        "Boris Titzl",
        "Miloš Sovák",
        "František Čáda",
        "Bohumír Popelář"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 18,
      "text": "Který autor je spojen s periodizací předinstitucionální – institucionální azylové – institucionální diferencované – odklon od ústavnosti?",
      "options": [
        "Jan Mauer",
        "František Štampach",
        "Miloš Sovák",
        "Boris Titzl"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 19,
      "text": "Který historický termín nejvíce zdůrazňuje vadu nebo defekt jako výchozí bod odborné práce?",
      "options": [
        "defektologie",
        "pedologie",
        "sociální pedagogika",
        "nápravná pedagogika"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 20,
      "text": "Který historický termín je nejblíže pedagogické protiváze medicínského pojetí?",
      "options": [
        "pedopatologie",
        "sociální pedagogika",
        "nápravná pedagogika",
        "speciální pedagogika"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 21,
      "text": "Kdo je správně spojen s rokem 1833 a s prvním doloženým užitím termínu andragogika?",
      "options": [
        "Emil Livečka",
        "Alexandr Kapp",
        "Ferdinand A. Kehrer",
        "Otto F. Bollnow"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 22,
      "text": "Která osobnost je spojena s rokem 2006 a s definicí speciální andragogiky?",
      "options": [
        "Jesenský",
        "Slowík",
        "Valenta",
        "Langer"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 23,
      "text": "Která podoblast speciální andragogiky se vztahuje k dospělým se sluchovým postižením a osobám ohluchlým?",
      "options": [
        "surdoandragogika",
        "tyfloandragogika",
        "somatoandragogika",
        "logoandragogika"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 24,
      "text": "Která podoblast speciální andragogiky se vztahuje k dospělým se získanými nebo přetrvávajícími poruchami komunikace?",
      "options": [
        "surdoandragogika",
        "psychoandragogika",
        "logoandragogika",
        "etoandragogika"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 25,
      "text": "Která podoblast speciální andragogiky se vztahuje k dospělým se zrakovým postižením?",
      "options": [
        "somatoandragogika",
        "logoandragogika",
        "psychoandragogika",
        "tyfloandragogika"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 26,
      "text": "Která podoblast speciální andragogiky se vztahuje k dospělým s poruchami chování, emocí a sociálních vztahů?",
      "options": [
        "tyfloandragogika",
        "etoandragogika",
        "surdoandragogika",
        "somatoandragogika"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 27,
      "text": "Kdo je správně spojen s rokem 1952 a s prvním užitím pojmu geragogika?",
      "options": [
        "Emil Livečka",
        "Otto F. Bollnow",
        "Ferdinand A. Kehrer",
        "Alexandr Kapp"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 28,
      "text": "Kdo je správně spojen s rokem 1962 a s pojetím gerontagogiky jako výzkumné disciplíny?",
      "options": [
        "Otto F. Bollnow",
        "Emil Livečka",
        "Langer",
        "Ferdinand A. Kehrer"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 29,
      "text": "Kdo je správně spojen s rokem 1979 a s charakteristikou gerontagogiky jako nově budované vědní disciplíny?",
      "options": [
        "Otto F. Bollnow",
        "Ferdinand A. Kehrer",
        "Alexandr Kapp",
        "Emil Livečka"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 30,
      "text": "Které vyjádření nejlépe odpovídá zdrojům gerontagogiky?",
      "options": [
        "pedagogika, fyziologie, audiologie a antropologie",
        "pedagogika, andragogika, gerontologie a psychologie",
        "speciální pedagogika, etopedie, logopedie a patologie",
        "sociologie, demografie, matematika a ergonomie"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 31,
      "text": "Které ustanovení školského zákona ukládá školám vytvářet podmínky pro rozvoj nadání a umožňuje rozšířenou výuku?",
      "options": [
        "§ 17",
        "§ 16",
        "§ 18",
        "§ 120a"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 32,
      "text": "Které ustanovení školského zákona umožňuje vzdělávání mimořádně nadaného žáka podle individuálního vzdělávacího plánu?",
      "options": [
        "§ 17",
        "§ 16",
        "§ 18",
        "§ 116"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 33,
      "text": "Která podmínka je přímo spojena s přeřazením mimořádně nadaného žáka do vyššího ročníku?",
      "options": [
        "předchozí zařazení do skupiny nadaných žáků v rámci školy",
        "předchozí vzdělávání podle individuálního vzdělávacího plánu",
        "předchozí doporučení SPC bez dalšího ověření učiva",
        "vykonání zkoušek z učiva ročníku, který žák neabsolvuje"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 34,
      "text": "Které školské poradenské zařízení má hlavní místo při zjišťování mimořádného nadání a metodické podpoře školy v této oblasti?",
      "options": [
        "SPC",
        "PPP",
        "SVP",
        "OSPOD"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 35,
      "text": "Který dokument je školním dokumentem konkrétního žáka a může být zpracován i pro mimořádně nadaného?",
      "options": [
        "zpráva",
        "doporučení",
        "individuální vzdělávací plán",
        "program poradenských služeb ve škole"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 36,
      "text": "Který dokument vyjadřuje institucionální rámec poradenské práce školy jako celku?",
      "options": [
        "program poradenských služeb ve škole",
        "doporučení školského poradenského zařízení",
        "individuální vzdělávací plán",
        "zpráva z vyšetření"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 37,
      "text": "Která dvojice je správně spojena s výstupy odborného poradenského posouzení ve školském poradenském zařízení?",
      "options": [
        "PLPP a IVP",
        "IVP a zpráva",
        "PLPP a doporučení",
        "zpráva a doporučení"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 38,
      "text": "Které vyjádření nejlépe odlišuje PPP a SPC?",
      "options": [
        "PPP zajišťuje jen diagnostiku, SPC jen metodické vedení",
        "PPP má širší vzdělávací a vývojový záběr, SPC je vázáno na konkrétní typ postižení",
        "PPP vydává jen zprávu, SPC jen doporučení",
        "PPP působí ve škole, SPC mimo školský systém"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 39,
      "text": "Která činnost nejlépe odpovídá SPC pro vady řeči?",
      "options": [
        "orientační logopedické vyšetření a depistáž poruch komunikace",
        "propedeutika Braillova písma a smyslová výchova",
        "nácvik orientace a samostatného pohybu",
        "sluchová výchova a příprava na audiometrii"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 40,
      "text": "Která činnost nejlépe odpovídá SPC pro žáky se zrakovým postižením?",
      "options": [
        "rozvoj českého znakového jazyka a sluchová výchova",
        "stanovení logopedických intervenčních programů a depistáž NKS",
        "propedeutika Braillova písma, tyflografika a zraková stimulace",
        "rozvoj slovní zásoby, alternativní čtení a bazální stimulace"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 41,
      "text": "Která skupina pomůcek zahrnuje lupy, hyperokulární skla a dalekohledové brýle?",
      "options": [
        "pomůcky pro čtení a psaní",
        "elektronická tyflotechnika",
        "optoelektronické pomůcky",
        "optické pomůcky"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 42,
      "text": "Která skupina pomůcek zahrnuje kamerové a televizní lupy, Optacon a zvětšovací zařízení?",
      "options": [
        "optické pomůcky",
        "optoelektronické pomůcky",
        "pomůcky pro orientaci a mobilitu",
        "drobné kompenzační pomůcky"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 43,
      "text": "Která skupina pomůcek zahrnuje počítač s hlasovým nebo hmatovým výstupem, braillský řádek a speciální tiskárny?",
      "options": [
        "optické pomůcky",
        "optoelektronické pomůcky",
        "pomůcky pro čtení a psaní",
        "elektronická tyflotechnika"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 44,
      "text": "Která skupina pomůcek zahrnuje bílou hůl, vodicího psa a orientační majáčky?",
      "options": [
        "pomůcky pro čtení a psaní",
        "drobné kompenzační pomůcky",
        "pomůcky pro orientaci a mobilitu",
        "elektronická tyflotechnika"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 45,
      "text": "Které vyjádření nejlépe vystihuje význam Braillova písma?",
      "options": [
        "soubor úprav černotisku pro slabozraké čtenáře",
        "šestibodový reliéfní systém čtení a psaní pro gramotnost nevidomých",
        "zvukový systém pro převod textu do řeči",
        "reliéfní systém pro mapy, plánky a schémata"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 46,
      "text": "Které vyjádření nejlépe vystihuje pojem tyflografika?",
      "options": [
        "úprava velikosti a kontrastu černotisku pro slabozraké",
        "hlasová navigace při samostatném pohybu nevidomých",
        "nácvik používání vodicího psa v městském prostoru",
        "tvorba a čtení reliéfních map, plánků a schémat"
      ],
      "correct": 3,
      "correctLetter": "D"
    },
    {
      "number": 47,
      "text": "Která organizace je nejvíce spojena s hájením zájmů zrakově postižených, odstraňováním bariér a klubovou činností?",
      "options": [
        "SONS",
        "Tyfloservis",
        "TyfloCentrum",
        "Raná péče"
      ],
      "correct": 0,
      "correctLetter": "A"
    },
    {
      "number": 48,
      "text": "Která organizace je nejvíce spojena se sociální rehabilitací, nácvikem orientace, sebeobsluhy, Braillova písma a tyflografiky?",
      "options": [
        "SONS",
        "TyfloCentrum",
        "Tyfloservis",
        "Raná péče"
      ],
      "correct": 2,
      "correctLetter": "C"
    },
    {
      "number": 49,
      "text": "Která organizace je nejvíce spojena se službami prvního kontaktu, sociálně-právním a pracovně-právním poradenstvím a průvodcovskými či předčitatelskými službami?",
      "options": [
        "SONS",
        "TyfloCentrum",
        "Tyfloservis",
        "SPC"
      ],
      "correct": 1,
      "correctLetter": "B"
    },
    {
      "number": 50,
      "text": "Které vyjádření nejlépe vystihuje význam SPC v podpoře žáka se zrakovým postižením?",
      "options": [
        "propojuje diagnostiku, přímou intervenci, metodické vedení a nácvik práce s pomůckami",
        "zajišťuje hlavně jednorázové vyšetření a předání zprávy škole",
        "řeší především sociální dávky, průkazy a kompenzace mimo školství",
        "nahrazuje školní podporu a přebírá odpovědnost za každodenní výuku"
      ],
      "correct": 0,
      "correctLetter": "A"
    }
  ]
});